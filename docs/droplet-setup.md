...
| `/root/whoeverwants/.env.api` | Gitignored env file for the `api` service (external-API + email secrets; see below) |
...
### Environment Variables (`.env.api`)

`docker-compose.yml`'s `api` service loads `/root/whoeverwants/.env.api` via `env_file`. It's gitignored (holds secrets) and created as an empty placeholder by `provision-droplet.sh` — populate it after provisioning via `scripts/remote*.sh`. **`env_file` is read only on container creation**, so after editing always `docker compose up -d --force-recreate api` (NOT `restart api`).

| Key | Required? | Purpose |
|-----|-----------|---------|
| `RESEND_API_KEY` | **Yes (for email)** | Resend send-only key. Without it, magic-link sign-in + recovery-email links are NOT delivered — `send_email()` just logs the link to stdout and the FE shows "This server isn't configured to send real emails". |
| `RESEND_FROM_EMAIL` | Recommended | Sender address on a Resend-verified domain (e.g. `noreply@contact.whoeverwants.com`). Defaults to `noreply@whoeverwants.com` if unset — which fails unless that exact domain is verified. |
| `INTERNAL_TICK_SECRET` | Yes (auto-generated) | Bearer for `POST /api/internal/tick` (deadline closes + push). Auto-generated by `provision-droplet.sh`. |
| `TMDB_API_KEY` / `RAWG_API_KEY` / `YELP_API_KEY` | Optional | External search providers (movies / games / restaurants). |
| `APNS_KEY_ID` / `APNS_TEAM_ID` / `APNS_AUTH_KEY_P8` | Optional | Apple Push Notification credentials (native iOS pushes). |
| `GOOGLE_OAUTH_CLIENT_IDS` / `APPLE_OAUTH_AUDIENCES` | Optional | OAuth verify-endpoint audiences (enable Google / Apple sign-in). |

> **Email verification:** the sending domain (`contact.whoeverwants.com`) is verified in Resend with DKIM/SPF DNS records in Route 53. Confirm a tier can send with `curl -s -X POST https://api.<tier>/api/auth/magic-link/request -H 'Content-Type: application/json' -H 'Origin: https://<tier-fe>' -d '{"email":"you@example.com"}'` — a healthy tier returns `{"accepted":true,"email_configured":true}`.
