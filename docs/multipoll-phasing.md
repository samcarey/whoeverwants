...
| 3.4 follow-up B — multipoll-level Submit for single-sub-poll multipolls + rename | ⏳ not started | Lifts Submit out of PollPageClient for the 1-sub-poll case too (matches the "sub-polls cannot be submitted by themselves" architecture). Renames PollPageClient → SubPollBallot, moves out of `app/p/[shortId]/`. (`apiSubmitVote`/`apiEditVote` callsites already retired in 3.4 follow-up A.5.) Re-keys `lib/ballotDraft.ts` from per-sub-poll → per-multipoll (one entry per multipoll containing all sub-poll drafts + voter name). |
