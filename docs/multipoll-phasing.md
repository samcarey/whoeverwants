...
| 3.4 follow-up — single Submit at the bottom of multi-sub-poll cards | ⏳ not started | The endpoint exists; rewiring the thread page's per-sub-poll Submit buttons to a single multipoll-level Submit button + per-sub-poll abstain still pending. Per-sub-poll Submit continues to work as a fallback in the meantime. |
