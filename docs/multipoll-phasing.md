...
| 3.3 — non-anchor yes_no external rendering | ✅ on this branch | Every yes_no sub-poll in a multi-group now uses the thread-page external Yes/No card (with tap-to-change → confirmation flow) instead of falling back to `PollPageClient`'s internal yes_no UI for non-anchor sub-polls. External card render moved into the per-sub-poll loop so cards inherit the section label and the proper sibling order. |
...
| 3.4 follow-up A — multipoll-level Submit for multi-sub-poll groups | 🚧 in progress on this branch | Hide per-sub-poll Submit/voter-name/confirmation in `PollPageClient` when it's a sub-poll of a multi-sub-poll group; render one Submit + voter name + confirmation at the multipoll wrapper level (the thread-page card group). PollPageClient exposes `getVoteItem()` / `applyVoteResult()` via ref. Single-sub-poll multipolls keep their existing PollPageClient Submit in this PR (lifted in PR B). |
| 3.4 follow-up A.5 — `PollPageClient.submitVote` routes through `apiSubmitMultipollVotes` | ✅ on this branch | The last per-poll `apiSubmitVote`/`apiEditVote` client callsite (covering ranked_choice / suggestion / time + the unreachable yes_no path) routes through the multipoll endpoint when `poll.multipoll_id` is set. Legacy per-poll branch is preserved as a fallback for `multipoll_id == null` (participation polls + any pre-Phase-4 unbackfilled poll). Decouples the "use the multipoll endpoint" rule from the "lift Submit out of PollPageClient" UI refactor so follow-up B is a pure render-tree change. |
| 3.4 follow-up B — multipoll-level Submit for single-sub-poll multipolls + rename | 🚧 in progress | `lib/ballotDraft.ts` is now per-multipoll: storage key `ballotDraft:m:<multipollId>` holds `{voter_name?, sub_polls: { [subPollId]: SubPollDraft } }`. `loadSubPollDraft(multipollId, subPollId)` / `saveSubPollDraft(...)` / `clearSubPollDraft(...)` route through the multipoll entry; legacy per-sub-poll entries auto-migrate on read; participation polls (`multipollId === null`) keep the legacy per-poll key. Still pending: lift Submit out of `PollPageClient` for the 1-sub-poll case (matches the "sub-polls cannot be submitted by themselves" architecture), rename `PollPageClient` → `SubPollBallot`, move out of `app/p/[shortId]/`. (`apiSubmitVote`/`apiEditVote` callsites already retired in 3.4 follow-up A.5.) |
