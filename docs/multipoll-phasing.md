...
| 3.4 follow-up B — multipoll-level Submit for mixed-type multi-sub-poll groups | ✅ on this branch | Mixed-type multi-sub-poll groups (e.g. yes_no + ranked_choice) now batch through ONE wrapper Submit + ONE ConfirmationModal + ONE atomic `apiSubmitMultipollVotes` call. `useMultipollSubmit` gate drops the `allYesNo` carve-out — fires for every multi-group with a multipoll wrapper. New `SubPollBallotHandle.prepareBatchVoteItem()` ref method synchronously validates + builds a `MultipollVoteItem` and returns `commit(vote)` / `fail(error)` closures capturing per-sub-poll state at build time. Wrapper button click snapshots staged yes_no choices + calls `prepareBatchVoteItem()` on each non-yes_no ref, aggregates items + closures into `pendingMultipollSubmit.preparedNonYesNo`, opens the modal; on confirm `confirmMultipollSubmit` builds the combined items array and calls `apiSubmitMultipollVotes` once. Returned `ApiVote`s distributed via `commit(vote)` per non-yes_no entry + `userVoteMap` updates per yes_no. Validation errors surface inline (via `setVoteError`) AND abort modal opening so the user fixes them before re-clicking Submit. (`apiSubmitVote`/`apiEditVote` callsites already retired in 3.4 follow-up A.5; the legacy branches remain as participation-poll fallbacks.) |
| 3.5 — multipoll-level `follow_up_to` as source of truth | ✅ on this branch | `PollResponse` carries `multipoll_follow_up_to` (the wrapper's follow_up_to, populated via `LEFT JOIN multipolls` in every server SELECT that feeds `_row_to_poll`). FE `lib/threadUtils.ts` walks multipoll-level chains: `buildPollMaps` returns `pollIdsByMultipoll` + `childrenByParentMultipoll`; `collectDescendants` fans every visited sub-poll out to siblings + parent's siblings + every child multipoll's sub-polls. `findThreadRootRouteId(poll, pollByMultipoll?)` walks the chain to root. Server `algorithms/related_polls.py` discovery walks multipoll-level chains; `PollRelation` carries `multipoll_id` + `multipoll_follow_up_to`. The legacy `polls.follow_up_to` column is still populated on writes (Phase 5 retires it) but the FE no longer reads it for chain logic. **Forks were removed entirely in migration 095** as part of this work — only `follow_up_to` remains. |
| 5 — cleanup of legacy columns + dual-codepath branches | ✅ on this branch | Migration 096 dropped polls.{short_id, creator_secret, creator_name, response_deadline, is_closed, close_reason, follow_up_to, thread_title, suggestion_deadline, sequential_id} along with the BEFORE-INSERT `trigger_generate_short_id`. The multipolls table is now the sole source of truth for wrapper-level fields. Server `_SELECT_POLL_FULL` LEFT JOINs polls + multipolls and aliases the wrapper fields under the legacy polls column names so internal logic that reads `row["is_closed"]` etc. keeps working — and so `PollResponse` continues to surface them to the FE without a 30-file callsite refactor. Legacy single-poll mutation endpoints removed: `POST /api/polls`, `POST /api/polls/{id}/{votes,close,reopen,cutoff-suggestions,cutoff-availability,thread-title}`, `PUT /api/polls/{id}/votes/{vote_id}`. New multipoll-level `POST /api/multipolls/{id}/thread-title`. FE drops `apiCreatePoll`/`apiSubmitVote`/`apiEditVote`/`apiClosePoll`/`apiReopenPoll`/`apiCutoffSuggestions`/`apiCutoffAvailability`/`apiUpdateThreadTitle`; new `apiUpdateMultipollThreadTitle`. `Poll.follow_up_to` field removed (use `multipoll_follow_up_to`). `FollowUpHeader` takes a multipoll_id and resolves via `apiGetMultipollById`. Phase 5b (deferred): refactor FE callsites that read `poll.is_closed`/`poll.response_deadline`/etc. to source from the `Multipoll` wrapper directly. |
