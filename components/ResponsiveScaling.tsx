 ...
 * Scaling is disabled for question pages to prevent excessive scroll issues
 */
export default function ResponsiveScaling({
  ...
  // Disable responsive scaling for question pages to prevent scroll issues
  ...
  return (
    <div
      className={`${isQuestionPage ? '' : 'responsive-scaling-container'} ${className}`.trim()}
    >
      {children}
    </div>
  );
