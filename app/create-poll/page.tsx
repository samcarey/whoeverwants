...
export function CreateQuestionContent() {
  ...
  const SEARCH_ROW_CLASS =
    "w-full flex items-center gap-4 px-5 py-[1.75px] text-left min-h-[0.4375rem] active:bg-gray-100 dark:active:bg-gray-800 disabled:opacity-50";
  const pollSearchBar = (
    <div
                ...
                {s.segments.map((seg, i) =>
                  seg.colorBorder ? (
                    ...
                    <span key={i} className="relative inline-block align-baseline">
                      {seg.label && (
                        <span
                          className={`absolute left-0 bottom-full translate-y-[0.6px] text-[9px] font-semibold uppercase tracking-wide leading-none ${seg.colorText}`}
                          aria-hidden
                        >
                          {seg.label}
                        </span>
