 ...
 *
 * The in-page flow deliberately doesn't surface group info BEFORE
 * redemption — we'd rather just navigate the user into the group
 * after they redeem than surface a "preview, click again to redeem"
 * step. The LINK PREVIEW (Open Graph metadata) is the exception:
 * the server-component shell in `./page.tsx` fetches
 * `GET /api/auth/invites/<token>/preview` so a shared invite link
 * shows the group's name in messaging apps. That's the intentional
 * Phase E leak channel: the invite URL is the creator deliberately
 * sharing the group, and the name is what makes the link legible.
 */
