-- Fix RLS issues for ranked choice function using SECURITY DEFINER approach only
-- This is the safest approach - no broad permissions granted to public

-- Update the function to use SECURITY DEFINER so it runs with creator's permissions
-- This allows the function to bypass RLS when managing elimination rounds
CREATE OR REPLACE FUNCTION calculate_ranked_choice_winner(target_poll_id UUID)
RETURNS TABLE(winner TEXT, total_rounds INT) 
SECURITY DEFINER  -- This makes the function run with creator's permissions, bypassing RLS
AS $$
DECLARE
    current_round INT := 1;
