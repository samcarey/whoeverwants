-- Add database-level validation to ensure voter conditions are subsets of poll conditions
-- This enforces data integrity even if client-side validation is bypassed
...
CREATE TRIGGER enforce_voter_conditions_subset
  BEFORE INSERT OR UPDATE ON votes
  FOR EACH ROW
  EXECUTE FUNCTION validate_voter_conditions_subset();
...
COMMENT ON TRIGGER enforce_voter_conditions_subset ON votes IS
'Enforces that participation poll voter conditions must be subsets of poll creator conditions.
Validates days, duration ranges, and time windows before insert/update.';
