-- Replace stub function with full time slot calculation algorithm
-- Uses procedural approach for better reliability
...
BEGIN
  ...
  -- Use a CTE approach that should work better
  FOR slot_rec IN
    WITH
      -- Unnest dates
      candidate_dates AS (
      ...
      candidate_start_times AS (
        SELECT st::time as start_time
        FROM generate_series(
          COALESCE(
            CASE WHEN (poll_rec.time_window->>'minEnabled')::boolean
              THEN (poll_rec.time_window->>'minValue')::time
              ELSE '00:00'::time
            END,
            '00:00'::time
          ),
          COALESCE(
            CASE WHEN (poll_rec.time_window->>'maxEnabled')::boolean
              THEN (poll_rec.time_window->>'maxValue')::time
              ELSE '23:45'::time
            END,
            '23:45'::time
          ),
          ...
          WITH RECURSIVE
            prioritized_voters AS (
              ...
              WHERE v.poll_id = poll_id_param
                ...
                AND (v.voter_days IS NULL OR acs.event_date = ANY(v.voter_days))
...
'Full implementation: Calculates optimal time slots for participation polls by generating candidate slots, filtering eligible voters, running greedy selection, deduplicating overlaps, and grouping into elimination rounds.';
