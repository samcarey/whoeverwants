-- Fix poll_results view to properly calculate winner for ranked choice polls
-- Current issue: winner field is always NULL for ranked choice polls
-- Fix: Query the ranked choice results to determine the actual winner

-- Drop and recreate the view to fix column type issues
...
SELECT 
    ...
    -- Winner determination for both yes/no and ranked choice polls
    CASE 
        WHEN p.poll_type = 'yes_no' THEN
            CASE 
