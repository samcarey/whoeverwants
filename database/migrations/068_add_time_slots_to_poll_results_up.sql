-- Update poll_results view to include time slot information for participation polls
-- Adds winning_time_slot (JSONB) and total_slot_rounds (INTEGER) fields
...
SELECT
    ...
    CASE
        ...
        ELSE NULL
    END as total_rounds,
    -- Winning time slot for participation polls (JSONB)
    CASE
        WHEN p.poll_type = 'participation' THEN
            (
                ...
                FROM (
                    ...
                    WHERE poll_id = p.id AND is_winner = true
                    LIMIT 1
                ) r
            )
        ELSE NULL
    END as winning_time_slot,
    -- Total slot elimination rounds for participation polls
    CASE
        WHEN p.poll_type = 'participation' THEN
            (
                SELECT MAX(round_number)
...
COMMENT ON COLUMN poll_results.winning_time_slot IS
'For participation polls: JSONB containing the winning time slot data (slot_date, slot_start_time, slot_end_time, duration_hours, participant_count, participant_vote_ids, participant_names). NULL if no valid slot exists.';

COMMENT ON COLUMN poll_results.total_slot_rounds IS
'For participation polls: Total number of elimination rounds (grouped by participant count). NULL if no time slot calculation has been performed.';
