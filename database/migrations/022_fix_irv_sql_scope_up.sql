-- Fix IRV function SQL scope issue with all_tied_with_borda CTE
...
BEGIN
    ...
    LOOP
        ...
        ELSE
            ...
            -- Use separate query to find elimination candidate with Borda tie-breaking
            ...
            -- Get elimination candidate (lowest Borda score)
            SELECT atb.tied_candidate, atb.total_borda_score 
            INTO elimination_candidate, elimination_borda_score
            FROM all_tied_with_borda atb
            ORDER BY atb.total_borda_score ASC, atb.tied_candidate ASC  -- Secondary sort for deterministic results
            LIMIT 1;
            
            -- Store Borda scores for tied candidates
            UPDATE ranked_choice_rounds 
            SET 
                borda_score = (
                    SELECT atb.total_borda_score 
                    FROM (
                        SELECT 
                            tc.tied_candidate,
                            COALESCE(bs.total_borda_score, 0) as total_borda_score
                    ...
                    WHERE atb.tied_candidate = ranked_choice_rounds.option_name
                ),
                tie_broken_by_borda = TRUE
            WHERE poll_id = target_poll_id 
              AND round_number = current_round 
              AND option_name = ANY(tied_candidates);
            
            -- Set elimination candidate
