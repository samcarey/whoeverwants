-- Update the poll_results view with ranked choice support
-- Using CREATE OR REPLACE to safely update the view without dropping
...
SELECT 
    ...
    -- Winner determination for yes/no polls only
    CASE 
        WHEN p.poll_type = 'yes_no' THEN
            CASE 
        ...
        ELSE NULL 
    END as winner,
    -- Placeholder for ranked choice data (will be populated by separate function call)
    NULL as total_rounds
FROM polls p
