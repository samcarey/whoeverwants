-- Fix critical bug in ranked choice calculation
-- Bug: Line 74 of original function tries to use array_length() on an INT variable
-- This causes the exit condition to never work properly, leading to incorrect results
...
DECLARE
    ...
    remaining_options INT;  -- FIXED: This is INT, not array (was the bug)
    total_ballots INT;
...
BEGIN
    ...
    LOOP
        ...
        -- Use a safer approach to handle array unnesting
        ...
        -- FIXED: Use remaining_options <= 1 instead of array_length(remaining_options, 1) <= 1
