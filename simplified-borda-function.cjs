...
// Let me try a simpler version that doesn't use nested CTEs in UPDATE
const functionDefinition = `CREATE OR REPLACE FUNCTION calculate_ranked_choice_winner(target_poll_id UUID)
...
DECLARE
    ...
    elimination_candidate TEXT;
    elimination_borda_score INT;
BEGIN
    ...
    LOOP
        ...
        ELSE
            -- BORDA COUNT TIE-BREAKING: Calculate and store Borda scores separately
            -- First, calculate and update Borda scores for tied candidates
            FOR i IN 1..array_length(tied_candidates, 1) LOOP
                -- Calculate Borda score for this candidate
                SELECT COALESCE(SUM(total_candidates - choice_rank + 1), 0) INTO elimination_borda_score
                FROM votes v,
                     unnest(v.ranked_choices) WITH ORDINALITY AS choices(choice_option, choice_rank)
                WHERE v.poll_id = target_poll_id 
                  AND v.vote_type = 'ranked_choice'
                  AND v.ranked_choices IS NOT NULL
                  AND array_length(v.ranked_choices, 1) > 0
                  AND choice_option = tied_candidates[i]
                  AND choice_option IS NOT NULL
                  AND choice_option != '';
                
                -- Update the round record with Borda score
                UPDATE ranked_choice_rounds 
                SET borda_score = elimination_borda_score,
                    tie_broken_by_borda = TRUE
                WHERE poll_id = target_poll_id 
                  AND round_number = current_round 
                  AND option_name = tied_candidates[i];
            END LOOP;
            
            -- Now find candidate with lowest Borda score for elimination
            SELECT option_name INTO elimination_candidate
            FROM ranked_choice_rounds 
            WHERE poll_id = target_poll_id 
              AND round_number = current_round 
              AND vote_count = min_votes
            ORDER BY borda_score ASC, option_name DESC
            LIMIT 1;
            
            options_to_eliminate := ARRAY[elimination_candidate];
        END IF;
...
const options = {
  ...
  port: 443,
  path: '/v1/projects/kfngceqepnzlljkwedtd/database/query',
  method: 'POST',
  headers: {
...
console.log('🔧 Applying simplified Borda function (no nested CTEs)...');
...
const req = https.request(options, (res) => {
  ...
  res.on('end', () => {
    if (res.statusCode >= 200 && res.statusCode < 300) {
      console.log('✅ Simplified Borda function applied successfully!');
    } else {
      console.error('❌ Function update failed:', res.statusCode, body);
    }
  });
});
