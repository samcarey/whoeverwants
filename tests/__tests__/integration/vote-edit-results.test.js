 ...
 *
 * Tests that question results correctly reflect edited votes.
 * Regression test for: preliminary results not updating after vote edits
 * because fetchQuestionResults() was gated by a stale closure value.
 */
import { describe, it, expect, beforeAll } from 'vitest'
import {
  isApiAvailable,
  apiCreateTestQuestion,
  apiSubmitTestVote,
  apiEditTestVote,
  apiGetResults,
...
describe('Vote Edit Results Update', () => {
  it('ranked choice results should reflect edited vote', async () => {
    ...
    // Create a 2-option ranked choice question with preliminary results enabled
    ...
    const vote1 = await apiSubmitTestVote(question.id, {
    ...
    const resultsBefore = await apiGetResults(question.id)
    expect(resultsBefore.winner).toBe('Alpha')

    // Edit vote1 from Alpha to Beta
    await apiEditTestVote(question.id, vote1.id, {
      ranked_choices: ['Beta', 'Alpha'],
      voter_name: 'Voter1',
      is_abstain: false,
    })
    ...
    const resultsAfter = await apiGetResults(question.id)
