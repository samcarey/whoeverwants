...
test.describe('Suggestion Poll Editing', () => {
  test('should create suggestion poll with default suggestions and show edit functionality', async ({ page }) => {
    ...
    // Create suggestion poll with default suggestions (creator's initial vote)
    const pollData = {
    ...
    // Should show "All suggestions:" with creator's suggestions displayed
    await expect(page.locator('text="All suggestions:"')).toBeVisible();

    // Verify creator's default suggestions are shown
    ...
    await page.screenshot({ path: 'test-results/suggestion-with-defaults-created.png' });
  });

  test('should allow editing suggestions and persist changes', async ({ page }) => {
    const homePage = new HomePage(page);
    const createPollPage = new CreatePollPage(page);
    const pollPage = new PollPage(page);
    ...
    const pollData = {
      title: 'Editable Suggestion Poll',
      type: 'suggestion' as const,
    ...
    // The existing suggestions should be pre-filled in input fields
    const inputs = page.locator('input[type="text"]');

    // Verify existing suggestions are in input fields
    await expect(inputs.nth(0)).toHaveValue('Option A');
    await expect(inputs.nth(1)).toHaveValue('Option B');

    // Edit the suggestions - change first option and add a new one
    ...
    // Should return to view mode and show updated suggestions
    await expect(page.locator('text="All suggestions:"')).toBeVisible();
    ...
    await page.screenshot({ path: 'test-results/suggestion-edited-successfully.png' });
  });

  test('should allow selecting/unselecting others suggestions in edit mode', async ({ page, browser }) => {
    // This test requires simulating multiple users, so we'll use two browser contexts
    const homePage = new HomePage(page);
    const createPollPage = new CreatePollPage(page);
    const pollPage = new PollPage(page);

    // Creator creates poll with initial suggestions
    await homePage.goToHomePage();
    await homePage.navigateToCreatePoll();

    const pollData = {
      title: 'Multi-User Suggestion Test',
      type: 'suggestion' as const,
    ...
    // Second user adds their suggestions
    const context2 = await browser.newContext();
    const page2 = await context2.newPage();
    await page2.goto(pollUrl);
    await page2.waitForTimeout(2000);

    // Second user should see existing suggestions and be able to vote
    await expect(page2.locator('text="Existing suggestions"')).toBeVisible();

    // Select existing suggestions and add new ones
    ...
    await newSuggestionInput.fill('Second User Option');
    ...
    // Now go back to creator and test editing with ability to unselect others' suggestions
    await page.reload();
    await page.waitForTimeout(2000);
    ...
    // Should see "Other suggestions" section with second user's suggestion
    await expect(page.locator('text="Other suggestions"')).toBeVisible();
    await expect(page.locator('text="Second User Option"')).toBeVisible();

    // Should be able to select/unselect the other user's suggestion
    const otherSuggestion = page.locator('button:has-text("Second User Option")');

    // Click to select it
    await otherSuggestion.click();
    await expect(otherSuggestion).toHaveClass(/bg-green/);

    // Click to unselect it
    await otherSuggestion.click();
    await expect(otherSuggestion).not.toHaveClass(/bg-green/);

    await page.screenshot({ path: 'test-results/suggestion-unselect-others.png' });
  ...
  test('should show changes to other users in real-time', async ({ page, browser }) => {
    ...
    // Creator edits their suggestion
    await page.click('button:has-text("Edit")');
    await page.waitForTimeout(1000);
    ...
    await page.screenshot({ path: 'test-results/suggestion-changes-creator-view.png' });
    await page2.screenshot({ path: 'test-results/suggestion-changes-other-user-view.png' });
  ...
  test('should hide edit button when poll is closed', async ({ page }) => {
    ...
    // Create suggestion poll with short deadline (1 minute for testing)
    await homePage.goToHomePage();
    await homePage.navigateToCreatePoll();
    ...
    // But suggestions should still be visible in read-only mode
    await expect(page.locator('text="Test Option"')).toBeVisible();

    await page.screenshot({ path: 'test-results/suggestion-poll-closed-no-edit.png' });
  });
});
