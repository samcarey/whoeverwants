...
test.describe('Suggestion Poll Abstain Restrictions', () => {
  test('should disable abstain button when suggestions exist and enable when cleared', async ({ page }) => {
    ...
    // Create suggestion poll with NO initial suggestions to test from scratch
    ...
    // Submit without any options (empty suggestion poll)
    ...
    // Empty suggestion poll creates empty vote, so we need to click Edit to get to voting interface
    await expect(page.locator('text="All suggestions:"')).toBeVisible();
    await expect(page.locator('text="No suggestions available"')).toBeVisible();
    ...
    // Abstain button should be ENABLED initially (no suggestions)
    ...
    const restrictionMessage = page.locator('text="To abstain, you must first remove all your suggestions."');
    ...
    await firstInput.fill('Test Suggestion');
    ...
    await page.screenshot({ path: 'test-results/abstain-02-disabled-with-suggestion.png' });
  ...
  test('should disable abstain when adding multiple suggestions and enable when all cleared', async ({ page }) => {
    const homePage = new HomePage(page);
    const createPollPage = new CreatePollPage(page);

    // Create empty suggestion poll
    await homePage.goToHomePage();
    await homePage.navigateToCreatePoll();
    await createPollPage.verifyPageLoaded();

    await createPollPage.fillTitle('Multiple Suggestions Abstain Test');
    await createPollPage.selectPollType('suggestion');
    await createPollPage.selectDeadline('10min');
    ...
    await inputs.nth(0).fill('Suggestion 1');
    await inputs.nth(1).fill('Suggestion 2');
    await inputs.nth(2).fill('Suggestion 3');
    ...
    // Clear suggestions one by one - should remain disabled until ALL are cleared
  ...
  test('should allow abstain after canceling abstain state and clearing suggestions', async ({ page }) => {
    const homePage = new HomePage(page);
    const createPollPage = new CreatePollPage(page);
    ...
    // Add a suggestion while not abstaining
    const firstInput = page.locator('input[type="text"]').first();
    await firstInput.fill('Test Suggestion');
    await page.waitForTimeout(1000);
  ...
  test('should not allow adding suggestions while abstaining is active', async ({ page }) => {
    const homePage = new HomePage(page);
    const createPollPage = new CreatePollPage(page);
    ...
    await createPollPage.fillTitle('Abstain vs Suggestions Test');
    await createPollPage.selectPollType('suggestion');
    await createPollPage.selectDeadline('10min');
    ...
    // Now should be able to add suggestions
