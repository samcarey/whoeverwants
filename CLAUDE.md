...
- **No bottom bar. No home button.** The old three-button bottom bar (Home / + / Profile) was removed. Navigation is:
  ...
  - **Category bubble bar on group-like pages**: portaled into `#draft-poll-portal`, which lives inside the floating `BubbleBarPanel` (`components/BubbleBarPanel.tsx`) at the viewport bottom on `/g/<id>` and the `/g/` empty placeholder — see the "BubbleBarPanel (Floating Bubble Bar)" section above for the panel's structure, visibility rules, and slide-with-page behavior. The row starts with a bold "+ New" button (catch-all opening the modal with the default `custom` category) followed by one button per `BUILT_IN_TYPES` entry — see `BUBBLE_ENTRIES` in `app/create-poll/page.tsx`. Each entry calls `openModalFor(category)` which seeds a fresh draft via `emptyDraft({category})` and opens the new-question modal. There is no separate draft poll card or always-on creation form. All buttons share the module-scope `BUBBLE_BUTTON_CLASS` constant; the retired trailing "Other" entry (also `custom`-category) was folded into "New" to avoid two buttons doing the same thing. Bubble padding is `px-[13.5px] py-[6.75px]` per a "+50% padding" tuning pass — half-pixel arbitrary values are house style; no border, the blue fill is enough. The panel uses `bg-background` so it blends seamlessly with the page bg and has no top divider — the rounded bubbles are sufficient visual separation from the page content above. The "+" icon scales with its label (1.6× the label's font-size, mirroring the new group button's `text-[28.8px]` over `text-lg`) so the "+ New" bubble reads as the same shape as the "+ Group" FAB on home. Earlier iterations: an underlined sticky "New" label with bordered bubbles + a top border on the panel; before that a 3-row wrapped grid with a centered "Create a New Poll" h2 above. (Earlier iterations had a visible draft card hosting staged questions + a Submit button; that's gone — the modal now submits the entire poll directly. Even earlier iterations had a floating What/When/Where 3-bubble bar with `?mode=time` / `?category=restaurant` URL preselection; also gone — see the historical note in the Active Plan section above. An intermediate iteration kept the bar in-flow at the bottom of the cards-wrapper; the current floating panel replaces that with hide-on-scroll-down behavior.)
