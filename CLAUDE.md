...
- **TODO (container-name clobber in `evict_excess_servers` — root cause of dev servers serving under the WRONG container name + getting reclaimed by a sibling branch):** `evict_excess_servers` (`scripts/mac-mini/dev-server-manager.sh`) runs `while IFS= read -r container ...; do ... done <<< "$victims"` with `container` NOT declared `local`. Because bash is dynamically scoped, that loop assigns to `cmd_upsert`'s `container` (set at the top of `cmd_upsert` from the slug). So whenever eviction actually fires (`effective_total > MAX_DEV_SERVERS` — i.e. the box is busy with several branches), `cmd_upsert` falls through to `docker run --name "$container"` using the LAST evicted victim's container name instead of the upserted slug's name. The `volume` / `db_name` / `BRANCH` env are separate locals and stay correct, so the launched container serves the RIGHT branch's code + DB but wears a SIBLING slug's name. Blast radius: (1) the `=== Dev server ready ===` summary prints the wrong `Container:` line (the giveaway — `Branch`/`Database`/`URL` are correct, `Container` names a different branch); (2) the container is undiscoverable by its own slug (`docker inspect whoeverwants-dev-<slug>` fails) so it's an orphan; (3) the Caddy snippet for the correct slug still points at the right port, so the site works *until* the branch whose name was borrowed gets upserted — that branch's `cmd_upsert` does `docker stop/rm whoeverwants-dev-<borrowed-name>` and destroys the mis-named container, 502-ing the victim hostname (and vanishing any static files served into it, e.g. demo screenshots under `public/screenshots/`). Observed 2026-05-25 on `claude/poll-detail-new-options-banner-6NzPz`: its container came up named `whoeverwants-dev-claude-ios-app-title-truncation-jylpb`, then died when that branch was next pushed. **Fix:** add `local container victim_slug` inside `evict_excess_servers` (or rename the loop var to `victim` so it can't alias `cmd_upsert`'s `container`). Audit every other `while read -r container` / `for container in ...` loop in the manager for the same missing-`local` (e.g. `prune` / `destroy-all` / `list` paths at the `docker ps -a --filter name=` callsites) — any loop that runs inside `cmd_upsert`'s dynamic scope and reuses one of its local names (`container`, `port`, `slug`, `volume`, `db_name`) is the same foot-gun. Belt-and-suspenders: after `docker run`, assert the launched container's name equals `$(slug_to_container "$slug")` and fail loudly otherwise. Workaround until fixed: `destroy` + re-`upsert` the affected branch lands a correctly-named container (eviction won't fire on the immediate re-run if the count dropped), but it recurs whenever the box is busy enough to trigger eviction.
