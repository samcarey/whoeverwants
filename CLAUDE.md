...
- **Bottom-pin (continuously re-applying `scrollTo(0, max)` until first user interaction) is dangerous when the page's `scrollHeight` is unstable.** A prior version of `GroupContent.applyScrollAdjustmentRef` did exactly this: on every layout tick (`useLayoutEffect` after each render + `ResizeObserver` on every group card + the document) it called `scrollTo(0, scrollHeight - innerHeight)`. During the first ~50ms of a page load, the polls list resizes constantly (placeholders being replaced by real cards, async result/vote fetches resolving), so `max` keeps changing. The pin reapplies with the new max, scrollY oscillates by hundreds of pixels, and on iOS this transiently spikes `visualViewport.offsetTop` to the URL-bar-expanded value (~200px), which (via `position: fixed`) reports the fixed group header's `getBoundingClientRect().top` as **-200** for one frame. That's where the user-visible "top bar scrolled off the top of the screen just a little" came from — same root cause as the "polls visibly slide up/down to a stable position" report. Fix: retire the bottom-pin entirely (and the no-expand `scrollTo(0, scrollHeight)` in the initial-load `useLayoutEffect` that fed it). The card-anchor pin (when `initialExpandedQuestionId` is set) is fine because the target's `offsetTop` is anchored to its position in the list, not to `scrollHeight`. If you ever need to re-introduce a "land at the bottom of the page" UX on initial load, gate it on `scrollHeight` being stable (e.g. a 200ms idle period after the last `ResizeObserver` fire) — don't re-fire on every layout tick.
...
- **Capacitor APNS registration is event-based, not promise-returning.** `PushNotifications.register()` returns void; the device token arrives via `addListener('registration', ({value}) => ...)` async. Wrap in a `Promise` that resolves on `registration` and rejects on `registrationError`, with a 15s timeout — otherwise the toggle hangs forever if APNS is misconfigured. Bundle ID is read from `@capacitor/app: App.getInfo()` (matches the iOS topic for the APNS push); fall back to `com.whoeverwants.app` if the lookup fails.
...
- **Three plugins now use this pattern.** If a fifth lands, extract a shared `loadCapacitorPlugin<T>(name, exportKey)` helper; the current ~4-line repetition is below the abstraction threshold.
