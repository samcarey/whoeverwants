...
- **Long-press a thread on home → bulk-forget selection mode.** `components/ThreadList.tsx` arms a 500ms `setTimeout` from `onTouchStart` (manual setTimeout, NOT `useLongPress` — the existing handlers need touch events for scroll detection + synthetic-click suppression, and `useLongPress` is pointer-event-based). Firing the timer enters selection mode with the long-pressed thread pre-selected; haptic via `navigator.vibrate(50)`. While in selection mode every `ThreadListItem` renders a circular checkbox (left of `RespondentCircles`); taps toggle selection instead of navigating. Cancel (X) + red trashcan are rendered via `<HeaderPortal>` (target `#header-portal` in `app/layout.tsx`, outside `ResponsiveScaling`) — same target the settings-page back arrow uses. The X button visually replaces the home page's gear icon by sitting at the same coords with `z-50`. Trashcan opens a `<ConfirmationModal>`; on confirm `forgetThread(thread)` (in `lib/forgetQuestion.ts`) loops the thread's questions through `forgetQuestion` then fires `apiLeaveThread(threadId ?? rootPollId)` to drop server-side membership. Selection mode does NOT auto-exit when `selectedThreadIds` empties — only the cancel button or Escape exits (matches the user spec: "Unchecking all items should not cancel edit mode"). Pitfalls when extending: (a) `onThreadsForgotten` carries every poll-id in every forgotten thread, NOT just root ids — a thread is multiple polls sharing `thread_id`, so filtering the parent's `polls` by root id alone leaves follow-ups behind and `buildThreads` rebuilds a ghost thread; (b) the home page applies an optimistic `setPolls(prev => prev.filter(...))` instead of awaiting `getMyThreads()` — `forgetThread` already invalidates the per-question + accessible-polls caches via `invalidateQuestion`, so the next natural refresh re-syncs without an extra round-trip; (c) don't add a `portalReady` mount-flag pattern around `<HeaderPortal>` — the portal already encapsulates that via its own `mounted` state.
