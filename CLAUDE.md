...
- **Entitlement** `com.apple.developer.associated-domains` in `ios/App/App/App.entitlements` lists `applinks:whoeverwants.com`, `applinks:latest.whoeverwants.com`, `webcredentials:whoeverwants.com`, and `webcredentials:latest.whoeverwants.com`. Both `aps-environment` (push) and `associated-domains` share the same plist now; future capabilities go here too.
