...
- **Ranked-choice closed polls skip the above-card winner badge.** The in-card compact pill is the single source of truth for the winner, so `getResultBadge` is only rendered in the above-card strip for non-`ranked_choice` types. If you add a new poll type whose winner is also shown in-card, skip its `getResultBadge` render too.
