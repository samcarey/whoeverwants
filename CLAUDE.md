...
- **The respondents roster becomes "Viewed (N)".** N = everyone who opened the poll = named voters + anonymous voters + ignored (viewed-no-action) viewers. Vote state is a **secondary per-person marker** (voted / abstained / viewed). Ignored viewers are mostly nameless (they never submitted a `voter_name`), so they surface as a muted "N viewed" count rather than named chips. Backend adds `viewed_ignored_count` to the poll-level voter aggregate (alongside the existing `voter_names` + `anonymous_count`) via `_compute_poll_voter_data`; FE threads it `PollResponse` → `Poll` → `Group` → `VoterList` and the list header/label flips to "Viewed (N)".
