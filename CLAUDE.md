...
- **Expanded card uses `pb-0`, collapsed uses `pb-0.5`.** The thread card wrapper picks bottom padding off `isExpanded`. Collapsed cards use `pb-0.5` (2px) so the status/pill footer row sits snug against the card edge; expanded cards use `pb-0` since the trailing compact links (e.g., the gold "Your Ballot" edit link) already have their own spacing and extra `pb` left visible dead space. When adding new trailing content to an expanded card, this padding is already at zero — don't re-pad inside `PollPageClient` to try to fix trailing whitespace.
