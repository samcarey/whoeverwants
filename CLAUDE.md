...
- **Next step (when threads start hitting hundreds of polls): extract the card render into a `React.memo`'d `<ThreadCardItem>` component with stable props.** The current implementation (idle-time progressive fill from the anchor) loads everything eventually, so memory grows linearly with thread size. To bound memory at large scale, the card needs to be a memoized child component so progressive scroll-driven mounts only re-render the newly-mounted card — and the IO-driven scroll-time window (mount/unmount as the user scrolls past ±2 viewport heights) becomes affordable. The card render is ~600 lines of inline JSX with many handlers that close over parent state; the refactor needs `useCallback` for handlers + a ref-based subscription pattern for high-frequency state (expandedQuestionId, pressedQuestionId, swipeThresholdQuestionId) so card re-renders aren't triggered by parent state changes that don't actually affect that specific card.
- **Initial mount = anchor only; rest fills in idle-time around the anchor.** Mounting all groups upfront pays a heavy initial-render cost on long threads (each card in the .map is ~200ms in dev mode); progressively mounting cards on scroll-driven IntersectionObserver entries causes visible stutter (each setMountedGroupKeys triggers a parent re-render that walks every card in the .map, since cards aren't React.memo'd). The current compromise: initial render contains only the URL-anchored card. A `useEffect` then walks a distance-from-anchor queue and mounts groups in batches of 4 per `requestIdleCallback` tick (falls back to `setTimeout(16)`), so the surrounding cards "fill in around" the anchor visibly, and within ~1s on prod every group is mounted — at which point scroll is steady because no more setState fires. Trade-off: this still loads everything for very long threads (memory grows linearly with thread size). For truly bounded memory at ~hundreds-of-polls scale, the next step is to extract the card render into a `React.memo`'d `<ThreadCardItem>` component with stable props (useCallback for handlers, ref-based subscription for the high-frequency state) so progressive scroll-driven mounts only re-render the newly-mounted card. That refactor wasn't in scope when the progressive-fill landed; revisit when threads start hitting that size.
