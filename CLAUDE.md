...
- **`CFBundleVersion` uses `date +%s` (Unix epoch), NOT `git rev-list --count HEAD`.** The old `git rev-list` scheme was monotonic on a single branch but collided across branches: canary (every-push to a feature branch) and prod (push to main / release) advance independently, and ASC's App Store Connect rejected prod uploads with `bundle version must be higher than the previously uploaded version: '<canary_count>'` whenever a canary build had pushed ahead of main. **`.app` and `.latest` apparently share CFBundleVersion space at the team level** — same Bundle ID Prefix (`com.whoeverwants`), and ASC enforces uniqueness across both records, not per-record. Switching to Unix epoch sidesteps the whole class because no two workflow runs ever share the same second. Apple accepts up to 18 chars for CFBundleVersion; the current 10-digit epoch fits comfortably. `fetch-depth: 0` is retained for other reasons (`git log` accuracy in workflow steps), not for the version counter.
