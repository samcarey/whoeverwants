...
- **Icon vs title-line centering is empirical and tied to card padding.** The icon wrapper uses `h-7 items-center` with a `mt-[Npx]` offset where N is tuned to the current collapsed-card padding. As of the latest tightening (`pt-1.5 pb-[5px]`), the value is `mt-[3px]`. Pure line-box alignment looks low because the line-box reserves descender space below the baseline; pure cap-to-baseline alignment looks high because emoji glyphs are centered-ish in their em-box, not bottom-aligned. If the card padding changes or the emoji set / title size changes, re-tune the `mt-[*px]` value — reducing card bottom padding (making the card shorter) shifts the icon's position relative to the card's visual center, so the offset needs to come down in lockstep.
