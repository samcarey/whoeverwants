...
- **Home → group always lands collapsed.** `getGroupHref(group)` returns bare `/g/<root>` and `GroupList`'s tap handler passes `expandedQuestionId: null` to `slideToGroup`. The user taps a card to expand it. Earlier iterations synthesized `/g/<root>?p=<targetPoll>` (computed via a `pickTargetedPoll` helper) and pre-expanded the awaiting card during the slide, but the slide-overlay handoff could race with the real-route's `?p=` cache lookup — when the targeted poll wasn't cached yet, the real route mounted `GroupContent` with `initialExpandedQuestionId=null` and the card collapsed right after the slide settled. The `?p=` mechanism is still honored for direct URL access (share links built via `getGroupHrefForPoll`, `/p/` legacy redirects) — it just isn't synthesized from the home list anymore. `isPollOpen(poll, now?)` (not closed AND deadline-not-passed) remains the canonical helper for that rule — don't re-inline the `is_closed`/`response_deadline` math.
...
- **"Select Days" pill (`mr-2 text-[15px]`, `bg-gray-200 dark:bg-gray-700`, `border-transparent` neutral / `bg-amber-100 ... border-amber-400` amber)** is the canonical "card-header action pill" — sits in the Time Windows label row at the right side of the card-aligned width. `mr-2` insets the pill from the card's right edge (parent's `px-1` alone left it crowding the edge). The neutral bg uses gray-200 specifically so it reads as visibly off from the sheet's gray-100 page bg behind it (gray-100 on gray-100 looks blended). `border-transparent` on the neutral state keeps the always-applied `border` class load-bearing for amber↔neutral height stability (per "Time-question split across cards" above) while removing the visible border the user didn't want. Mirror this pattern for any new "card-header action pill": always-applied `border`, transparent-when-neutral, full-color-when-amber, mr-2 inset from card edge.
...
- **DO NOT use `overlay.scrollTop` to align cards with the header.** The overlay's `contain: strict` (+ its `transform: translate3d`) makes `position: fixed` descendants behave as absolute-positioned inside the overlay — they scroll WITH the overlay's scrollTop. Setting `scrollTop=8` to align the first card flush with the GroupHeader yanks the header itself 8px above the viewport top; the user sees "the top bar shifts down" when the overlay unmounts and the header drops back to its real `top: 0`. The fix is the `--group-card-gap` CSS variable (next bullet) — collapse the gap inside the overlay so no scroll is needed. The "no-expand" case (slide to bubble bar at bottom) still uses scrollTop because the user doesn't look at the header in that case.
- **`--group-card-gap` CSS variable** (set in `GroupContent`'s `.pb-2` `paddingTop` calc, read at value `0.5rem` default) is the gap between the fixed header and the first card. The slide overlay sets `--group-card-gap: 0px` on its wrapper when there's an `expandedQuestionId`, so the first card sits at `offsetTop = headerHeight` (flush with header) without any scroll. The real route keeps the 0.5rem default and reaches the same visual position via `window.scrollY = 8` (its own initial-scroll effect). Both layouts converge on the same on-screen pixel positions — the unmount handoff is seamless. If you ever change `.pb-2`'s paddingTop calc, audit `slideOverlay.tsx`'s `COLLAPSE_CARD_GAP_STYLE` along with it.
...
- **Header chrome must match destination for clean handoff.** The FAB's overlay started with a bare `<GroupHeader title="New Group">` while the real `/g/<short_id>` route mounts `GroupHeader` with the participant avatar + Share button + tappable title — when the overlay unmounted, the header visibly populated. `EmptyPlaceholder({inOverlay})` now mirrors the destination's chrome (`participantNames: []`, `anonymousCount: 0`, no-op `onTitleClick`, `<GroupShareButton routeId="" title="New Group" />`). Two supporting tweaks: `GroupShareButton` short-circuits on empty `routeId` so the placeholder button is functionally inert, and `usePageReady` is skipped when `inOverlay` (the underlying route is still the source page, so writing `data-page-ready=/g` would lie to other in-flight view-transitions). Rule: any new "caller owns navigation" kind needs its overlay placeholder to render the SAME header chrome the destination will, not just the title — even if the buttons need empty-routeId / no-op handlers during the transient overlay window.
