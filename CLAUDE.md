...
**⚠️ STATUS (owner review, June 2026): direction LOCKED. Gaps 1 + 4 IMPLEMENTED on branch `claude/todo-selection-handling-EfPMu` — pending owner review of the interaction details (open questions B + C) before merge.** A prototype of the original gaps (segmented Open/Decided filter + 3 layout variants + decisions-ledger drawer + "the plan" summary + outcome-notification body + home rollup) was built on branch `claude/group-hub-ui-variants-FzkNe`, deployed, and compared by the owner, who picked the **segmented-filter** layout and **redefined the semantics** into the follow/ignore model below. That prototype was reverted; the spec was merged. The from-scratch implementation now lives on `claude/todo-selection-handling-EfPMu` (NOT yet merged — handed to the owner as a dev link for B/C sign-off per the process note).

**Gap 1 + 4 — IMPLEMENTED (claude/todo-selection-handling-EfPMu). What shipped, resolving the open questions with the spec's proposed defaults:**
  ...
  - **Open questions B (✕/+ affordance) + C (no auto-aging) are IMPLEMENTED per the proposals but flagged design-sensitive** — the dev-server link is the owner-review gate before merge. Tests: `server/tests/test_follow_state.py` (7), `tests/__tests__/follow-state.test.ts` (8).
...
**Process note (still applies for the gap-1 build):** the To Do/New/Old layout is the chosen direction, but the ✕/+ affordance (open question B) and the exact row chrome are still design-sensitive — when implementing, deploy to a dev server and hand the owner a link before finalizing the interaction details rather than shipping unilaterally.
...
Real-device feedback after shipping surfaced four refinements. All four (1–4) are now **done**.
