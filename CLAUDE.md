...
- **TODO / follow-ups (deferred):**
  ...
  - **Invites accumulate** if never redeemed: each plus-one submission mints a fresh single-use invite (no dedup against an existing active invite for the same (group, poll, account)). Harmless (single-use, the poll closes) but a dedup/cleanup is a possible tidy-up.
