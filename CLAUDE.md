...
- **Voting UI: `components/PlusOnesInput.tsx`** — rendered on the poll detail page (`PollDetailPage`) when `poll.allow_plus_ones && !isClosed`. Reuses the options-field-card row visual (input + red × per row) but with an explicit **"+ Add a person"** button instead of type-to-grow (so a blank row is a valid unnamed plus-one — the name is optional), and a **contacts-backed autocomplete dropdown** (candidate names from `apiGetGroupInvitableAccounts(routeId)`, filtered client-side — the same "look up a person who might be a user" mechanism as the invite-members screen, NOT the location/restaurant search `OptionsInput` uses). State `plusOneNames: string[]` is poll-level; `useGroupVoting` reads it via a `getPlusOneNames` getter (ref-stable) and threads it into every `apiSubmitPollVotes` call (both the multi-question batch AND the single-yes/no auto-submit paths). On mount the list is **pre-filled from the viewer's existing vote** (`apiGetVotes` → `mine.plus_one_names`) so editing a ballot doesn't silently drop previously-added people.
- Tests: `server/tests/test_plus_ones.py` (default ON/OFF by type + override, yes/no + unnamed weighting, disallowed-poll clamp, named/duplicate/unnamed roster, edit updates count).
