...
- **Thread URL targets the oldest open+unresponded poll, falling back to newest.** `getThreadRouteId(thread)` returns `thread.targetedPoll.short_id` (or `thread.targetedPoll.questions[0]?.id`). `targetedPoll` is computed by `pickTargetedPoll` in `lib/threadUtils.ts`: among polls where the viewer has at least one un-responded question (the per-question gold-outline rule applied at poll level), pick the oldest by `created_at`; otherwise pick the newest poll in the thread. The home `ThreadList` uses `getThreadRouteId` for both prefetch and click navigation. Two helpers encapsulate the rules: `isPollOpen(poll, now?)` (not closed AND deadline-not-passed) and `pollHasAwaitingQuestion(poll, voted, abstained, now?)` (open AND any sub-question un-responded). Use them whenever you need either rule — don't re-inline the `is_closed`/`response_deadline`/`some(...)` math.
- **The home `ThreadList` appends `?thread=1` to its links** (the constant is `THREAD_QUERY_PARAM` in `lib/threadUtils.ts`). The flag is a hint that the URL was picked by the thread-level rule, not a direct share — but `PollPageInner` no longer gates `suppressExpand` on it. The original gate broke on refresh: tap-expand, `/info`/`/edit-title` back-nav, create-poll redirects, and direct deep-links all land on `/p/<id>` without `?thread=1`, and the user would see an expanded card for a poll they'd already responded to. The current rule: **suppress auto-expand iff the user has responded (voted or abstained) to every question in the linked poll.** Closed polls the user never voted on still expand (so shared closed polls show their results); open polls with any unresponded question still expand (so they can act). When all questions are responded, `initialExpandedQuestionId={null}` passes through to `ThreadContent` and the initial scroll lands at the bottom of the document so the user sees the draft poll form. The `?thread=1` flag is still emitted by `ThreadList` for forward-compat / debugging but isn't currently consumed.
- **Single poll route + thread sub-routes.** The legacy `/thread/[threadId]/` route is gone — every thread view goes through `/p/[shortId]/`. The page resolves the URL param (poll short_id, poll uuid, or question uuid) to the thread root + the targeted poll, then renders `ThreadContent` with that poll's first question expanded. `/p/` with no param is the empty placeholder (was `/thread/new/`); the home page's "+" FAB and the What/When/Where bubble bar both land here. Sub-routes `/p/<id>/info/` and `/p/<id>/edit-title/` carry the same `[shortId]` segment but read it as a thread root id — they're navigated to from the `ThreadContent` title click with the resolved root, never with an arbitrary poll id. Cards expand in place on tap (see "Expandable Question Cards" below) — there is no separate standalone question page. The long-press modal exposes Copy + Forget, plus Reopen when the question is closed and the browser knows the creator secret (or is running in dev mode).
- **Floating bubble bar auto-follows-up** when on a poll page via `document.body.getAttribute('data-thread-latest-question-id')` — the thread page sets this attribute on mount. The home page does NOT render the bubble bar; it has the single "+" FAB instead, which navigates to `/p/` (the empty placeholder). `/p/` shows the bubble bar (since `isThreadRootView` matches it); the user picks a category from there. If the user dismisses the modal without submitting, the empty placeholder remains visible with the bubble bar; tapping back returns to home. On submit from the empty placeholder, the new question has no `follow_up_to` so it becomes its own thread root.
...
- **Thread sub-routes:** `/p/<id>/info` (participant list + total count, with Back/Edit buttons) and `/p/<id>/edit-title` (input to set/clear the `thread_title` override). These render their own fixed headers and read `params.shortId` (kept as a local `threadId` var since the resolved route id IS a thread root id — they're never navigated to with an arbitrary poll id). `isThreadRootView(pathname)` in `lib/questionId.ts` distinguishes the root view (`/p` or `/p/<id>`, gets the thread-like FAB + bottom padding) from sub-routes (plain layout, no FAB) via the regex `^\/p(\/[^/]+)?\/?$`. Update that helper when adding more poll sub-routes. The function name is stale (it's about polls, not threads) — kept for diff minimization since the consumer call sites are stable.
- **Empty placeholder route:** `/p/` (no shortId) is the empty-thread route surfaced by tapping the home page's "+" FAB. Implemented as `app/p/page.tsx` which serves two roles: with `?id=<question-uuid>` it redirects to the friendly `/p/<short_id>` URL (legacy deep-link compatibility); with no params it renders `EmptyPlaceholder` (shared `ThreadHeader` with `title="New Thread"`, the instructional message, and a `<div id="draft-poll-portal" />` for `CreateQuestionContent` to portal the in-progress draft into). No fetch, no thread state. The placeholder matches `isThreadRootView`, so the What/When/Where bubble bar is rendered by the template — the user picks a category from there to open the create modal. The thread "materializes" only when the user actually creates a question. The previous "`/thread/new/` is detected as a string check inside `ThreadPageInner`" pattern is gone — the bare `/p/` route is its own page.tsx file. Don't reintroduce a `shortId === 'new'` magic-string branch inside `PollPageInner`.
- **`PollPageInner` is the resolution wrapper** at the bottom of `app/p/[shortId]/page.tsx`. Synchronously resolves the URL param to a `{question, rootRouteId}` from `questionCache` via `useMemo([shortId])`; falls back to async `apiGetPoll{ById,ByShortId}` → `apiGetQuestionById` → `discoverRelatedQuestions` → `getAccessiblePolls` on cache miss. The memo dep `[shortId]` is critical: an IIFE without memoization allocates a new object every render, which (when the result is in the useEffect dep array) re-fires the async fetch effect on every parent re-render. Always use `useMemo` (not a bare IIFE) for sync-cache lookups whose result feeds an effect dep.
- **Shared `ThreadHeader` component** lives at `components/ThreadHeader.tsx`. Props: `headerRef`, `title`, optional `participantNames` + `anonymousCount` (renders `RespondentCircles` when provided), optional `subtitle`, optional `onTitleClick` (makes the title block a button when provided), optional `onBack` (defaults to navigating to `/`), optional `rightSlot` (renders an action node on the right; when provided, the title becomes centered and the right padding tightens from `pr-4` to `pr-2`). Used by `ThreadContent` (real-thread props), `EmptyPlaceholder` in `app/p/page.tsx` (just `title`), the `/info` sub-route (`onBack` + `rightSlot=<Edit button>`), and the `/edit-title` sub-route (`onBack` + `rightSlot=<Save button>`). Don't re-implement the fixed `top:0 + padding-top:env(safe-area-inset-top) + headerRef + back button` markup in another route — extend `ThreadHeader` or import it.
- **Thread page back button always navigates to `/`**, regardless of in-app history. Earlier the button used `hasAppHistory() ? navigateBackWithTransition() : navigateWithTransition('/')`, but after creating a question on `/p/`, the prior history entry was the now-empty placeholder — back popped the user back to it instead of home. Hard-coding `'/'` is the cleanest fix and matches the user's mental model ("back from a thread → main list"). The `/info` and `/edit-title` sub-routes still use the conditional-back pattern because their natural back target is the thread, not home.
- **`useEffect(..., [])` + conditional early return = ref never attaches.** When a page renders a loading placeholder on first paint and then swaps to the real content after an async load, an effect with empty deps runs once on the first render — when the real refs don't exist yet — and never re-fires, so observers like `ResizeObserver` silently fail to attach. Fix: gate the real content behind an inner component that only mounts when data is ready (`if (loading) return <Loading/>; return <Inner {...}/>`). Effects inside `Inner` then run against refs that definitely exist. Used in `app/p/[shortId]/info/page.tsx` and `.../edit-title/page.tsx`.
...
- **One component, one route.** `ThreadContent` (exported from `app/p/[shortId]/page.tsx`) renders the thread list. `PollPageInner` (in the same file) resolves any `/p/<id>/` URL — short_id, poll uuid, or question uuid — to a `{question, rootRouteId}` and mounts `ThreadContent` with `initialExpandedQuestionId={question.id}`. The thread root is computed via `findThreadRootRouteId` walking the accessible-polls list. There is no longer a standalone question page — `QuestionBallot` is only rendered inline inside an expanded card.
- **Template treatment of `/p/*`.** The template computes `isThreadLikePage = isThreadRootView(pathname)` (matches `/p`, `/p/`, `/p/<id>`, `/p/<id>/` but NOT sub-routes) and uses it for layout (flex-col overflow-hidden, no duplicate header/back button). A separate `isPollPage` (`pathname === '/p' || pathname === '/p/' || pathname.startsWith('/p/')`) covers the broader "any page under `/p/`" check used by the fallback header gate. The copy-link button is rendered per-card in the upper-right of each card's compact header (visible whether collapsed or expanded), not at the page-corner level, so users can copy a question's share link directly from the thread view without navigating away. The title uses `flex-1 min-w-0` alongside the `shrink-0` button wrapper so it wraps (line-clamp-2) before colliding with the button. Tap-to-expand is gated off the button via `stopPropagation` on the button wrapper's click/touch events.
...
- **URLs reference polls, never questions.** Polls have `id` (uuid) and `short_id` — the friendly URL is `/p/<short_id>/`. Sub-questions have a `questions.id` uuid for foreign-key purposes inside the DB, but it is not URL-able. Never construct `/p/<question-uuid>/` — use the parent poll's `short_id`. (The legacy fallback `/p/<question-uuid>/` happens to resolve via `apiGetQuestionById` today because the loader cascades, but treat that as a transitional artifact, not an API.) The previous `/thread/<id>/` route is gone — every thread view goes through `/p/[shortId]/` now.
...
- **Poll**: top-level entity. Owns: optional context, voting cutoff, optional shared suggestion/availability cutoff, `follow_up_to`, `is_closed`, `close_reason`, `creator_secret`, `short_id`. Target of `/p/<shortId>/` (the only URL form — `/thread/<id>/` is gone).
...
- **Manually-triggered upsert may not stop the previous Next.js dev server.** When a previous dev server crashes or is killed without going through `dev-server-manager.sh stop`, its PID in `.dev-meta.json` goes stale; the next upsert assigns a fresh port (e.g. 3001 → 3004), starts a new Next.js, and Turbopack's per-directory lock detects the orphaned old process in the same dir and refuses to start: `⨯ Another next dev server is already running. PID: <old>`. The old process keeps serving on the OLD port; the new server doesn't bind. Fix: `kill <old PID>` (the log shows it explicitly) + `fuser -k <old port>/tcp` and re-run upsert. The lock check is per-directory, not per-port, so even with different ports they conflict.
...
- **`CompactNameField` returns `null` when the user already has a name set + the field isn't being edited.** Both wrapper-level Submit blocks in `app/p/[shortId]/page.tsx` (poll multi-Submit + 1-question wrapper Submit), every legacy per-question Submit fallback (`QuestionBallot`, `RankingSection`, `SuggestionVotingInterface`, `TimeBallotSection`), and the create-poll voter-name block consume the same component and want the same hide-when-set behavior — the gate lives inside the component, not at the call site.
