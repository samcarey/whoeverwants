...
- **All group cards mount upfront, not progressively.** The first version of this added a window IntersectionObserver that mounted cards as they entered ±2 viewport heights of the visible region. That sounds memory-friendly but each setMountedGroupKeys call re-rendered every card in the .map (the parent isn't memoized + the cards aren't React.memo'd), and on dev that's ~200ms per render. Scrolling through a 25-card thread fired ~10 mount events → 10 frame-blocking re-renders → visible stutter ("scroll glitchy"). Mount-all is a single initial-render cost; after that, no scroll-time mounts. If you ever need to support truly long threads (hundreds of polls), revisit virtualization but ALSO memoize each card so per-mount re-renders only touch the new card.
- **Placeholder divs are kept around for the brief window between a new poll arriving (POLL_HYDRATED_EVENT, follow-up creation) and the maintenance useEffect adding it to mountedGroupKeys.** Each placeholder renders as a `<div>` with `style={{ height: groupHeightById.get(key) ?? ESTIMATED_GROUP_HEIGHT }}` so the doc height is stable across the swap. A shared `ResizeObserver` populates `groupHeightById` from each rendered group's `borderBoxSize` (NOT `offsetHeight` — that forces a layout per entry, which on iOS URL-bar transitions stutters the scroll because every observed card fires at once).
