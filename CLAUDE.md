...
- **Template treatment of `/p/*`.** The template computes `isThreadLikePage = isPollPage || isThreadPage` and uses it for layout (flex-col overflow-hidden, no duplicate header/back button). The copy-link button is rendered per-card (absolute-positioned in the upper-left of each expanded poll card's content), not at the page-corner level, so users can copy a poll's share link directly from the thread view without navigating to `/p/<id>/`.
...
- **Single pending-action confirmation modal.** Forget and Reopen share one `ConfirmationModal` driven by `pendingAction: { kind: 'forget' | 'reopen'; poll: Poll } | null`, which varies title/message/button class by kind. Avoids two near-identical confirmation blocks.
...
- **Fixed header bars need to cover the notch zone, not just sit below it.** A header anchored at `top: env(safe-area-inset-top)` leaves the area above it (the notch zone) uncovered; in PWA standalone with a scrollable list underneath, stray `scrollTop` on an ancestor can let items appear above the bar. Instead, anchor the bar at `top: 0` and push its content down with `padding-top: env(safe-area-inset-top, 0px)` so the background fills from the physical screen top. To keep the bar's bottom at the same vertical position as before, the measurement ref (for computing a sibling's offset/padding) must be on the inner content div so `offsetHeight` stays content-only — not on the outer div whose padding already accounts for the safe area via `.pwa-safe-top`. Pattern used in `app/thread/[threadId]/page.tsx`.
...
- **Reserve space for the bottom bar by its measured height, not a hardcoded Tailwind class.** The old `mb-14` (56px) on the scroll container didn't match the bar's actual rendered height in the browser (~50px — `py-1.5` content + 1px border, no safe-area inset on web). That left a ~6px white gap between the scroll area's bottom and the bar's top edge. Fix: a `ResizeObserver` on the bar element stores its height; the scroll container's `marginBottom` follows it, with a `transition: margin-bottom 200ms ease-out` matching the bar's slide transform so the bar's top edge stays flush with the list as it hides. In PWA standalone mode the `.safari-scroll-container { margin-bottom: 0 !important }` rule still wins — that's intentional, the bar extends into the home indicator and overlays content there.
- **Thread pages must force the bottom bar visible on pathname change**, not just suppress hides during scroll. The scroll handler's `if (isThreadPageRef.current) visible = true` guard only runs when a scroll/touch event fires. If the bar was hidden on a previous page and the user navigates to a thread without scrolling, it stays hidden. The pathname-tracking effect should call `setShowBottomBar(true)` whenever the new path starts with `/thread/`.
...
- **`overflow: hidden` does not reliably stop scroll drift on iOS PWA — don't trust it, don't try to "snap back to 0" either.** iOS WebKit in standalone mode will still drift `scrollTop` on overflow-hidden ancestors, AND the `scroll` events don't always fire (or don't fire in time to matter), so an ancestor-walking listener that resets `scrollTop = 0` just doesn't catch the drift. The correct fix for elements that must stay visually pinned to the safe-area boundary is `position: fixed; top: env(safe-area-inset-top)` — it escapes normal flow entirely so no amount of ancestor scrolling can shift it. The thread page header (`app/thread/[threadId]/page.tsx`) uses this pattern; the scroll list below it gets a measured `paddingTop` via `ResizeObserver` to reserve header space. Viewport-relative `position: fixed` works here only because `.responsive-scaling-container` has no `transform` on mobile (any `transform` — even `scale(1)` — creates a containing block that traps fixed children).
