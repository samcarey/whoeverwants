...
- **FE must forward `?p=` from the URL into `apiGetThreadByRouteId`** for the auto-grant above to fire. `lib/api/threads.ts: apiGetThreadByRouteId(routeId, { pollShortId })` builds `/by-route-id/<routeId>?p=<pollShortId>` when the option is set; both `ThreadPageInner`'s async fetch and `ThreadContent`'s refresh fetch pass the URL's `?p=` value through. Forgetting this causes a chicken-and-egg deadlock for cold-load direct links: the first by-route-id call returns 404 (no membership, no access), `rootPoll` stays null, the `apiGrantPollAccess(targetPoll.id)` `useEffect` (which depends on `targetPoll`, which depends on `rootPoll`) never fires, and the page is permanently stuck on "Thread Not Found / This thread may not exist or you don't have access." Symptom: the failure repros only on a fresh browser (Playwright with no localStorage); existing browsers with prior membership or poll_access don't see it. A regression here is invisible to the developer because their own browser already has membership from prior testing — verify cold-load direct-link flows with a fresh `incognito`/Playwright session.
...
- **Pitfall: visibility-gated reads return 404 to crawlers.** The original `GET /api/threads/by-route-id/{id}` enforces `poll_access` / `thread_members` visibility against the request's `browser_id`. Server-component metadata fetches from Vercel run with no browser identity → no membership → 404. That's why the public `/preview` endpoint is a separate route, not just a query param. Don't try to hack visibility-bypass into the visibility-aware endpoint; keep "public preview" and "authenticated read" as separate URLs with separate threat models.
