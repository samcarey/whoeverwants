...
- **Turnout context on the group card ("M of V seen") — SHIPPED.** The viewer-vs-voter distinction is a social signal — a 1-vote poll that 6 people OPENED is "low turnout," not a decisive result. `PollResponse.viewed_total` (FE `Poll.viewed_total`) is a counts-only distinct-viewer total over `poll_views`, computed in `_compute_poll_voter_data` (now a **4-tuple** `(voter_names, anonymous_count, viewed_ignored_count, viewed_total)`) as `COUNT(DISTINCT COALESCE(ub.user_id, pv.browser_id))` via `LEFT JOIN user_browsers` — so a signed-in viewer on two devices counts once (account-aware, mirroring `load_user_visibility`). NO 5-min / no-action filter (unlike `viewed_ignored_count`); it's the raw "how many saw it" denominator. Threaded `_row_to_poll` (new `viewed_total` kwarg) → all 8 callsites → `toPoll` → `isPollContentEqual` (so the card re-renders when it changes on the 5s refresh). The group card (`GroupCardItem`) renders a muted `"{V} seen · {M} voted"` prefix to the respondent bubbles where `M = voter_names.length + anonymous_count`, gated on `seenCount >= 2 && seenCount > respondedCount` — i.e. ≥2 viewers AND a real gap, which filters the trivial "just the viewer glanced at a fresh poll" case (seen=1) and hides the line at full turnout (no gap). Counts only — viewer identities never cross the API (consistent with the ballot-privacy TODO). The poll-detail "Viewed (N)" roster was deliberately left unchanged (it already conveys turnout via voters + the "N viewed but haven't responded" ignored sub-line; `viewed_total` is the card-side denominator only). Tests: `server/tests/test_notification_events.py::test_viewed_total_counts_distinct_viewers` + `::test_viewed_total_collapses_account_browsers`. **Demo caveat:** the line renders identically for open + closed polls, but a CLOSED poll is hidden from a fresh viewer by the closed-before-join filter, so a live demo of the turnout line uses an OPEN poll. **Possible follow-ups (not built):** a raw-views total distinct from `viewed_total` if a tighter "saw but never engaged" metric is wanted; an optional "nudge the lurkers" affordance.
