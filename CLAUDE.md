...
- **Group list row layout** (`components/GroupListItem.tsx`): NO hairline dividers between rows; the visual structure is avatar-driven, not chrome-driven. Each row is a flex with `[selection checkbox?] [countdown column] [avatar] [text block]`. The countdown column sits in its own fixed-width slot (`w-7`) left of the avatar — always-reserved when an avatar renders, so indentation stays consistent whether or not an active deadline exists. Renders a single-unit `<SimpleCountdown compact>` in `text-green-600 dark:text-green-400` when `soonestUnvotedDeadline` is set; the slot stays empty otherwise. `compact` mode formats via `formatCompactCountdown(diffMs)` in `lib/timeUtils.ts` — promotes to the next larger unit only when its count is ≥ 2 (0s–119s as seconds, 2m–119m as minutes, 2h–47h as hours, 2d–13d as days, then `w` / `mo` / `y`). The pill is `self-center` vertically with the avatar. Gap tuning is done with negative right-margin on the countdown column (`-mr-[10.5px]` → 1.5px effective countdown↔avatar gap) since flex `gap-3` (12px) is otherwise enforced uniformly; the avatar↔text gap uses `-ml-[3px]` on the text block (→ 9px effective). The outer `mr-1.5` (right-only) and the inner row's `pl-px` keep the left edge tight against the viewport's safe-area padding while preserving asymmetric right-side breathing room. Both are gated on `!draftMode` so the draft poll card on group pages keeps the symmetric `mx-1.5` + `pl-2` chrome it relies on for its dashed border. (Earlier this column held a red `unvotedCount` pill AND a separate full-format countdown floated to the lower-right of the latest-poll body via a phantom `float-right` span on line 1; both were collapsed into this single compact-countdown slot. The `unvotedCount` prop on `GroupListItem` is gone — `hasUnvoted` still drives title emphasis.)
