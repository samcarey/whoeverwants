...
The `poll_type='participation'` type, its FE components (`ParticipationConditions`, `MinMaxCounter`, `SubPollField`, `ParticipationConditionsCard`), the `algorithms/participation.py` priority algorithm, the `auto_close.py` capacity-watcher, and every supporting column/constraint were dropped in **migration 094** (schema) plus the matching code removal. There is no longer a "participation" poll type or codepath.
