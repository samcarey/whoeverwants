...
- **Thread card chrome lives outside the card.** Each thread item is a 2-col × 3-row CSS grid: col 1 row 2 holds the category icon, col 2 row 1 holds the above-card status element (`SimpleCountdown` / "Taking Suggestions" / "Collecting Availability" — empty for closed polls since the winner pill in the card below is the single source of truth), col 2 row 2 holds the bordered card itself (title + compact preview), col 2 row 3 holds the below-card row (creator/age label on the left, respondents bubble row on the right). The `row-start-2` placement of the icon cell pins its top to the card's top without a magic padding. `PollPageClient` no longer renders its own countdown inside the ballot; the above-card strip is the single source of status info. Time polls' deferred-deadline notice ("Availability cutoff Xmin after first response") still renders in the ballot because it conveys run-duration info the above-card "Collecting Availability" label doesn't — the parallel "Suggestions cutoff …" notice was removed.
