...
- **Multi-question section header uses `getQuestionSectionTitle(question)` from `lib/questionListUtils.ts`.** Format: `"<Label> for <Context>"` mirroring the server's auto-title (e.g. a Time question with details="Partie" reads "Time for Partie" instead of just "Partie"). Without the type signal, a Time question's section header was indistinguishable from a Restaurant question's. The helper special-cases `time` (the Time bubble stores question_type=time but leaves category=custom, same load-bearing convention as `_category_for_title` server-side) and `yes_no` (server uses "Yes/No", BUILT_IN_TYPES has "Yes / No" — special-case keeps the FE in lockstep with server-generated wrapper titles).
...
- **`bottom: max(1rem, env(safe-area-inset-bottom, 0px))` makes a corner FAB sit visibly higher in iOS PWA than in the browser.** The env() resolves to ~34px in iOS PWA standalone mode (home indicator zone), so the FAB jumps from `bottom: 16px` (browser) to `bottom: 34px` (PWA) — a noticeable upward shift. Even using a flat positive value like `bottom: 0.25rem`, the home indicator zone underneath the FAB shows the page's white background and reads as a thick white bar beneath the button — natively-styled iOS apps don't have this. For a corner-anchored FAB (e.g. the home page's "+" button at bottom-right), use `bottom: 0` so the FAB sits flush against the screen bottom edge — the home indicator pill is centered (not in the right corner), so the FAB can render into the home indicator zone without colliding with it. The `max(...)` pattern is still right for elements that span the full bottom width (e.g. the floating bubble bar) where overlap with the home indicator would obscure interactive content.
