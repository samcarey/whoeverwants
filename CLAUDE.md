...
The earlier docked-panel layout (poll-level fields in a bottom sheet, question form in a top sheet) was retired. The current design puts all poll-level state INSIDE the draft poll card itself, with the question form as the only slide-up sheet.

- **Draft poll card hosts everything poll-level.** Title (inline-editable via `InlineTitleField`), up-arrow Submit (top-right), the questions list with per-row pencil-edit, and the Settings rows (voting cutoff, suggestion/availability cutoff when applicable, notes, voter name) all live in the same card. Portal target `#draft-poll-portal` is rendered only on thread-like pages (the home page lost its portal — drafts can't exist there).
- **Question form modal is a true blocking modal** that slides up from the bottom with a backdrop. Tapping the backdrop or X strips `?create=1` from the URL and the bubble bar reappears. The drag-to-dismiss / `--bottom-modal-height` / `CREATE_PANEL_FINALIZE_EVENT` / `OPEN_QUESTION_FORM_EVENT` / `QUESTION_FORM_STATE_CHANGE_EVENT` plumbing is gone — modal lifecycle is URL-driven via `?create=1`.
- **Bubble bar (What/When/Where) is always pinned at the bottom of thread-like pages**, hidden when `?create=1` is present (the modal's backdrop covers it anyway, but skipping the render avoids tap leakage during animations).
- **Inline title field** lives in `components/InlineTitleField.tsx`. Greyed-out italic auto-generated title by default; tap to edit; user-typed text in blue M PLUS 1 Code monospace (matches the visual idiom from `CategoryForLine`). Clearing + blurring an empty input reverts to auto.
- **Poll-level Context field was deleted** when the inline title became editable (the auto-title's "for X" suffix collapsed into the user-controllable title). Per-question `forField` ("for ‹context›" inside `CategoryForLine`) remains in the question form.
...
- **POLL_HYDRATED has a fallback path for when POLL_PENDING bailed.** If `placeholder.follow_up_to` resolves to null at synthesis time (cache miss on the parent poll), the POLL_PENDING handler short-circuits and the placeholder isn't added to the thread. Without a fallback, POLL_HYDRATED's `prev.polls.some(p => p.id === placeholderId)` is false and the real poll never lands either — the user sees the form clear, the API call succeeds, but no new card appears. The fallback re-runs the same `isFollowUp || isOwnRoot` check against the **real** poll's `follow_up_to` (which the server returned authoritatively) and rebuilds the thread from `getCachedAccessiblePolls()` if the real poll belongs.
...
- **Auto-open useEffect uses an `openTriggerConsumedRef`** to guard against React re-running the effect with a still-stale `useSearchParams` after `router.replace` strips trigger params. Once the effect opens the modal it sets the ref true; subsequent renders bail until the URL is fully scrubbed of every open-trigger param (`openForm`, `duplicate`, `voteFromSuggestion`, `followUpTo`), at which point the ref clears and a fresh signal can re-trigger. `router.replace` updates Next.js's internal router state and `useSearchParams` synchronously enough that you'd think this isn't needed — but with multiple synchronous setStates in the same render cycle the effect runs once with stale params, and that's enough to re-open the modal mid-close.
- **`CREATE_FLOW_URL_PARAMS` (in `createPollHelpers.ts`)** is the canonical list of query-string params that gate or carry create-flow state. `stripCreateParams` iterates it on close so the bubble bar reappears cleanly. If you add another create-flow param (e.g. a new entry-point trigger), add it to this constant — don't duplicate the strip list inline.
