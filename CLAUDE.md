...
**Gap 3 (TODO — wanted, not yet built) — outcome notifications.** Make the poll-closed push deliver the *outcome*, not just "a poll closed": e.g. body "Decided: Thai · Sat 7pm". The prototype implemented this server-side (a `_poll_decision_summary` that computed each question's winner and set the close-notification body) and it was reverted along with everything else — re-implement it as part of this initiative. Must compose with gap 1's Old-set suppression (don't send to viewers who ✕'d the poll). Delivery can't be demoed (needs a real push subscription); the recipient-selection + payload are what's testable.
...
### Plus-Ones (vote on behalf of others) — migration 130
...
- **`polls.allow_plus_ones BOOLEAN` (migration 130)** — poll-level. Default resolved at create in `_insert_poll`: ON when the poll has any `time` question, OFF otherwise; `CreatePollRequest.allow_plus_ones` (bool|None) is the FE override (None → server default). Surfaced on `PollResponse.allow_plus_ones` (and `Poll.allow_plus_ones`, via `toPoll`). The create-poll bottom settings card has a "Allow voting for others (plus-ones)" `SliderSwitch`; FE state `allowPlusOnes: boolean | null` (null = follow `effectiveAllowPlusOnes = allowPlusOnes ?? pollHasTimeQuestion`), persisted in `questionFormState`, reset to null in `discardAndClose`.
- **`votes.plus_one_names JSONB` (migration 130)** — array with one entry per represented person ("" = unnamed plus-one). The vote weighs `1 + len(plus_one_names)`. **Poll-level on the wire** (`SubmitPollVotesRequest.plus_one_names`): the batch endpoint writes the SAME value onto every sibling question's row. `_sanitize_plus_one_names` (routers/polls.py) trims + bounds (≤50 people, ≤50 chars each, blanks kept) and is CLAMPED to None when `allow_plus_ones` is off — a crafted request can't inflate a poll that opted out. Edit path direct-assigns `plus_one_names` (not COALESCE), same as `voter_day_time_windows`, so the FE must always re-send the current list (it does).
