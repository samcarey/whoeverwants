...
- **`bottom: max(1rem, env(safe-area-inset-bottom, 0px))` makes a corner FAB sit visibly higher in iOS PWA than in the browser.** The env() resolves to ~34px in iOS PWA standalone mode (home indicator zone), so the FAB jumps from `bottom: 16px` (browser) to `bottom: 34px` (PWA) — a noticeable upward shift. For a corner-anchored FAB (e.g. the home page's "+" button at bottom-right), use a flat `bottom: 0.25rem` instead — the home indicator pill is centered at the bottom and doesn't reach the right corner, so the FAB can sit close to the screen edge without colliding with it. The `max(...)` pattern is still right for elements that span the full bottom width (e.g. the floating bubble bar) where overlap with the home indicator would obscure interactive content.
