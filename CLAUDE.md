...
- **Every poll type has a compact "top result" strip in the lower-right of the thread card header, rendered outside the grid-rows expand clip and mounted regardless of `isExpanded`** — the pill stays in a stable DOM position across toggle so there's no remount flicker. Yes/no uses `<YesNoResults hideLoser={!isExpanded}>`; the `hideLoser` prop internally animates the loser card's reveal when the user expands. Ranked choice / suggestion / time use smaller type-specific components (`CompactRankedChoicePreview`, `CompactSuggestionPreview`, `CompactTimePreview`) in `components/PollResults.tsx`, which stay visible even when the card is expanded — the pill then acts as a persistent header above the full breakdown (rounds visualizer / time-slot grid) rendered inside the clip by `PollPageClient`. Prior iteration gated them on `!isExpanded`, which caused a one-frame gap during the 300ms expand animation: the pill unmounted immediately on tap while the heavy content was still sliding in. To add a preview for a new poll type: (a) add the component, (b) add a `poll.poll_type === '...'` block (no `!isExpanded` gate) in `app/thread/[threadId]/page.tsx` right after the yes/no external block, (c) extend the `wantsResults` allowlist in `maybeFetch` so results get fetched when the card enters the viewport.
