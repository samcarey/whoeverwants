...
- **Empty placeholder route:** `/t/` (no threadShortId) is the empty-thread route surfaced by tapping the home page's "+" FAB. Implemented as `app/t/page.tsx` with two roles: with `?id=<question-uuid>` (legacy deep-link form) it resolves the question → its poll → its thread root and `router.replace`s to `/t/<root>?p=<pollShort>` via `getThreadHrefForPoll`; with no params it renders `EmptyPlaceholder` (shared `ThreadHeader` with `title="New Thread"`, instructional message, `<div id="draft-poll-portal" />` for `CreateQuestionContent`). The placeholder matches `isThreadRootView`, so the What/When/Where bubble bar is rendered by the template — the user picks a category from there to open the create modal. The thread "materializes" only when the user actually creates a question.
...
- **Binary 2-option ranked-choice ballot is rendered like the yes/no card pair (when no suggestion phase is in flight).** `RankingSection`'s `questionOptions.length === 2 && !canSubmitSuggestions` branch delegates to `components/QuestionBallot/BinaryRankedChoiceBallot.tsx`, which mirrors `YesNoResults`'s expanded view: two cards side-by-side, winner card colored from the live first-round IRV count (`results.ranked_choice_rounds`, `round_number === 1`), blue checkmark badge in the outer corner of the user's chosen card, % + count row beneath the cards, and an "Abstain" / "You abstained" text link to the LEFT (replacing `AbstainButton` for THIS branch only). The drag-to-reorder branch keeps `AbstainButton` below the rank list — when restructuring, move the abstain control inside its branch so each branch owns its own abstain affordance. Visual divergence from yes/no: green/gray instead of green/red (a losing option isn't a negation), and `flex-1 min-w-0` cards instead of `w-24` so rich `OptionLabel` content (restaurants/locations) fits.
  - **Suppress the rounds-list preliminary results when binary cards are visible.** `QuestionBallot.preliminaryResultsBlock` gates on `!suppressBinaryRcHere`, where `suppressBinaryRcHere = ranked_choice && questionOptions.length === 2 && !canSubmitSuggestions && (!hasVoted || isEditingVote)`. The `(!hasVoted || isEditingVote)` clause is critical: post-vote, the "Your Ballot" amber link replaces the cards, so the rounds list is the only result surface — leaving it on. Without that clause the post-vote view would render an amber link sitting in empty space.
...
- **No bottom bar. No home button.** The old three-button bottom bar (Home / + / Profile) was removed. Navigation is:
  - **Floating "+" FAB on home only**: a single circular blue "+" button pinned bottom-right via `position: fixed` + `max(1.5rem, env(safe-area-inset-right, 0px))` / `max(1rem, env(safe-area-inset-bottom, 0px))`. Tapping it navigates to `/p/` (the empty placeholder), where the user then picks a category bubble. The home page deliberately does NOT show the bubble bar — choosing What/When/Where is conceptually a per-thread decision, not a "starting fresh" decision.
  - **Floating What/When/Where bubble bar on thread-like pages**: rendered on `isThreadLikePage = isThreadRootView(pathname)` (matches `/p`, `/p/`, `/p/<id>`, `/p/<id>/`). Three pill-shaped buttons (`h-8 px-2.5 rounded-full`) each containing a big `+` glyph (`text-[1.4rem]`) plus the lowercase label (`text-[1.12rem]`) with a subtle text drop shadow, labeled "what", "when", "where", centered horizontally along the bottom via `position: fixed; left: 50%; transform: translate-x(-50%)` with `bottom: max(1rem, env(safe-area-inset-bottom, 0px))`. Each opens the create-question modal in place with a different preselection: **What** → no preselection, **When** → `?mode=time` (locks to time question), **Where** → `?category=restaurant` (preselects restaurant — user can switch to Place/custom). Auto-sets `followUpTo=<latest>` when `<body>` exposes `data-thread-latest-question-id`. The `openCreateFromBubble(extraParams)` callback in `app/template.tsx` is the single helper for this flow.
...
- **Every thread-like page (`/p`, `/p/`, `/p/<id>`) renders the dashed-border draft poll card pinned at the bottom of the poll list.** The card persists even when there's no in-flight question — it just shows the title placeholder, the What/When/Where bubble bar, and the Settings rows (voting cutoff, notes, voter name). After submit, drafts are cleared and the card resets to its empty state below the newly-added poll.
- **The What/When/Where bubble bar lives INSIDE the draft card**, just under the question list. The legacy floating bottom bar was removed from `app/template.tsx` along with `BUBBLE_BUTTON_*` constants (consumed in one place now, kept inline in `app/create-poll/page.tsx`). The home page's "+" FAB stays put; tapping it navigates to `/p/` where the user picks a category bubble inside the draft card.
