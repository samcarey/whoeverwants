...
- **Group list row layout** (`components/GroupListItem.tsx`): NO hairline dividers between rows; the visual structure is avatar-driven, not chrome-driven. Each row is a flex with `[selection checkbox?] [countdown column] [avatar] [text block]`. The countdown column sits in its own fixed-width slot (`w-7`) left of the avatar — always-reserved when an avatar renders, so indentation stays consistent whether or not an active cutoff exists. Three render states driven by `Group.unvotedDeadlineKind`:
  - `'response'` → green compact `<SimpleCountdown compact blankOnExpire>` (voting cutoff).
  - `'prephase'` → blue compact `<SimpleCountdown compact blankOnExpire>` (suggestion / availability timer is ticking).
  - `'prephase-pending'` → solid blue dot (`w-2.5 h-2.5 rounded-full bg-blue-500 dark:bg-blue-400`). The poll is in suggestion / availability phase but the timer hasn't started (deferred until first response) — there's no scheduled time to render, so a non-time-bearing indicator.
  - `undefined` → empty slot.
  Within one poll an active prephase always beats `response_deadline` (we don't surface a voting cutoff while suggestions are still open); across polls the soonest deadline wins regardless of kind. Pending prephase only surfaces when no other poll in the group has a concrete deadline. `blankOnExpire` clears the text to empty on cross-zero so no stray "Expired" word flashes before the parent's `isOpen` filter unmounts the row. `compact` mode formats via `formatCompactCountdown(diffMs)` in `lib/timeUtils.ts` — promotes to the next larger unit at the unit boundary itself (0s–59s as seconds, 1m–59m as minutes, 1h–23h as hours, 1d–6d as days, 1w–3w as weeks, 1mo–11mo as months, then `y`). Differs from `compactDurationSince` which uses a ≥ 2 threshold to preserve more precision in "X ago"-style labels; countdowns prefer the larger unit so the on-screen glyph matches the duration the creator typed (e.g. "Suggestions cutoff in 1h" reads as `1h`, not `60m`). Font size on the countdown wrapper is `text-[15.84px]` (was `text-xs` originally; tuned `text-xs` → `text-lg` (+50%) → `text-[14.4px]` (−20%) → `text-[15.84px]` (+10%) to land at a readable size that fits the column's 28px width without overflow). The countdown number span itself uses `numberClass="font-bold tracking-tighter"` (overrides `SimpleCountdown`'s default `font-mono font-semibold`); fixed-width digits weren't needed for the single-unit `Ns`/`Nm`/etc. format and a non-mono bold renders more compactly. The text/dot is `self-center` vertically with the avatar. Gap tuning is done with negative right-margin on the countdown column (`-mr-[4.224px]` → 7.776px effective countdown↔avatar gap) since flex `gap-3` (12px) is otherwise enforced uniformly; the avatar↔text gap uses `-ml-[3px]` on the text block (→ 9px effective). The outer `mr-1.5` (right-only) and the inner row's `pl-[8.064px]` keep the left edge tight against the viewport's safe-area padding while preserving asymmetric right-side breathing room. Both are gated on `!draftMode` so the draft poll card on group pages keeps the symmetric `mx-1.5` + `pl-2` chrome it relies on for its dashed border. (Earlier this column held a red `unvotedCount` pill AND a separate full-format countdown floated to the lower-right of the latest-poll body via a phantom `float-right` span on line 1; both were collapsed into this single state-aware slot. The `unvotedCount` prop on `GroupListItem` is gone — `hasUnvoted` still drives title emphasis.)
- **Text block structure** (same component): row 1 is `flex items-baseline` with the title (`truncate flex-1`) + optional draft pill + the relative-time stamp (`shrink-0`); the time stamp moved upper-right and the "X questions" prefix is gone. Below that, an empty-group status row renders only when `createdAt` is null. The latest-poll body is a `text-sm leading-tight` div with `maxHeight: 2.55em` + `overflow: hidden` to cap at 2 lines — `-webkit-line-clamp` is NOT used because it's incompatible with floated siblings (the `-webkit-box` display ignores or breaks `float` descendants), even though the float-right countdown that motivated this constraint is gone (kept the same height cap for consistent row heights). The `pr-4` (16px) right-edge buffer on the text block reserves whitespace so neither the wrapping text nor the right-aligned chrome (timestamp) touches the row's right edge.
