...
- **`CompactNameField` returns `null` when the user already has a name set + the field isn't being edited.** Both wrapper-level Submit blocks in `app/g/[groupShortId]/page.tsx` (poll multi-Submit + 1-question wrapper Submit), every legacy per-question Submit fallback (`QuestionBallot`, `RankingSection`, `SuggestionVotingInterface`, `TimeBallotSection`), and the create-poll voter-name block consume the same component and want the same hide-when-set behavior — the gate lives inside the component, not at the call site.
