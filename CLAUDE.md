...
- **Turnout context on the group card ("M of V seen") — SHIPPED.** The viewer-vs-voter distinction is a social signal — a 1-vote poll that 6 people OPENED is "low turnout," not a decisive result. `PollResponse.viewed_total` (FE `Poll.viewed_total`) is a counts-only distinct-viewer total over `poll_views`, computed in `_compute_poll_voter_data` (now a **4-tuple** `(voter_names, anonymous_count, viewed_ignored_count, viewed_total)`) as `COUNT(DISTINCT COALESCE(ub.user_id, pv.browser_id))` via `LEFT JOIN user_browsers` — so a signed-in viewer on two devices counts once (account-aware, mirroring `load_user_visibility`). NO 5-min / no-action filter (unlike `viewed_ignored_count`); it's the raw "how many saw it" denominator. Threaded `_row_to_poll` (new `viewed_total` kwarg) → all 8 callsites → `toPoll` → `isPollContentEqual` (so the card re-renders when it changes on the 5s refresh). The group card (`GroupCardItem`) shows these as text labels (NO emojis): `viewed_total` is `{N} View(s)` on the bottom-LEFT (after author·date); `voted` (= `voter_names.length + anonymous_count`) and `suggestion_count` are in the bottom-RIGHT corner cluster as a blue `{N} Suggestion(s)` part + a green `{N} Vote(s)` part, each with its phase-deadline countdown in parens and hide-rules (see the Row-layout bullet's Row 3). A companion `suggestion_count` aggregate (5th element of `_compute_poll_voter_data`'s tuple) counts distinct non-empty `votes.suggestions` across the poll's ranked_choice questions. **The respondent/viewed bubbles were removed from the group card** and now live only on the poll detail page. Counts only — viewer identities never cross the API (consistent with the ballot-privacy TODO). The poll-detail "Viewed (N)" roster was left unchanged (it conveys turnout via the bubbles + the "N viewed but haven't responded" ignored sub-line). Tests: `server/tests/test_notification_events.py::test_viewed_total_counts_distinct_viewers`, `::test_viewed_total_collapses_account_browsers`, `::test_suggestion_count_distinct`. **Demo caveat:** a CLOSED poll is hidden from a fresh viewer by the closed-before-join filter, so a live demo uses OPEN polls. **Possible follow-up (not built):** an optional "nudge the lurkers" affordance.
