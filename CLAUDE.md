...
- **TODO (name gate):** a nameless user tapping Claim gets the server's 400 instead of the `AccountGateModal` (the gate is wired for the wrapper-Submit path, which limited_supply bypasses). See the inline `// TODO (name gate)` in `QuestionBallot.tsx: handleSupplyClaim`.
