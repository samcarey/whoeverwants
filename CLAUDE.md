...
### Thread-Page Virtualization

- **The thread page renders only group cards within ~2 viewport heights of the visible region; everything else collapses to a measured-height placeholder div.** State lives on `mountedGroupKeys: Set<string>` (group key = poll_id, or `solo-<question_id>` for legacy single-question entries). On first render `mountedGroupKeys` is seeded synchronously with the URL-targeted group's key (or the last group's key when `suppressExpand` puts us in bottom-pin mode), so the layout-shift compensation has a stable anchor from frame 0. A separate `IntersectionObserver` with `rootMargin: \`${vh*2}px 0px\`` adds groups to mountedGroupKeys when they enter the buffer and removes them (other than the always-mounted anchor) when they leave. The window observer activates only after `initialScrollApplied` flips so its first batch of "intersecting" entries reflects the user's intended scroll position, not the pre-scroll `scrollY=0`.
- **Placeholders match the card they replaced.** A shared `ResizeObserver` (in `groupSizeObserverRef`) tracks every mounted card's `offsetHeight` into `groupHeightById` (a ref Map). Placeholder divs use that measured height; first-mount uses `ESTIMATED_GROUP_HEIGHT = 110` until measured. Result: unmount→remount cycles don't shift the document layout because the placeholder takes exactly the height the card occupied.
...
- **Gate on user input, not scrollY deltas.** The first version tracked `prev.offsetTop` and scrolled by `newOffsetTop - prevOffsetTop` to preserve visual position. That broke when cards above mounted SMALLER than estimated → doc shrank → browser silently CLAMPED scrollY (e.g. 1796 → 1568) → my prev was stale → my delta calculation produced wrong scrolls. Hard to distinguish browser-clamp scroll events from user-initiated ones at the JS level. Switching to "pin until first wheel/touchstart/keydown" sidesteps the entire problem: layout settling is unambiguous (no user input has happened), so we just re-pin every time.
- **`userInteractedRef` listens to `wheel` / `touchstart` / `keydown` only.** NOT `scroll` — programmatic scrolls (our own `scrollTo`, browser clamps when doc shrinks) all fire `scroll` events with `isTrusted: true`, indistinguishable from a user gesture. The three input events fire only on real user input, so they're the only safe way to detect "the user has interacted".
- **The ResizeObserver also observes `document.documentElement`.** Card-only observation misses post-card growth — the always-on draft poll form portal filling in, async-loaded fonts/images. Without docElement observation, the bottom-pin mode would land on a stale "max" before those settle. Treat the docElement entry as `layoutDirty=true` regardless of the card-height check.
- **`cardRefs` registers BOTH placeholders and mounted cards.** Both have `dataset.questionId` and `dataset.groupKey`. Existing scroll-helper code that iterates cardRefs and calls `getBoundingClientRect()` works for both — the placeholder occupies the right slot in the document, and BCR.top is correct. Only `cardFrameRefs` and `expandedWrapperRefs` (for FLIP animation and expanded-content height measurement) stay card-only because they target inner card structure that placeholders don't have.
- **React keys differ between placeholder and card.** Placeholder uses `key={\`placeholder-${group.key}\`}`, card uses `key={question.id}`. Different keys → React mounts/unmounts cleanly (separate elements). Sharing a key would re-use the same DOM div with vastly different children/refs, which fights the ref-callback lifecycle.
- **Order of definitions matters: `anchorGroupKey` (a `useMemo`) must come before any effect that references it in deps or body.** TDZ errors otherwise. The flow at the top of `ThreadContent`: `groupedThreadQuestions` → `anchorGroupKey` → `mountedGroupKeys` cleanup effect → `groupSizeObserverRef` setup → window IntersectionObserver setup → `applyScrollAdjustmentRef` definition + layout effect → user-interaction listener.
...
### Create-Question Modal — Current Architecture (Apr 2026)

The earlier docked-panel layout (poll-level fields in a bottom sheet, question form in a top sheet) was retired, then the slide-up question modal itself was retired. The current design puts the inline question form INSIDE the draft poll card with no modal layer at all.

- **Inline question form lives at the top of the draft card.** A small left-justified "Add Question" subheader sits flush at the top, then `CategoryForLine` (the `<category> for <context>` line) renders below it inside the dashed-blue card. Below: `+ Question` button (stages the inline form into the draft list), horizontal divider, then poll-level settings (voting cutoff, suggestion cutoff, notes, voter name). When questions are staged, they render as compact white cards above the inline form so the form acts as the "next item to add at the bottom of the list".
- **The up-arrow Submit lives ABOVE the card**, not inside it — on the same row as the centered "Create Poll" header. The header row uses `relative + flex items-center justify-center` with the Submit button absolute-positioned at `right-3` (so the centered label doesn't shift off-axis when the button mounts/unmounts). The label gets a `mt-[3px]` optical-centering nudge against the button's visual midline (cap-height text vs. the button's symmetric body) — same pattern as the documented icon-vs-title-line `mt-[7px]` in the thread-card chrome section. When tweaking spacing, modify the row's `mt-*` (gap above the row) or `mb-*` (gap to card top) directly; don't add inner wrappers — the `<span>` for "Create Poll" is the only flex child besides the absolute button.
- **No modal layer.** The bubble bar, the slide-up question sheet, the `?create=1` / `?openForm=1` / `?mode=time` / `?category=…` URL machinery, and `InlineTitleField` (wrapper title input) are all gone. `CREATE_PANEL_FINALIZE_EVENT`, `OPEN_QUESTION_FORM_EVENT`, `QUESTION_FORM_STATE_CHANGE_EVENT`, `BUBBLE_BUTTON_*`, `--bottom-modal-height` plumbing, `CREATE_FLOW_URL_PARAMS`, `stripCreateParams`, `reEnableForm` — all retired. Wrapper title is server-auto-generated from question categories + poll context; users can override later via `/p/<id>/edit-title`.
