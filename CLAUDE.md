...
- **Floating bubble bar auto-follows-up** when on a thread page via `document.body.getAttribute('data-thread-latest-poll-id')` — the thread page sets this attribute on mount. Special case: on the home page (`pathname === '/'`) each bubble navigates to `/thread/new?create=1&...` (with the bubble's preselection params) instead of opening the create modal in place. `/thread/new/` is handled by the existing `[threadId]` dynamic page (see "Empty thread placeholder" below) — it shows the same fixed-header chrome with a "Create a poll and then share the link!" prompt; the modal opens on top because `?create=1` is in the URL. The home → `/thread/new` navigation uses `navigateWithTransition` for the iOS-style slide; the modal mounts after the slide commits. If the user dismisses the modal without submitting, the URL params are stripped and the empty placeholder remains visible; tapping back returns to home. On submit, the new poll has no `follow_up_to` so it becomes its own thread root.
...
- **Empty thread placeholder:** `/thread/new/` is the empty-thread route surfaced by tapping any of the What/When/Where bubbles on the home page (each navigates to `/thread/new?create=1&...` so the create modal auto-opens). It is NOT a separate route file — `ThreadPageInner` in `app/thread/[threadId]/page.tsx` checks `if (threadId === 'new') return <EmptyThreadView />` BEFORE running any of `ThreadContent`'s heavy fetch/state setup. `EmptyThreadView` renders the shared `ThreadHeader` with `title="New Thread"` and an instructional message; no fetch, no thread state. If the user dismisses the modal without submitting they see the empty placeholder underneath; the bubble bar is still visible (rendered by the template, not by the page) so a second pick is one tap away. The thread "materializes" only when the user actually creates a poll (the new poll has no `follow_up_to` so it becomes its own thread root). If you ever need other placeholder thread routes (`/thread/anything-special`), reuse the same in-`ThreadPageInner` branch pattern.
...
**Status**: phasing plan in `docs/multipoll-phasing.md`. **Phases 1 and 2.1–2.2 shipped** in main:
...
**Existing legacy polls remain `multipoll_id IS NULL` indefinitely** — Phase 4 will backfill. Until then, `/p/[shortId]/` resolves whichever table has the short_id (multipoll wins on conflict).
...
- `pollDataToMultipollRequest()` in `app/create-poll/page.tsx` is the canonical mapper from the existing flat pollData into a single-sub-poll multipoll request. Wrapper-level `context` carries today's `details` field (1-sub-poll multipolls); per-sub-poll `context` is reserved for Phase 2.4 disambiguation. Add new fields to BOTH the multipoll-level OR sub-poll-level branch — never both — and keep participation polls on the legacy `apiCreatePoll` path.
...
- **No bottom bar. No home button.** The old three-button bottom bar (Home / + / Profile) was removed. Navigation is:
  - **Floating What/When/Where bubble bar**: rendered only on the home page and thread-like pages (`/` plus `isThreadLikePage = isPollPage || isThreadPage`). Three pill-shaped buttons (`h-12 px-5 rounded-full`) labeled "What", "When", "Where", centered horizontally along the bottom via `position: fixed; left: 50%; transform: translate-x(-50%)` with `bottom: max(1rem, env(safe-area-inset-bottom, 0px))`. Each opens the create-poll modal with a different preselection: **What** → no preselection, **When** → `?mode=time` (locks to time poll), **Where** → `?category=restaurant` (preselects restaurant — user can switch to Place/custom). On the home page each bubble navigates to `/thread/new?create=1&...` so the modal auto-opens after the slide transition. On thread / poll / empty-thread pages each bubble adds `?create=1&...` to the current URL in place, plus `followUpTo=<latest>` when `<body>` exposes `data-thread-latest-poll-id`. The `openCreateFromBubble(extraParams)` callback in `app/template.tsx` is the single helper for this flow.
