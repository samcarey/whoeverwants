...
- **Submit button shows "for X"** when plus-ones are added: X = 1 (you) + entries. **A single yes/no poll that allows plus-ones routes through the explicit staged-Submit flow** (`usePollSubmit = isMultiPoll || (allYesNo && allow_plus_ones)`) instead of auto-submitting on tap — so the voter can add people first and the button label can show the count.
- Tests: `server/tests/test_plus_ones.py` (default ON/OFF by type + override, yes/no + unnamed weighting, disallowed-poll clamp, named/duplicate/unnamed roster, edit updates count, **seeded editable vote + responded-flag + edit-by-represented-account, can't-seed-non-contact, skip-already-responded**).
