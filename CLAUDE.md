...
- **No bottom bar. No home button.** The old three-button bottom bar (Home / + / Profile) was removed. Navigation is:
  ...
  - **`LazyCreateQuestionContent` mounts unconditionally on `isMounted`, NOT gated on `isGroupLikePage`.** The component owns the bubble-bar portal AND the create-poll modal. Earlier it was gated on `isGroupLikePage` so it only mounted after pathname flipped to a group route — which meant the overlay-slide transition (`slideToGroup`) showed an empty `#draft-poll-portal` for the full duration of the slide, and the bubble bar popped into view ~150-300ms AFTER the slide visually completed. Symptom: "buttons at the bottom don't appear until after the page finishes sliding." Mounting unconditionally lets the always-on `MutationObserver` inside `CreateQuestionContent` detect the overlay's `#draft-poll-portal` the moment the overlay enters the DOM, well before `router.push` flips the pathname. The component renders nothing visible when there's no portal target AND no open modal, so mounting it on the home page costs nothing visual — only its lazy module's effects (form-state load from localStorage, `getUserName` lookup, MutationObserver attach). The lazy module is already preloaded via the existing `requestIdleCallback` preload, so the mount cost is one React commit. Don't re-introduce a route-based mount gate without an alternative path for the slide-overlay case.
