...
- **Inline question form lives at the top of the draft card.** `CategoryForLine` (the `<category> for <context>` line) renders flush inside the dashed-blue card, with the up-arrow Submit pinned to the right of that row. Below: `+ Question` button (stages the inline form into the draft list), horizontal divider, then poll-level settings (voting cutoff, suggestion cutoff, notes, voter name). When questions are staged, they render as compact white cards above the inline form so the form acts as the "next item to add at the bottom of the list".
