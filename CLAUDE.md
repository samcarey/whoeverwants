...
- **Auto-deploy can silently fail on uncommitted local mods.** The webhook does a plain `git pull`; if the droplet's `/root/whoeverwants` checkout has any unstaged file (typically `scripts/dev-server-manager.sh` after a hotfix), the pull aborts with `error: Your local changes to the following files would be overwritten by merge`, the webhook logs the failure but the deploy "completes" with no rebuild. Vercel meanwhile auto-deploys the new FE → FE/API contract drift. Symptom from a recent occurrence (#290 deploy stuck): every `POST /api/polls` with `thread_id: <uuid>` came back with a brand-new thread_id in the response — the old server didn't read `thread_id` at all (still expected the retired `follow_up_to` field), so polls "disappeared" into freshly-minted threads. Diagnostic: `tail /var/log/dev-webhook.log | grep -A3 'Git pull failed'` to spot the blocking file; on the droplet, `cd /root/whoeverwants && git status` to confirm; `git log --oneline -3` to confirm the actual deployed commit. Fix: stash the local diff, `git pull`, apply any pending migrations, `docker compose up -d --build`. Recovery for orphaned data: any thread minted while the deploy was stale needs to be merged into its intended thread (`UPDATE polls SET thread_id = <real> WHERE thread_id = <orphan>`, then `DELETE FROM threads WHERE id = <orphan>`).
- **Defensive log when requested `thread_id` is unknown.** `_resolve_or_create_thread` in `server/routers/polls.py` emits a `WARNING` when `req.thread_id` is provided but no matching `threads` row exists. The mint-fresh-thread fallback is intentional (per the function's docstring) but a sustained stream of these warnings is a tripwire for: (a) a stale deploy missing a schema migration, (b) the FE sending a stale/cached thread_id from before a forget+re-discover cycle, (c) cross-thread races. Surface via `bash scripts/remote.sh "docker compose logs --tail 200 api | grep 'thread_id'"` when investigating "polls landing in the wrong thread" reports.
...
- **Thread URL targets the oldest open+unresponded poll, falling back to newest.** `getThreadHref(thread)` builds the link the home list uses: returns `/t/<root>?p=<targetPoll>` when `thread.unvotedCount > 0` (auto-expand the awaiting poll on landing), else `/t/<root>` (no expand, scroll to bottom — done state, encourage starting a new poll). The targeted poll is computed by `pickTargetedPoll` in `lib/threadUtils.ts`: among polls where the viewer has at least one un-responded question, pick the oldest by `created_at`; otherwise pick the newest poll. Two helpers encapsulate the rules: `isPollOpen(poll, now?)` (not closed AND deadline-not-passed) and `pollHasAwaitingQuestion(poll, voted, abstained, now?)`. Use them whenever you need either rule — don't re-inline the `is_closed`/`response_deadline`/`some(...)` math.
- **Floating bubble bar auto-follows-up** when on a thread page via `document.body.getAttribute('data-thread-latest-question-id')` — the thread page sets this attribute on mount. The home page does NOT render the bubble bar; it has the single "+" FAB instead, which navigates to `/t/` (the empty placeholder). `/t/` shows the bubble bar (since `isThreadRootView` matches it); the user picks a category from there. If the user dismisses the modal without submitting, the empty placeholder remains visible with the bubble bar; tapping back returns to home. On submit from the empty placeholder, the new question has no `follow_up_to` so it becomes its own thread root.
...
- **`useThread(threadId)` is the canonical thread loader** (`lib/useThread.ts`). Returns `{thread, loading, error}`. Initializes synchronously from the in-memory cache via `buildThreadSyncFromCache` (from `lib/threadUtils.ts`) and only falls through to the async fetch path on cache miss — so cache hits don't trigger redundant `apiGetThreadByRouteId` / `getAccessiblePolls` round-trips. Also writes `data-page-ready` on `<html>` so view transitions capture a fully-rendered snapshot. Use this hook for any new page that needs the thread for a route id; don't re-implement the cache-first + fallback pattern inline.
- **Thread sub-routes:** `/t/<id>/info` (participant list + total count, with Back/Edit buttons) and `/t/<id>/edit-title` (input to set/clear the `thread_title` override). These render their own fixed headers and read `params.threadShortId`. `isThreadRootView(pathname)` in `lib/questionId.ts` distinguishes the root view (`/t` or `/t/<id>`, gets the thread-like FAB + bottom padding) from sub-routes (plain layout, no FAB) via the regex `^\/t(\/[^/]+)?\/?$`. Update that helper when adding more thread sub-routes. The legacy `/p/<id>/info` and `/p/<id>/edit-title` routes are thin redirect stubs (`LegacyRedirectPage` from `app/p/[shortId]/_legacyRedirect.tsx`).
- **Empty placeholder route:** `/t/` (no threadShortId) is the empty-thread route surfaced by tapping the home page's "+" FAB. Implemented as `app/t/page.tsx` with two roles: with `?id=<question-uuid>` (legacy deep-link form) it resolves the question → its poll → its thread root and `router.replace`s to `/t/<root>?p=<pollShort>` via `getThreadHrefForPoll`; with no params it renders `EmptyPlaceholder` (shared `ThreadHeader` with `title="New Thread"`, instructional message, `<div id="draft-poll-portal" />` for `CreateQuestionContent`). The placeholder matches `isThreadRootView`, so the category bubble bar (in the draft poll card) is rendered — the user picks a specific category bubble from there to open the new-question modal. The thread "materializes" only when the user actually creates a question.
...
- **`ThreadPageInner` is the resolution wrapper** at the bottom of `app/t/[threadShortId]/page.tsx`. The path id is unambiguously a poll short_id / poll uuid (the thread root) — no question-uuid cascade. Synchronously resolves to a Poll from `questionCache` via `useMemo([threadShortId])`; falls back to async `apiGetPoll{ById,ByShortId}` (404 → `setError`) on cache miss. Reads `?p=<pollShortId>` via `useSearchParams` and resolves it to `initialExpandedQuestionId` (the poll's first question id) via the same cache lookup. Legacy `/p/<id>` URLs with arbitrary ids (poll short_id, poll uuid, OR question uuid) resolve via `app/p/[shortId]/_legacyRedirect.tsx` → 302 to canonical `/t/<root>?p=<pollShort>` before this component mounts. Don't reintroduce question-uuid resolution into `ThreadPageInner` — handle it in the redirect stub if a new legacy form needs supporting.
- **Helpers for cache-walk + URL-build patterns:** `lib/threadUtils.ts: resolveThreadRootRouteId(poll)` walks `poll.follow_up_to` via `getCachedAccessiblePolls()` to find the thread root and returns its route id (short-circuits when `poll.follow_up_to` is null). `lib/threadUtils.ts: getThreadHrefForPoll(poll)` returns `/t/<root>?p=<pollShort>` — the canonical "navigate to this poll's thread with this poll expanded" URL. `lib/questionCache.ts: getCachedPollForShortId(id)` resolves an ambiguous id (poll uuid / poll short_id / question uuid) to a cached Poll. Use these whenever you need either pattern — don't re-inline the `getCachedAccessiblePolls + buildPollMap + findThreadRootRouteId` triplet or hand-roll the `?p=` URL.
...
- **Shared `ThreadHeader` component** lives at `components/ThreadHeader.tsx`. Props: `headerRef`, `title`, optional `participantNames` + `anonymousCount` (renders `RespondentCircles` when provided), optional `subtitle`, optional `onTitleClick` (makes the title block a button when provided), optional `onBack` (defaults to navigating to `/`), optional `rightSlot` (renders an action node on the right; when provided, the right padding tightens from `pr-4` to `pr-2`). The title is always left-justified within its `flex-1` container. Used by `ThreadContent` (real-thread props), `EmptyPlaceholder` in `app/t/page.tsx` (just `title`), the `/info` sub-route (`onBack` + `rightSlot=<Edit button>`), and the `/edit-title` sub-route (`onBack` + `rightSlot=<Save button>`). Don't re-implement the fixed `top:0 + padding-top:env(safe-area-inset-top) + headerRef + back button` markup in another route — extend `ThreadHeader` or import it.
- **Thread page back button always navigates to `/`**, regardless of in-app history. Earlier the button used `hasAppHistory() ? navigateBackWithTransition() : navigateWithTransition('/')`, but after creating a question on `/t/`, the prior history entry was the now-empty placeholder — back popped the user back to it instead of home. Hard-coding `'/'` is the cleanest fix and matches the user's mental model ("back from a thread → main list"). The `/info` and `/edit-title` sub-routes still use the conditional-back pattern because their natural back target is the thread, not home.
- **`useEffect(..., [])` + conditional early return = ref never attaches.** When a page renders a loading placeholder on first paint and then swaps to the real content after an async load, an effect with empty deps runs once on the first render — when the real refs don't exist yet — and never re-fires, so observers like `ResizeObserver` silently fail to attach. Fix: gate the real content behind an inner component that only mounts when data is ready (`if (loading) return <Loading/>; return <Inner {...}/>`). Effects inside `Inner` then run against refs that definitely exist. Used in `app/t/[threadShortId]/info/page.tsx` and `.../edit-title/page.tsx`.
- **`useMeasuredHeight(deps?)` (`lib/useMeasuredHeight.ts`) is the canonical hook for the fixed-header padding-top compensation pattern.** Returns `[ref, height]`. Pass `[loaded]` as deps when the element is gated behind a loading early return inside the same component (e.g. `ThreadContent` passes `[thread]`); use the default `[]` when the element mounts once with the component (e.g. `EmptyPlaceholder`, `Info`, `Editor` — those gate the placeholder at the parent level so their inner component only mounts post-load). Don't re-inline `useLayoutEffect + ResizeObserver + offsetHeight` in new thread chrome.

### Expandable Question Cards (Thread View)

- **One component, one route.** `ThreadContent` (exported from `app/t/[threadShortId]/page.tsx`) renders the thread list. `ThreadPageInner` (in the same file) resolves the URL params and mounts `ThreadContent` with `initialExpandedQuestionId` set to the `?p=` poll's first question (or null when `?p=` is absent → no auto-expand). There is no longer a standalone question page — `QuestionBallot` is only rendered inline inside an expanded card.
- **Template treatment of thread-family pages.** The template computes `isThreadLikePage = isThreadRootView(pathname)` (matches `/t`, `/t/`, `/t/<id>`, `/t/<id>/` but NOT sub-routes) and uses it for layout (flex-col overflow-hidden, no duplicate header/back button). A separate `isThreadFamilyPage` covers the broader "any /t/* OR /p/* (legacy redirect)" check used by the fallback header gate so neither canonical nor redirecting pages get the template's centered title bar. The copy-link button is rendered per-card in the upper-right of each card's compact header (visible whether collapsed or expanded), not at the page-corner level, so users can copy a poll's share link directly from the thread view without navigating away. The title uses `flex-1 min-w-0` alongside the `shrink-0` button wrapper so it wraps (line-clamp-2) before colliding with the button. Tap-to-expand is gated off the button via `stopPropagation` on the button wrapper's click/touch events.
- **URL sync without remount.** On expand we call `window.history.replaceState` to swap the URL to `/p/<expanded-poll>/` without triggering a Next.js navigation. On collapse we leave the URL alone — the user's mental model is "I'm viewing this poll" so the just-collapsed poll stays in the URL. (Earlier the collapse branch wrote `/thread/<root>/`; that fallback was removed when the `/thread/` route went away.) CLAUDE.md warns that `history.replaceState` + App Router back-nav can fight, but here it's only for URL display (the browser's own back button still pops to the prior real history entry).
...
> **Migration 106 retired per-poll access; thread URLs grant whole-thread
...
> `GET /api/threads/by-route-id/{route_id}` (with or without `?p=`)
> writes a `thread_members` row inline — sharing a thread URL is the
> canonical "invite someone" mechanism. Visibility filter collapses to
> "thread membership only" + the legacy bridge during the rollout
> window:
>
...
>      contains a question_id whose poll lives in T — thread-level, no
>      closed_at filter, /api/threads/mine only.
>
...
> thread (per the user spec: "if they received a direct link to a poll
> closed before they joined, just show the thread and don't try to show
...
> empty visible-polls list returns 200 with `[]` so the thread page can
> still render its chrome (header + Share button + the "Create Poll"
> CTA). The previous "non-member with no `?p` → 404" rule is gone.
>
> **`grant_thread_membership_inline(conn, thread_id, browser_id)`** in
> `services/threads.py` is the canonical helper for the read-endpoint
...
> `services/memberships.py` (`join_thread`, `join_thread_for_poll`)
> still open their own connections (decoupled-transactions rule); only
> the visit-path uses the inline flavor.
>
> **`UserVisibility` (in `services/threads.py`)** carries
> `joined_by_thread` + `bridged_thread_ids` only — `access_poll_ids` is
> gone with migration 106. The previously-shared
> `thread_ids_for_poll_ids` helper is also deleted (it only existed to
> fan out access-thread sets).
>
> **Top-right Share button** in `ThreadHeader.rightSlot`
> (`components/ThreadShareButton.tsx`). Tapping invokes
...
> dismissed share sheet is swallowed silently. Shares the BARE thread
> URL (`/t/<routeId>` with no `?p=`) — per-card copy-link buttons still
> emit `?p=<short>` URLs ("share this poll's view of the thread"), but
> the access semantics are now identical: both forms grant the
> recipient thread membership on visit. Wired into ThreadHeader's
> existing `rightSlot` prop, which centers the title when populated.
> The `/info` and `/edit-title` sub-routes still pass their own
> rightSlot (Edit / Save) — only the canonical thread view gets the
> Share button.

> **`DELETE /api/threads/{route_id}/membership` ("leave thread") shipped
> in #268** as the explicit teardown counterpart to the auto-join
> writes. The endpoint removes the caller's `thread_members` row for
> the resolved thread (idempotent — strangers and
...
> fire-and-forget. Note that re-visiting the thread URL after leave
> writes a fresh `thread_members` row with a new `joined_at`
...
> `/api/threads/mine` becomes retirable — `thread_members` will be the
...
> older client bundles. `next.config.ts` adds the `/api/threads` rewrites
...
- **URLs reference polls inside threads, never questions.** Threads have a route id (currently the root poll's `short_id`; Phase B will mint a real `threads.short_id`). Polls have `id` (uuid) and `short_id`. The canonical URL is `/t/<threadShortId>?p=<pollShortId>` — path is the thread root, query names the poll to expand. Sub-questions have a `questions.id` uuid for FK purposes but is never URL-able. Legacy `/p/<id>` URLs resolve via the redirect stub at `app/p/[shortId]/_legacyRedirect.tsx`; new code should always emit `/t/...?p=...` form (use `getThreadHrefForPoll(poll)` rather than constructing the URL by hand). The previous `/thread/<id>/` route is gone too.
...
- **Per-question section header uses `getQuestionSectionTitle(question)` from `lib/questionListUtils.ts`, rendered for both single- and multi-question polls.** Format: `"<Label> for <Context>"` mirroring the server's auto-title (e.g. a Time question with details="Partie" reads "Time for Partie" instead of just "Partie"). Without the type signal, a Time question's section header was indistinguishable from a Restaurant question's. The helper special-cases `time` (the Time bubble stores question_type=time but leaves category=custom, same load-bearing convention as `_category_for_title` server-side). **Yes/No is the exception: it has an explicit user-typed title (`is_auto_title === false`), so the helper returns `question.title.trim()` directly for yes_no questions instead of the literal "Yes/No" category label.** Falling through to the label here would make the section header read as if "Yes/No" were the question — surfacing "Yes/No" right under e.g. the user's "Should we go to the park?" prompt — and the hanging category icon next to the text already carries the type signal. Earlier the helper hard-coded `'Yes/No'` for parity with the server's auto-title format ("Yes/No" no spaces vs BUILT_IN_TYPES "Yes / No"); the server format only matters when the FE is mirroring `generate_poll_title` output, which the section helper never does for yes_no since the user's title is always present. Single-question polls used to render the icon alone (no title text) on the rationale that the poll title at the top of the card already named the question — that left the expanded body visually empty in degenerate cases (closed ranked_choice with zero rounds = "All voters abstained" with nothing else), so single- and multi-question rendering converged on the same header block. **Per-question titles in multi-question polls**: today every question row inside one poll shares the same `polls.title` (write-time), but the FE `getQuestionSectionTitle` reads `question.title` which on the wire is the wrapper title — for multi-question polls with multiple yes_no questions this means the section text for every yes_no question is identical to the card top. Disambiguating those is a separate architectural concern (no per-question yes_no title field exists yet); for now `details` (per-question context) is the discriminator, and yes_no's user title at least makes single-question polls correct.
...
- **Use `findChainRoot(polls)` for "pick the chain root from a list of polls."** Lives in `lib/threadUtils.ts`. Returns `polls.find(mp => !mp.follow_up_to) ?? polls[0]` (or null on empty input). The fallback to `polls[0]` is load-bearing under Phase C.3 visibility filtering — when ancestor polls are hidden from the caller, the visible polls all carry a `follow_up_to` pointing outside the visible set, and a strict `find(!follow_up_to)` returns undefined. `polls[0]` (oldest visible) is the right anchor for `buildThreadFromPollDown` so the thread renders the partial chain rather than 404'ing. Used by `buildThreadSyncFromCache`, `useThread`, `ThreadContent.fetchThread`, `ThreadPageInner.rootInitial`, and `ThreadPageInner`'s async fetch — every "I have a list of this thread's polls, what's the root?" site routes through this helper. Don't re-inline the find/fallback pair, and don't add a separate `polls.length === 0` guard before the call — the helper already returns null for empty input.
- **Audit-write rule for vote/create paths: decoupled transactions for membership writes.** Anything triggered by a vote/create/abstain that needs to write to `thread_members` MUST run in its own `get_db()` transaction, NOT share the action's transaction. Pattern: helper functions in `services/memberships.py` that open their own connection, run `INSERT … ON CONFLICT DO NOTHING`, and `try/except Exception: log+continue` so audit failures don't block the action and action failures don't strand audit rows. The composite-PK `ON CONFLICT` is load-bearing: re-voting must NOT advance `joined_at`, since visibility compares poll closure timestamps against that watermark. Order in the handler: vote/abstain endpoint writes membership BEFORE the vote (so a rejected vote still records "attempted to participate"); poll-create endpoint writes membership AFTER the create (the root-poll's `thread_id` only exists post-`_insert_poll`). The visit-path auto-join (migration 106) is the EXCEPTION — `services/threads.py: grant_thread_membership_inline` runs on the CALLER'S connection because the read endpoint already holds one and the visibility filter immediately downstream needs to see the new row. Counterpart `services/memberships.py: leave_thread(conn, thread_id, browser_id)` also runs inline on the caller's connection. The DELETE silently affects 0 rows when no row exists, which is the intended idempotent semantics.
...
- **Visibility helpers live in `services/threads.py`** (`UserVisibility`, `load_user_visibility`, `filter_visible_polls`, `grant_thread_membership_inline`). Both threads endpoints share them — adding visibility enforcement to a third endpoint would call `load_user_visibility(conn, browser_id, legacy_question_ids=...)` once and pass the result to `filter_visible_polls(conn, candidate_pids, visibility)`. Don't reinvent the rule inline; the helper is the single source of truth so changes (e.g. a dedicated `closed_at` column, retiring the legacy bridge) ripple through every read path. Migration 106 dropped `access_poll_ids` from `UserVisibility` along with the per-poll signal — the visibility rule is now thread-membership + the legacy bridge.
- **The legacy `accessible_question_ids` bridge is THREAD-level, not poll-level.** The Phase B.3 contract is "any question_id grants access to its whole thread" (the FE always passed one question_id and got every poll in the thread back). The bridge preserves this for backwards-compat: `bridged_thread_ids` resolves question_ids → thread_ids, and the visibility filter shows every poll in those threads with no closed_at filter. A per-poll bridge would silently shrink threads on first refresh post-rollout. Slated for retirement once enough rollout time has passed for active browsers to acquire `thread_members` rows via the auto-join paths (vote / create / visit).
- **Forget bridge in `/api/threads/mine`.** When `accessible_question_ids` is non-empty, member-threads are narrowed to those still represented in the bridge list. Without this, a `thread_members` row would keep a thread alive on the home list even after the user forgot every question in it. Membership-only callers (empty list) skip the narrowing — the bridge is opt-in. Eventually retired once `apiLeaveThread` (forget-of-last-poll) becomes the standard exit path.
...
- **Poll**: top-level entity. Owns: optional context, voting cutoff, optional shared suggestion/availability cutoff, `follow_up_to`, `is_closed`, `close_reason`, `creator_secret`, `short_id`. Reachable inside its thread via `/t/<threadRoot>?p=<shortId>`.
...
Sharing `/t/<thread>?p=<pollShortId>` to iMessage / Slack / Twitter / etc. should render a card with the linked poll's title. Implementation:

- **Backend `GET /api/threads/by-route-id/{route_id}/preview` (`server/routers/threads.py`)** is a public, identity-free endpoint that returns ONLY `{title, description}`. No visibility filtering, no membership writes — crawlers have no browser identity, and gating them on visibility would 404 every share. Returning ONLY title + description (never vote data, voter names, or per-question contents) keeps the surface tight enough that "anyone with the URL can see it" is acceptable. The matching `ThreadPreviewResponse` Pydantic model is defined inline in the same router file. **Title** is the poll's auto-generated title via `generate_poll_title(...)`; the `thread_title` override is intentionally bypassed so a custom thread name (often a participant-name string like "Alice, Bob") doesn't replace the poll's actual subject. **Description**: comma-joined options across the poll's questions if any are set; else the poll's `details` (Notes); else null. Capped at 200 chars. The endpoint accepts an optional `?p=<pollShortId>` to target a specific poll within the thread; otherwise it falls back to the thread's most recent poll by `created_at DESC`.
- **Frontend metadata is on a server-component `app/t/[threadShortId]/page.tsx`**, NOT on a layout. The actual client UI lives in `app/t/[threadShortId]/ThreadPage.tsx` (`"use client"`); the new `page.tsx` is a thin server-component shell that exports `generateMetadata` and default-exports `<ThreadPage />`. The shell exists ONLY to access `searchParams.p` — page-level `generateMetadata` receives `searchParams`, layout-level does NOT. Without this split, the layout would have to pick a poll heuristically (latest by `created_at`) and shares of older polls in a multi-poll thread would surface the wrong title. When you need to add metadata to ANY route whose page is `"use client"`, follow this same split: rename existing → `<RouteName>.tsx`, new `page.tsx` is server-component with `generateMetadata` + `import + default-export`.
- **Use `getApiEndpoint('threads')` from `lib/api/_internal.ts`** for the server-side fetch. Don't roll your own SSR-aware base URL helper — the existing one already handles `NEXT_PUBLIC_API_URL` overrides + branch-slug resolution + the `typeof window === 'undefined'` SSR branch. Earlier this work introduced a duplicate `lib/serverApi.ts` that missed the `NEXT_PUBLIC_API_URL` case; deleted in favor of the existing helper. The existing fetch wrapper `fetchWithBase` in `_internal.ts` is intentionally bypassed here — it injects `X-Browser-Id` (which would mint per-crawler `thread_members` rows on the visibility-aware endpoint), and the preview endpoint is identity-free anyway.
- **Cache lightly: `next: { revalidate: 60 }`** on the metadata `fetch` so repeated crawls don't hammer the API but a poll's title becomes preview-correct within a minute of an edit. Next.js's Data Cache keys on the full URL including search params, so `?p=A` and `?p=B` are independent entries. If you change the schema (add a field to `ThreadPreviewResponse`), bump the revalidation window down to 0 once and back to 60 after the rollout, or wait the cache out.
- **No `og:image` / `twitter:image`** on thread previews. Per user spec: messaging-app previews dedicate the freed space to the title at larger size. Note: Next.js child Metadata REPLACES the parent's `openGraph` and `twitter` blocks entirely (NOT field-by-field merge), so simply omitting the `images` array on the child is enough to drop the inherited 512×512 image — no explicit `images: undefined` needed.
- **`metadataBase: new URL('https://whoeverwants.com')` on the root layout** (`app/layout.tsx`) is required for any relative-URL `og:image` / `twitter:image` to resolve. Without it, Next.js emits a relative path that crawlers can't fetch and skips the thumbnail. Even though thread previews are imageless, root-level image metadata still needs `metadataBase` set so the home / settings / create-poll routes' previews work.
- **`_category_for_title(question_row)` in `routers/polls.py`** is the canonical source for "what category string should drive the auto-title for this question?" Time questions store `category="custom"` in the DB even when the user picked the Time bubble (the form's default category is "custom" and the bubble doesn't override it post-Phase-2.3). The helper returns `"time"` for any row with `question_type="time"` regardless of the stored category, so a time poll auto-titled "Time for Movie" doesn't get re-rendered as "Custom for Movie" by `_compute_display_title` or the link-preview endpoint. The helper is shared between `_compute_display_title` (in-app) and `get_thread_preview` (link previews) so they can't drift. If you add a new question type whose category column may diverge from its conceptual category, extend this helper rather than fixing it at each callsite.
- **Pitfall: visibility-gated reads return [] to crawlers.** The visibility-aware `GET /api/threads/by-route-id/{id}` enforces `thread_members` visibility against the request's `browser_id`. Server-component metadata fetches from Vercel run with no browser identity → fresh middleware-minted browser_id → membership inline-grant fires → SSR fetch sees only the polls in this brand-new "membership window" (open polls, closed polls untouched since the SSR call started). For link-preview content that should reflect a poll regardless of close timing, use the public `/preview` endpoint (separate route, no membership writes). Don't try to hack visibility-bypass into the visibility-aware endpoint; keep "public preview" and "authenticated read" as separate URLs with separate threat models.
- **Pitfall: Next.js metadata cache outlives the deploy.** After a server-side preview-format change (e.g. fixing the time-question category bug), the FE cached the OLD `{title, description}` body for up to 60s — `curl http://localhost:3001/t/<id>/?p=<poll>` keeps returning stale `<title>` until the cache window expires or you cache-bust by adding any unrelated query param. When verifying preview changes on dev, expect a 60s lag or hit a slightly different URL.
...
- **The thread page's `QUESTION_VOTES_CHANGED_EVENT` patcher must include `prephase_deadline`.** The handler in `app/t/[threadShortId]/page.tsx` refetches the wrapper after a vote and merges fields into thread state via `patchThreadPolls`. The `<ThreadCardItem>` status row reads `wrapper?.prephase_deadline` to choose between the static "Taking Suggestions" / "Collecting Availability" labels and a live `<SimpleCountdown>`. If the patcher omits `prephase_deadline` (as it did originally — only `voter_names`, `anonymous_count`, `questions`), the freshly-fetched timestamp is silently dropped and the card stays stuck on the static label until a manual refresh. `QuestionBallot`'s own `setSuggestionDeadlineOverride` only updates the ballot's local UI, not the wrapper-driven thread card. Same field also covers the deferred availability-timer for time questions. When adding any future "this field starts on first vote" semantics, audit every `patchThreadPolls(...)` callsite to ensure the field is propagated.
...
- **Stable handler/setter identity is invariant across the component lifetime — the equality fn does NOT compare them.** This is a deliberate assumption: useState dispatchers are inherently stable, callbacks passed to `ThreadCardItem` are wrapped in `useCallback([])` (`attachCardEl`, `detachCardEl`, `resetSwipeRef`) or pinned via `useRef(...).current` in `lib/useThreadVoting.ts` (`setPollVoterName`, `handleWrapperSubmitStateChange`, `submitSwipeAbstain`). If a parent stops pinning a callback, it'll show up as a stale-closure bug, not as missed re-renders — fix it in the parent. New handlers passed to ThreadCardItem MUST be stable; before adding one, audit its closure for reactive state and pick the right pinning pattern.
- **Card-local handlers (touch/swipe/click) live inside `ThreadCardItem`, not in the parent.** `handleTouchStart`, `handleTouchEnd`, `handleTouchMove`, `finalizeSwipe`, `toggleExpand`, `handleClick` close over per-card props (`question`, `group`, `swipeEligible`, `isExpanded`) plus stable refs/setters/callbacks passed in. Recreating them per render is cheap because they only ever execute when the user touches that specific card. The previous version recreated all N×6 handler closures on every parent render — moving them to the child means each card's handlers are recreated only when that card itself re-renders.
- **The placeholder branch stays in the parent's `.map()`.** Unmounted groups render as `<div style={{height: groupHeightById.current.get(group.key) ?? ESTIMATED_GROUP_HEIGHT}} />` directly inside the parent — they're trivial JSX with no per-card state that needs memoization. `attachCardEl`/`detachCardEl` are shared between the placeholder and `ThreadCardItem` so both register in `cardRefs` (the scroll-helper logic that iterates `cardRefs` works regardless of mount state).
- **`SwipeState` and `ThreadCardGroup` types live in `ThreadCardItem.tsx`** and are imported by the parent. Same-file colocation keeps the prop interface and the consumer's primitive computation discoverable in one place; if a future refactor needs these elsewhere, lift them to a shared `threadCardCommon.ts`.
- **`pendingPollSubmit`, `pendingVoteChange`, `voteChangeSubmitting`, `confirmPollSubmit`, `confirmVoteChange` stay in the parent** — they only feed the page-level confirmation modals, never the per-card chrome. Don't push them into `ThreadCardItem`'s prop interface.
- **Future bounded-memory scroll-window (deferred): the IO-driven mount/unmount window past ±2 viewport heights is now affordable** (a card's mount/unmount is ~free for siblings since memo skips them). Implementing this means changing the progressive-fill effect to instead drive `mountedGroupKeys` from the IntersectionObserver: add when crossing into a `±2 × innerHeight` envelope, remove when crossing out. The placeholder's measured height keeps scroll position stable across mount/unmount cycles. Hold off until threads actually hit hundreds of polls — premature scroll-driven virtualization can cause flicker if the placeholder height is mismeasured.
- **Initial mount = anchor only; rest fills in idle-time around the anchor.** Mounting all groups upfront pays a heavy initial-render cost on long threads (each card in the .map is ~200ms in dev mode). The current compromise: initial render contains only the URL-anchored card. A `useEffect` then walks a distance-from-anchor queue and mounts groups in batches of 4 per `requestIdleCallback` tick (falls back to `setTimeout(16)`), so the surrounding cards "fill in around" the anchor visibly. With memoized cards, each progressive-fill batch only re-renders the newly-mounted cards — not the entire list — so the fill is visually smooth even on long threads.
...
- **The create-poll panel is two stacked sheets, NOT a single modal.** Triggered by the `?create` query parameter on any page; titled "New Poll" (bottom panel) + "New Question" (top form). The bottom panel is a docked, **non-blocking** sheet — no backdrop, page stays scrollable + interactable. Backed by a single `CreateQuestionContent` component that renders BOTH the bottom panel (poll-level fields) and the top form (per-question fields) via portals, plus a "draft poll" card portaled into the page's poll list at `#draft-poll-portal`. Drafts persist in `questionFormState` localStorage as `QuestionDraft[]` (richer than legacy `CreateQuestionParams`-snapshot so editing round-trips losslessly).
- **The bottom panel is bottom-anchored and content-sized.** It uses `absolute bottom-0 left-0 right-0` with `maxHeight: calc(100% - env(safe-area-inset-top, 0px) - 15px)` so it grows from the bottom up to its content, only capping when content would push above the safe-area+15px gutter. The inner scrollable wrapper is `overflow-auto overscroll-contain min-h-0` (NOT `flex-1`) — `flex-1` would collapse to 0 under a content-sized parent, leaving the form invisible; `min-h-0` lets the same wrapper shrink-to-fit when the cap kicks in for tall content. Avoid reintroducing a fixed `top:` constraint or you'll be back to "modal always covers same area regardless of content," with white space below short forms.
- **The top question form anchors to `bottom: calc(var(--bottom-modal-height) + 8px)`** so it sits just above the bottom panel and follows it as the panel grows / shrinks. `template.tsx` publishes `--bottom-modal-height` on `<html>` via a `ResizeObserver` on the bottom-panel sheet ref. Max-height clamps at `calc(100vh - var(--bottom-modal-height) - safe-area-top - 24px)` so the top sheet never crosses the notch when the bottom is short.
- **`CreateQuestionContent` is exported** from `app/create-poll/page.tsx` and lazy-loaded via `React.lazy` in `template.tsx`. The `/create-poll` route redirects to `/?create`.
- **Drafts list lives in the page poll list, NOT the panel.** Each route that wants the draft poll card to appear renders an empty `<div id="draft-poll-portal" />` at the bottom of its existing list (home page after `<ThreadList>`; thread page after the threads loop; empty-thread placeholder). `CreateQuestionContent` portals the draft poll card into that target. The lookup uses a `MutationObserver` on `document.body` (subtree+childList) so route navigations swap the target node without leaving us pointing at a detached element. Re-queries are coalesced into one `requestAnimationFrame` per frame to avoid running the lookup hundreds of times during typing / scroll / animation.
- **The draft poll card uses the SAME `ThreadListItem` component as the live poll list** with a `draftMode` flag (dashed-blue chrome + DRAFT pill) and a `finalizing` flag (transient — releases the chrome over a CSS transition on submit). The submit-time morph (border-style, border-color, bg, DRAFT pill collapse, title color) lands on the same DOM tree the live card uses, so the transition reads as 'becoming real' rather than rebuilding. `ThreadListItem.hideRespondents` skips the left RespondentCircles column entirely so the draft doesn't show a "?" placeholder before anyone has voted.
- **`draftPollPreview(drafts, pollContext)` mirrors `server/algorithms/poll_title.generate_poll_title`** so the draft card's preview title matches what the server picks on submit. The duplication is intentional and called out in a header comment; both sides have to evolve together when adding a new category to `BUILT_IN_TYPES`. The auto-title rules (matched on both sides):
...
- **`sharedDraftContext(drafts)` exported from `createPollHelpers.ts`** returns the case-insensitive-shared `forField` across every staged draft, or null when one is missing or values diverge. Used by `openModalFor` in `app/create-poll/page.tsx` to pre-fill the new question form's `forField` so adding a 2nd "Restaurant for Tonight" question to a "Movie for Tonight" draft poll inherits "Tonight" without retyping. Editing existing drafts (`handleEditDraft`) preserves their own `forField` — the inheritance only applies on the new-draft path. Don't double-apply: `emptyDraft({ forField: ... })` plumbs the value into a fresh draft only; mutating an existing draft's `forField` from "shared" logic would silently overwrite a user's deliberate divergence.
- **Custom event names live in `lib/eventChannels.ts`** with typed `Detail` interfaces — `QUESTION_FORM_STATE_CHANGE_EVENT` (top form open/close → template hides bubble bar), `OPEN_QUESTION_FORM_EVENT` (bubble tap when panel already open → CreateQuestionContent opens fresh top form), `CREATE_PANEL_FINALIZE_EVENT` (submit → template slides panel down). Same pattern as the existing `QUESTION_VOTES_CHANGED_EVENT` in `lib/api/votes.ts` — never inline these strings at call sites.
- **`?openForm=1` URL marker** is set by the bubble bar in `template.tsx: openCreateFromBubble` when the panel was closed at click time. CreateQuestionContent's auto-open useEffect treats it as a trigger so tapping any bubble (including "what" with no preselect) opens BOTH the panel and the top form in one gesture. The marker is `replaceState`d off the URL after consumption so refresh / view-transitions don't re-trigger. Plain reload-with-saved-drafts still keeps the top form closed so the user sees their drafts list first.
- **Floating What/When/Where bubble bar visibility** is `(isThreadLikePage || isCreateModalOpen) && !questionFormOpen && !modalClosing`. When `isCreateModalOpen`, the bar floats at `bottom: calc(var(--bottom-modal-height) + 8px)`; otherwise (thread-like page, no panel) at `bottom: max(1rem, env(safe-area-inset-bottom))`.
- **`modalClosing` must be reset on EVERY `isCreateModalOpen` transition**, not just on open. Earlier the reset useEffect early-returned when `isCreateModalOpen` flipped false, so after the user closed the panel the `modalClosing=true` set by `handleCloseCreateModal` stayed stuck and gated off the bubble bar visibility (`&& !modalClosing`) forever. The reset effect now has no early return — its body runs on both open and close transitions, with the close still showing the slide-down animation because `setModalClosing(true)` was called synchronously by the close handler 300ms before the URL flip.
- **Poll-level Context vs Notes** are two separate fields on the bottom panel: Context is a short single-line input that drives the auto-title's "for X" suffix (maps to `polls.context`); Notes is a multi-line textarea with link support (maps to `polls.details` — wrapper column added in migration 103, wired through `CreatePollRequest` / `PollResponse` / `_insert_poll` / `_row_to_poll`). Don't confuse with `questions.details` (per-question context, added by migration 068 to the OLD polls table that 097 renamed to `questions`) — the two `details` columns live on different tables and carry different meaning. **Notes never feed the autotitle.** The FE-only `Context` field doesn't exist yet, so submit always sends `context: null` and `draftPollPreview(drafts, '').title` mirrors that — passing the Notes textarea (`details` state) into `draftPollPreview` would make the preview show a "Cat for <notes-text>" title that the server would never produce, divorcing preview from reality. If a Context UI field is added later, it goes into the `pollContext` arg of `draftPollPreview`, not Notes. **Poll-level notes render in the expanded thread card** via `<QuestionDetails details={wrapper.details} label="Notes: " />` at the top of the expand-clip wrapper in `app/t/[threadShortId]/ThreadCardItem.tsx` — same component used for `question.details` (per-question context inside `QuestionBallot`, no `label` passed), so multi-line text + link rendering + truncation/expand chevron come for free. The `label` prop renders as `<span className="font-medium">` inline before the text — match this `font-medium` (weight 500) convention with the existing "Notes:" label in the create-poll draft form (`app/create-poll/page.tsx:1778`); don't reach for `<strong>` (weight 700) since the two surfaces should visually match. The optimistic placeholder (`synthesizePlaceholderPoll`) carries `details` through too so the user sees their typed Notes immediately on submit, before the API resolves.
...
- **`applyDraftToState` URL guard**: only call `router.replace` when the `mode` URL param is actually changing. Without the guard, every "open new question" tap pushes a redundant history entry (and may interact with view transitions). Pattern applies to any state-sync via URL params — read first, write only on diff.
...
### Create-Question Modal — Current Architecture (May 2026)

The flow has gone through three iterations: docked-panel (top + bottom sheets), then always-visible inline form inside the draft card, and now (current) a category-bubble bar + a single rounded-corner modal for the per-question form.

- **Bubble bar at the bottom of the scroll** (in-flow, not floating fixed) lists every `BUILT_IN_TYPES` entry plus a `Custom` tile. Centered + wrapping. Tapping any bubble calls `openModalFor(category)` which seeds a fresh `emptyDraft({ category })` and opens the modal. The bubble bar always renders on thread-like pages; the gap below the bubbles is just the page's outer `paddingBottom` (template.tsx) — no spacer trickery. Constant lives at module scope as `BUBBLE_ENTRIES` (don't recreate per render). Order matches the `TypeFieldInput` dropdown so muscle memory carries over.
- **The draft poll card only renders when `drafts.length > 0`.** Tapping a bubble + dismissing the modal without confirming leaves no trace of an in-progress poll — exactly matching the user-spec "if you were creating a first question for a draft poll the poll card will not be created."
- **Modal: rounded-corner sheet anchored bottom on mobile, centered on `sm:` breakpoint.** Header bar has X-on-left (dismiss), centered "New Question" / "Edit Question" label, and a checkmark on the right (commit). Backdrop click + Escape both dismiss. Body is `flex-1 overflow-y-auto` with `pb-[4.5rem]` matching the page's `4.5rem` outer bottom padding so elements have the same breathing room above the modal edge that the bubbles have above the screen edge. Mounted via `ModalPortal`.
- **Edit flow: don't pop on pencil-click.** The pencil icon on a staged draft calls `handleEditDraft(index)` which (a) loads the draft into modal form state, (b) sets `editingDraftIndex`, (c) opens the modal. The draft stays in the list. Confirm REPLACES at that index; dismiss leaves the list untouched. The earlier "pop on edit, re-insert on confirm" pattern lost the original draft if the user dismissed.
- **Auto-open the modal when URL params (`?duplicate=`, `?voteFromSuggestion=`) prefill the form.** Without this the prefill is invisible because the draft card stays hidden (drafts.length === 0). One-line `setIsModalOpen(true)` after the prefill effects.
...
- **Smooth-scroll the just-materialized draft card up under the fixed header** when drafts goes 0 → 1 (only — subsequent additions don't move the card enough to warrant scrolling). `scrollDraftCardUnderHeader` reads the header's bottom via `[data-thread-header]` (added to `ThreadHeader`'s outer fixed div), computes `target = scrollY + cardTop - headerBottom - 12`, and `scrollTo({behavior: 'smooth'})`. Three rules:
  1. **Don't push the card downward.** If `target <= window.scrollY`, return without scrolling.
  2. **Don't artificially extend the page.** Earlier we added a 70vh trailing spacer so the browser could always scroll the card to the top — that fights the user's spec ("if we are at scroll bottom and the draft card is lower than the top, that's fine"). Without the spacer, the browser clamps to `maxScroll` naturally.
  3. **Two `requestAnimationFrame`s are enough** — fire after `setIsModalOpen(false)+setDrafts`. The first lets React commit; the second runs after the body-scroll-lock cleanup's instant `scrollTo` so our smooth scroll wins. Don't add a `setTimeout` retry — the portal is mounted by then.
- **`THREAD_HEADER_ATTR` lives in `lib/threadDomMarkers.ts`.** Producer (`ThreadHeader`) writes `data-thread-header=""` directly (a literal attribute, not the `{...{[ATTR]:''}}` spread trick). Consumer (`create-poll/page.tsx`'s `scrollDraftCardUnderHeader`) reads via `querySelector(\`[${THREAD_HEADER_ATTR}]\`)`. Mixing the two — constant on the consumer, literal on the producer — is fine: the value is a literal string and JSX-attribute names can't be dynamic without the spread trick anyway.
- **Implicit follow-up via `<body data-thread-latest-question-id>`.** The earlier bubble bar wrote `?followUpTo=<qid>` into the URL; that path is gone. The submit handler reads the body attribute directly to determine the parent question. Skipped when on `/p` (empty placeholder) — by construction the user is starting a new thread, and the body attribute can be stale (the thread route's cleanup is a useEffect return that React/HMR/view-transitions can delay). `EmptyPlaceholder` also clears the attribute on mount as belt-and-braces.
...
- **Implicit follow-up via `<body data-thread-latest-question-id>`.** The previous bubble bar wrote `?followUpTo=<qid>` into the URL; that path is gone. The submit handler now reads the body attribute directly to determine the parent question. Skipped when on `/p` (empty placeholder) — by construction the user is starting a new thread, and the body attribute can be stale (the thread route's cleanup is a useEffect return that React/HMR/view-transitions can delay). `EmptyPlaceholder` also clears the attribute on mount as belt-and-braces.
- **No DOM-mutation form-disable.** The submit handler used to imperatively walk `document.querySelector('form').querySelectorAll(...)` and set `.disabled = true`. Two failure modes: it only matched the FIRST form on the page (potentially the wrong one), and the captured form ref went stale across `router.replace` so `reEnableForm` operated on a detached tree — manifesting as "submit button unresponsive after first submit". Every input/select/button already takes `disabled={isLoading}` from React state; that's the single source of truth.
- **Empty-thread submits stay on `/p` until the API resolves.** Eagerly redirecting to `/p/<placeholderPoll.id>/` (the previous flow) loses, because `pending-...` ids don't resolve as UUIDs or short_ids — the destination renders "Poll Not Found" and that view doesn't include `#draft-poll-portal`, so a subsequent API failure leaves restored drafts + error message invisible (they have nowhere to render). Stay on `/p`; success branch redirects to the real short_id, failure branch leaves the user in place with the portal intact.
- **Same-kind / distinct-context pre-validation.** Mirrors `server/routers/polls.py: _validate_request` client-side: groups drafts by `(question_type, category)` and rejects when group contexts collide (case-insensitive, empties collide). Surfaces inline in the same red box that displays post-submit errors (gated on `drafts.length > 0`) so the live `validationError` is visible whenever Submit is disabled — not just after a tap, since a disabled button blocks the click that would otherwise call `setError`.
...
- **`rebuildThreadFromCacheOrPrev(prev, mutate?)` is the shared rebuild for POLL_PENDING / POLL_HYDRATED / POLL_FAILED.** Source-of-truth merge: `byId = prev.polls + cached + (mutate.add) - (mutate.remove)`. **Always merging `prev.polls`** is the resilience fallback for stale `accessiblePollsCache` — the cache has a 60s TTL, and the submit handler's `cacheAccessiblePolls([...getCached() ?? [], newPoll])` pattern wipes every other poll out of the cache when the cache happens to be stale (idle >60s). Without `prev.polls` in the merge, `buildThreadFromPollDown(rootPollId, ...)` can't find rootPollId and bails, leaving the thread frozen. Earlier the success path was an in-place `prev.polls.map(p => p.id === placeholderId ? realPoll : p)`, with a fallback for "POLL_PENDING bailed"; the hand-rolled swap was fragile across browsers (Firefox iOS hit a listener-registration race that left the swap a no-op). Both POLL_HYDRATED and POLL_FAILED route through this rebuild now and return `prev` when the rebuilt poll-id sequence matches (so identity-based memos stay stable). The submit handler ALSO uses `updateAccessiblePollsIfFresh(merge)` (`lib/questionCache.ts`) instead of the raw `cacheAccessiblePolls(...getCached() ?? [])` pattern — the helper's null-guard prevents wiping the cache to just `[newPoll]` when the cache was stale.
- **`mutate.add` / `mutate.remove` are required so swap is explicit.** Without them, leaving the placeholder AND the real poll both in the rebuild source yields a thread containing both as children of the same parent. POLL_PENDING passes `{ add: placeholder }`, POLL_HYDRATED passes `{ add: realPoll, remove: placeholderId }`, POLL_FAILED passes `{ remove: placeholderId }`.
- **`isPendingPollId(id)` (`lib/threadUtils.ts`) is the canonical check** for placeholder polls. Used by the thread page's `maybeFetch` and POLL_FAILED-state guard, and by `ThreadList`'s viewport prefetch — all to skip API calls on `pending-...` ids that 500 server-side (`psycopg.errors.InvalidTextRepresentation` because they're not valid UUIDs). Don't inline `id.startsWith('pending-')`.
- **POLL_PENDING / POLL_HYDRATED also setMountedGroupKeys directly.** ThreadCardItem rendering is virtualized: groups not in `mountedGroupKeys` render as a fixed-height gray placeholder div, with progressive-fill walking outward from the URL anchor in idle-time batches of 4. When a brand-new poll lands, the validation effect (`mountedGroupKeys = prev ∩ validKeys + anchor`) doesn't add the new key — it was never in `prev` and isn't the URL anchor — so the new card renders as a gray div until progressive-fill reaches it. On a 50+ poll thread anchored mid-thread, that's ~17 batches (≈270ms) for a chronologically-last new poll. Fix: each handler does its own `setMountedGroupKeys((prev) => { if (no-change) return prev; const next = new Set(prev); next.add/delete; return next; })` alongside the setThread, mounting the new card on the same render tick. The validation effect's intersection preserves these keys on subsequent runs since they end up in `validKeys` once the rebuild lands them in thread state. Symptom of forgetting this: card animates in via POLL_PENDING (it happened to be in the queue from prior progressive-fill), then disappears into a gray div on POLL_HYDRATED.
- **POLL_HYDRATED async-refresh fallback for missing-ancestor case.** When the new poll's parent isn't in `prev.polls` (e.g. the parent was discovered AFTER thread state was built — accessiblePollsCache was invalidated by discovery, so the cache fallback can't fill it in either), the in-place rebuild leaves the new poll out of the chain. After the optimistic setThread, async `await getAccessiblePolls()` (which fetches based on localStorage's full accessibleIds, including newly-discovered ancestors) and re-runs the rebuild. **Skip the async refresh when the optimistic already added the realPoll** (`optimisticWillAdd = hasPlaceholder || isFollowUp || isOwnRoot`, computed from `threadRef.current` BEFORE setThread because setState is async). On the happy path this saves a cache-fetch + redundant setState per submit. The async path remains the only fix for the missing-ancestor case where the optimistic bails to prev.
- **Submit handler always caches the real poll (or evicts the placeholder on failure) before dispatching, so the cache is the source of truth.** POLL_FAILED additionally early-returns when no `pending-...` poll is in thread state — the failure typically fires while the user is on a different thread.
- **Home page listens to POLL_HYDRATED only.** Newly-created polls auto-appear in the home list without refresh. POLL_FAILED is intentionally not listened to (placeholder polls never reach the home cache, so failure can't change the home list). The handler diffs poll-id sequences before calling `setPolls` — `getAccessiblePolls()` returns a new array on every call, so a content-equality check prevents re-rendering every `ThreadList` row when nothing changed.
...
- **Hot-path event listener pattern: state-via-ref + empty deps.** ThreadContent's POLL_PENDING listener used to depend on `[thread]`, which tore the listener down + re-added on every vote / hydration / cache refresh. Now `threadRef.current = thread` is updated in a separate effect and the listener reads from the ref with empty deps. Same pattern as the existing POLL_HYDRATED listener. Apply this any time a long-lived window listener needs to read state that changes frequently.

### Always-On Draft Poll Card

- **Every thread-like page (`/p`, `/p/`, `/p/<id>`) renders the dashed-border draft poll card pinned at the bottom of the poll list.** The card persists even when there's no in-flight question — it just shows the title placeholder, the per-category bubble bar (`BUBBLE_ENTRIES` = `BUILT_IN_TYPES` + `Other`), and the Settings rows (voting cutoff, notes, voter name). After submit, drafts are cleared and the card resets to its empty state below the newly-added poll.
- **The category bubble bar lives INSIDE the draft card**, just under the question list. Each bubble represents a single category (Time / Restaurant / Place / Movie / Yes/No / Pet Name / Video Game / Other). Tapping a bubble calls `openModalFor(category)` which seeds a fresh draft with that category preselected via `emptyDraft({category})` and opens the new-question modal. The earlier floating What/When/Where 3-bubble bar was removed; the legacy bottom-floating bar was removed from `app/template.tsx` along with `BUBBLE_BUTTON_*` constants. The home page's "+" FAB stays put; tapping it navigates to `/p/` where the user picks a category bubble inside the draft card.
- **DOM markers are named constants, not magic strings.** `lib/threadDomMarkers.ts` exports `THREAD_LATEST_QUESTION_ID_ATTR` and `DRAFT_POLL_PORTAL_ID`. The thread page (writer) and create-poll (reader) both import them. Hardcoded strings on either side would silently break the integration without the constant.
- **MutationObserver for the `#draft-poll-portal` target stays armed for the component's lifetime, not self-disconnecting.** The portal target is rendered inside ThreadContent's main return — but ThreadContent has loading + error early-returns that don't render it. When the page transitions through one of those states (or after a route change to a fresh thread), a self-disconnecting observer keeps a stale reference to a detached node and the draft card portals into invisibility. Always-armed + rAF coalescing keeps the cost negligible.
- **Within-thread sort is pure chronological ascending (`created_at` oldest first, newest at the bottom).** `lib/threadUtils.ts: collectDescendants` no longer groups closed/expired polls separately — the comparator is just `new Date(a.created_at) - new Date(b.created_at)`. Result: the most recently submitted poll (open or closed) always lands at the very bottom of the thread, just above the always-on draft card. `latestActivityMs` is still computed as the true `max(created_at)` across all polls so the home page's cross-thread "by recency" sort doesn't get coupled to the in-thread order.
- **Submit button is the only Submit on the page.** Lives in the upper-right of the inline form (CategoryForLine row). `disabled={submitDisabled}` reads `isLoading || isSubmitted || !!validationError` straight from React state. `validationError` is also displayed in the red error box whenever `drafts.length > 0` so the user sees what's blocking submit, not just a greyed-out button.
- **The "Create Poll" header above the card swaps to a live preview of the autogenerated title once the form is valid.** The gate is `projectedDrafts.length > 0 && !validationError` — deliberately NOT `!submitDisabled`, since the latter would flicker the preview back to "Create Poll" during `isLoading`/`isSubmitted` mid-submit. The preview computes `draftPollPreview(projectedDrafts, details).title`, which mirrors the server's `generate_poll_title` (poll-level `details` → `polls.context` → "for X" suffix). Note the placeholder card title comes from `synthesizePlaceholderPoll` which currently passes `''` for context (and uses `deriveDraftTitle` for single drafts via per-question `forField`) — so when `details` is set, the user sees a brief title flicker on submit: preview-with-context → placeholder-no-context → final-with-context. Fixing this would require threading poll-level context through both `synthesizePlaceholderPoll` and `deriveDraftTitle`'s suffix logic.
- **`truncate` on a `<span>` needs explicit block formatting.** The header span uses `inline-block truncate max-w-[calc(100%-4rem)]` so the preview ellipsis renders reliably. Default `<span>` is inline; even though flex blockification SHOULD apply, browser support for `text-overflow: ellipsis` on inline spans inside flex containers is patchy. Add `inline-block` (or `block`) explicitly anywhere you apply `truncate` to a span. The `max-w-[calc(100%-4rem)]` reserves space for the absolute-positioned 36px Submit button at `right-3`.
- **Stage-current-question is a bare-blue check glyph on the right of the CategoryForLine title row**, not a separate "+ Question" button below the form. It's `absolute right-0 top-1/2 -translate-y-1/2 w-9 h-9 text-blue-600` with an inline 24px check `<svg>`. The outer wrapper is `relative` and the `CategoryForLine`/`AnimatedTitle` lives inside an inner `pr-12` wrapper so the right-padding is reserved against the button — without it, the title's font-fit binary search measures full-width and the title collides with the button. Don't switch to flex (`<div className="flex"><CategoryForLine/><button/></div>`) — `CategoryForLine` is `text-center` internally and reads its container width for font-fitting, so the absolute-positioned pattern + `pr-12` reservation keeps the centering symmetric within the available content area. Drop `flex-shrink-0` if you copy this pattern: it's a no-op on `position: absolute` elements (out of flex flow).
- **The save check-button uses a *lighter* gate than the per-question full validation.** `inlineFormHasDraftableContent = category !== 'custom' || forField.trim() !== '' || options.filter(o => o.trim() !== '').length >= 2`. Click still runs `getCurrentQuestionFormError()` inside `stageCurrentQuestion` so an invalid form (e.g. yes_no without a title) surfaces the error inline rather than failing silently. Resist the urge to fold this into the existing `inlineFormHasContent` callback — the latter is "has the user typed anything at all?" (used for auto-stage on Submit + projected-drafts preview, fires on the FIRST option character) while the new gate is "is there enough signal that we should offer an explicit save button?" (≥2 options). Different thresholds, different consumers; the proximity is intentional.
- **Staged draft rows are tap-to-edit + long-press-to-delete; no per-row pencil/trash buttons.** `DraftRow` (the row component inside `app/create-poll/page.tsx`) wires `onClick` → `handleEditDraft(i)` and `useLongPress` → `requestDeleteDraft(i)`. Long-press opens a shared `<ConfirmationModal>` with `pendingDeleteIndex` driving its `isOpen`. **Synthetic-click-after-touch-release race**: when the long-press fires the timer, the modal mounts; on touch release the browser dispatches a click at the touch coordinates, which now sit under the just-mounted modal backdrop — `onCancel` would close the modal on the same gesture. Gated by `deleteOpenedAtRef = useRef(0)` (set to `Date.now()` when opening), with `cancelDeleteDraft` returning early if `Date.now() - deleteOpenedAtRef.current < 400`. Same 400ms guard pattern as `FollowUpModal`. Don't extract this into a shared constant unless you're also refactoring FollowUpModal — both can drift independently.
- **Inline-form contributions to poll-level predicates must gate on `isModalOpen`.** `pollHasPrephase` / `pollHasRankedChoice` are the union of "any staged draft matches" + "the in-progress inline form matches". After confirm/dismiss, `applyDraftToState(emptyDraft())` resets the inline form to defaults — `questionType='question'`, `category='custom'`, no options — which incidentally satisfy `isSuggestionMode` (`questionType === 'question' && category !== 'yes_no' && category !== 'time' && hasNoOptions`) and `inlineFormIsRankedChoice` (`questionType === 'question' && category !== 'yes_no' && category !== 'time'`). Without an `isModalOpen` gate, those defaults wrongly light up the prephase predicates after every staged draft, surfacing "Suggestion/Availability Cutoff" + "allow voting before options are finalized" in Settings even when no question has a prephase. Don't try to fix this by tweaking `emptyDraft()`'s defaults (they're load-bearing for the open-modal first-paint UX) and don't reach for `inlineFormHasContent()` — that helper checks "has the user typed anything?" (used for auto-stage on Submit), which is a stricter gate than needed. The right gate is exactly "is the user editing right now?" which is `isModalOpen`. Same fix applies to any future poll-level predicate that mixes staged-draft state with inline-form state.
- **The Suggestion/Availability cutoff label is unified, not type-conditional.** Earlier the cutoff field flipped between "Suggestions Cutoff" (any non-time prephase) and "Availability Cutoff" (any time-question draft) via `drafts.some(d => draftDbQuestionType(d) === 'time')`. The label is now fixed as "Suggestion/Availability Cutoff" because (a) a multi-question poll can have BOTH a suggestion-mode question and a time question sharing the same cutoff, so picking one term hides the other, and (b) the toggle's gate is `pollHasPrephase` regardless of which kind. Don't reintroduce a conditional label — the unified form is correct for every combination.
- **Question-modal dismiss is backdrop-click + Escape only, no X button.** When extending the new-question modal in `app/create-poll/page.tsx`, the header is a centered title + right-side blue checkmark; clicking out discards. The pattern intentionally treats backdrop-click as the dismiss affordance to keep the chrome minimal. `dismissModal()` itself doesn't touch `drafts`, so dismissing on the first question (no draft poll yet) leaves `drafts.length === 0` and the draft poll card stays hidden — preserving the "first question dismissal doesn't materialize a draft poll" invariant without needing a special-case branch.
- **`useLongPress` requires a per-instance hook call, so `.map()` rows must extract a child component.** Hooks can't be called inside loops. `DraftRow` is the row-extraction in `app/create-poll/page.tsx`. Symptom of forgetting this pattern: "Rendered fewer hooks than expected" / "Hooks called conditionally" errors when adding/removing draft rows. Use this pattern for any list of rows that need per-row gesture state.
...
### CompactNameField Visibility (Hide When Set)

- **`CompactNameField` returns `null` when the user already has a name set + the field isn't being edited.** Both wrapper-level Submit blocks in `app/t/[threadShortId]/page.tsx` (poll multi-Submit + 1-question wrapper Submit), every legacy per-question Submit fallback (`QuestionBallot`, `RankingSection`, `SuggestionVotingInterface`, `TimeBallotSection`), and the create-poll voter-name block consume the same component and want the same hide-when-set behavior — the gate lives inside the component, not at the call site.
- **Pitfall: don't put the gate at the call site for stateful components.** The original create-poll commit wrapped `CompactNameField` in `{!creatorName.trim() && (...)}`. That looks fine but breaks first-time typing: user clicks "Add" → input mounts with focus → first keystroke calls `setName('J')` → parent re-renders → wrap evaluates false → entire component unmounts → focus is lost mid-keystroke. The internal gate (`if (!isEditing && name.trim()) return null;`) keeps the component mounted through the whole edit→blur cycle, so focus survives typing. General rule: any conditional render whose condition depends on state the component itself mutates must live INSIDE the component, otherwise the parent's re-render unmounts it on the first internal mutation.
- **Wrapping divs that exist only for spacing get `empty:hidden`.** When `CompactNameField` returns `null`, its wrapping `<div className="mb-3 …">` becomes empty and would still render its margin (12px gap below "nothing"). Tailwind's `empty:hidden` modifier maps to the CSS `:empty` pseudo-class — when no children render, `display: none` applies and the gap disappears. Use this anywhere a stateful component is wrapped purely for spacing and may render `null`.
...
- **`/t/<id>` polls `apiGetThreadByRouteId` every 5 seconds** (`app/t/[threadShortId]/ThreadPage.tsx`) so other users' new polls and votes appear without a manual reload. Recursive `setTimeout` (5s after the previous response resolved), NOT `setInterval` — a slow network can't pile up overlapping fetches; the `inFlight` flag rejects re-entry. Skipped when the tab is hidden (`document.visibilityState !== 'visible'`); a `visibilitychange` listener fires an immediate refresh on re-show. Skipped while ANY placeholder poll is in thread state — `POLL_PENDING_EVENT` / `POLL_HYDRATED_EVENT` owns that timeline and racing it would re-attach the placeholder after the user already saw the real card.
- **`mergePollListPreservingIdentity` (`lib/threadRefresh.ts`) keeps prev `Poll` references for polls whose content didn't change.** `ThreadCardItem`'s `arePropsEqual` compares `prev.group.poll !== next.group.poll` by reference — passing the SAME `Poll` object is the difference between zero and N card re-renders per refresh tick. Steady-state ticks (no remote writes since the last fetch) return `prev` from the merge → `setThread` returns `prev` from the updater → no re-render at all. Same pattern for `mergeQuestionResultsMap`: returns `prev` Map identity when no result-content changed. **Do NOT replace `thread` wholesale on a refresh** — even if every poll's data is unchanged, fresh `Poll` objects fail `arePropsEqual`'s reference check and every card re-renders.
- **`isPollContentEqual` covers every field that can change at runtime** — `is_closed`, `close_reason`, `response_deadline`, `prephase_deadline`, `prephase_deadline_minutes`, `thread_title`, `title`, `context`, `details`, `anonymous_count`, `updated_at`, `creator_name`, `voter_names` (length + values), and per-question `isQuestionContentEqual` (which itself walks `title`, `updated_at`, `response_count`, `question_index`, `poll_id`, `options`, `results`). When adding a new field that flips on user action (vote/close/edit), extend BOTH equality helpers OR a stale ref will be silently kept and the card won't re-render. The `updated_at` comparison alone is NOT sufficient: votes don't bump `updated_at` on the poll (the close-trigger only refreshes it on `is_closed` flips), and the FE-only `voter_names` aggregate is recomputed server-side per query.
- **Defer `loadVotedQuestions()` to inside the `setThread` updater's changed-content branch.** The function allocates two fresh `Set<string>` instances and parses localStorage every call. Calling it once per 5s tick means most ticks (the no-change steady state) pay the cost for no reason. Move the call below the `if (!merge.changed) return prev;` short-circuit so it only runs when a poll genuinely changed.
...
- **Coalesce concurrent API calls** with `coalesced()` in `lib/api.ts`. React StrictMode double-mounts effects in dev, causing two simultaneous calls to the same endpoint. Same idiom for `getMyThreads` and `getAccessiblePolls` — both use an in-flight promise to dedupe.
...
- **Destination pages that are navigated to frequently should initialize state synchronously from `questionCache`.** Example (`app/t/[threadShortId]/page.tsx`, `app/thread/[threadId]/page.tsx`): the `useState` initializer reads `getCachedQuestionById` / `getCachedQuestionByShortId` and uses the result directly. No loading spinner if cache hit.
