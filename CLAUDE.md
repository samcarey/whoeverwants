...
- **Floating "+" auto-follows-up** when on a thread page via `document.body.getAttribute('data-thread-latest-poll-id')` — the thread page sets this attribute on mount.
...
- **Thread-card respondent list uses `VoterList singleLine`** (under the card at `col-start-2 row-start-3`, wrapped in `flex justify-end` + `max-w-[75%]`). The mode hides the count/icon prefix, renders one horizontal row (`whitespace-nowrap overflow-hidden`), and collapses overflow into a trailing `+N` badge. Measurement is a `useLayoutEffect` + `ResizeObserver` on the container that walks each child's `offsetWidth`, reserves space for the `+N` badge, and sets `display: none` imperatively on items that don't fit (bubbles are imperatively hidden; the `+N` badge itself is React-state-driven via the `overflow` state + `style.display`). Keep the two mechanisms separate — don't imperatively set the badge's display inside the effect or React will fight the DOM on re-render.
- **Single pending-action confirmation modal.** Forget, Reopen, and Close Poll all share one `ConfirmationModal` driven by `pendingAction: { kind: 'forget' | 'reopen' | 'close'; poll: Poll } | null`. Per-kind copy (title/message/confirmText/confirmButtonClass) lives in a module-level `PENDING_ACTION_COPY: Record<PendingActionKind, ...>` lookup table. The modal is conditionally mounted (`{pendingAction && (...)}`) so each prop is a single lookup rather than parallel ternaries; `ConfirmationModal` already returns null on `!isOpen`, so no animation is lost. To add a new kind, extend the union + the table; don't rewrite the ternaries. The `onConfirm` body keeps the three-case `if/else` since each branch's state-update logic genuinely diverges.
- **Close Poll lives in the long-press modal, not the ballot.** `FollowUpModal` renders a red "Close Poll" button row (`onClosePoll` prop) when the poll is open AND `getCreatorSecret(pollId)` is known (or dev mode), parallel to the Reopen row's gating. The thread page wires `onClosePoll` → `setPendingAction({ kind: 'close', poll })` → shared ConfirmationModal → `apiClosePoll` + optimistic `setThread({ is_closed: true, close_reason: 'manual' })`. `PollPageClient` no longer carries its own close/reopen handlers, state, confirmation modals, or `PollManagementButtons` usage — all deleted as dead code when the button moved. If you see `handleCloseClick`/`handleReopenClick` referenced in any future PR, it's a merge conflict with stale code.
...
- **Thread card chrome lives outside the card.** Each thread item is a 2-col × 2-row CSS grid: col 1 row 2 holds the category icon, col 2 row 1 holds the above-card strip (creator/age left, status element right — `SimpleCountdown` / closed badge / "Taking Suggestions"), col 2 row 2 holds the bordered card itself (title + optional Not-voted / responses pills). The `row-start-2` placement of the icon cell pins its top to the card's top without a magic padding. `PollPageClient` no longer renders its own countdown inside the ballot; the above-card strip is the single source of status info. Time polls' deferred-deadline notice ("Availability cutoff Xmin after first response") still renders in the ballot because it conveys run-duration info the above-card "Collecting Availability" label doesn't — the parallel "Suggestions cutoff …" notice was removed.
- **Expanded card uses `pb-0.5`, collapsed uses `pb-2`.** The thread card wrapper picks bottom padding off `isExpanded`. Collapsed cards need `pb-2` for the compact header's breathing room; expanded cards trail with compact links (e.g., the gold "Your Ballot" edit link) where `pb-2` left visible dead space. When adding new trailing content to an expanded card, this padding is already tight — don't re-pad inside `PollPageClient` to try to fix trailing whitespace.
- **Above-card badge is skipped when `results.total_votes === 0`.** The `getResultBadge` helper returns `{ text: 'No voters', emoji: '🦗' }` when there are no votes, but the thread page's above-card status slot short-circuits to `null` in that case — the in-card preview already renders an empty state via `<EmptyNote text="No voters" />`. Adding a new poll type: either render an in-card empty state or flip the condition to match.
...
### Compact Preview Strips (Thread Card Lower-Right)

- **Every poll type has a compact "top result" strip in the lower-right of the thread card header, mounted regardless of `isExpanded` so the pill never remounts on toggle.** Yes/no uses `<YesNoResults hideLoser={!isExpanded}>`; the `hideLoser` prop internally animates the loser card's reveal when the user expands (one component morphs in place — no separate heavy view). Ranked choice / suggestion / time use smaller type-specific components (`CompactRankedChoicePreview`, `CompactSuggestionPreview`, `CompactTimePreview`) in `components/PollResults.tsx`. Each is wrapped in an **inverse grid-rows clip** (`grid-rows-[1fr]` when collapsed, `grid-rows-[0fr]` when expanded — opposite of the heavy-content expand clip below). The two clips animate in lockstep over 300ms: pill smoothly shrinks to 0 height while the rounds visualizer / time-slot bubble grid grows in to fill the same vertical space. No flicker (pill stays in DOM, no remount), no redundant duplication of winner info when expanded, and the header reclaims the pill row for the heavy content. Prior iteration #1 gated the pills on `!isExpanded` (caused unmount-flicker — instant 32px gap collapse during the 300ms grid-rows growth, leaving the corner empty). Prior iteration #2 left them rendered always (DOM-stable but visually duplicated the winner once in the pill and once in the rounds visualizer). Inverse-clip handoff is the synthesis. To add a preview for a new poll type: (a) add the component, (b) add a `poll.poll_type === '...'` block in `app/thread/[threadId]/page.tsx` wrapping the pill in the same inverse `grid-rows-[1fr]/[0fr]` clip + `overflow-hidden` child + `aria-hidden={isExpanded}`, (c) extend the `wantsResults` allowlist in `maybeFetch` so results get fetched when the card enters the viewport.
- **Shared pill primitives** in `PollResults.tsx`: `PILL_CLASS`, `PILL_COLORS_OPEN` (blue), `PILL_COLORS_CLOSED` (green). Reuse these rather than copying class stacks — the review agents flag divergence fast. Empty-state copy ("No voters", "No suggestions yet") is NOT rendered in the card's lower-right anymore — it lives below the card in the respondents row (see the "Respondent Row" section below). Every compact preview component (YesNoResults hideLoser path, `CompactRankedChoicePreview`, `CompactSuggestionPreview`, `CompactTimePreview`) returns `null` when empty and its wrapper at the callsite is also skipped so no `mt-2` gap lingers.
- **Closed polls show "Closed Xm ago" (faint) in the above-card status slot** — the in-card compact pills (Yes/No, CompactRankedChoicePreview, CompactSuggestionPreview, CompactTimePreview) are the single source of truth for the winner, so the above-card slot is repurposed for timing info only. Uses `compactDurationSince(closedAt)` from `lib/pollListUtils.ts`, which promotes to the next larger unit only when that unit's count would be ≥ 2 (13d stays `13d`; 14d becomes `2w`). The `closedAt` source is `response_deadline` when `close_reason === 'deadline'` (more accurate than `updated_at`, which would drift on subsequent edits), else `updated_at` (reliable for manual / max_capacity / uncontested closes — the DB trigger refreshes it on every `is_closed` flip). Don't call `getResultBadge` here — it's no longer imported into the thread page.
- **Time polls in the availability phase render "Collecting Availability" in the above-card strip**, not in-card — same slot and styling as "Taking Suggestions". Use `isInTimeAvailabilityPhase(poll)` (in `lib/pollListUtils.ts`) as the single check; the in-card `CompactTimePreview` returns null during that phase so nothing is duplicated.
