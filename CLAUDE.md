...
- **Explain decision OUTCOMES in plain language; fix the empty-brainstorm dead-end.** (Decision from the social-test review, May 2026.) Surface *meaning*, not just *mechanics*. **Actionable parts (a) + (b)-Layer-1 are SHIPPED** (see "Outcome legibility" bullet below); **(b)-Layer-2 (reconsider the headline aggregation) remains an open, owner-in-the-loop exploration.**
  ...
  - **(b) IRV result is opaque, and may be the wrong method for social groups.** Pure Instant-Runoff (Borda only as tiebreak, then alphabetical) eliminates the option with the FEWEST first-place votes each round — so a broadly-liked compromise (everyone's #2, few #1s) is eliminated EARLY and a polarizing plurality wins (documented in `social_tests/tests/test_ranked_preferences.py::test_condorcet_scenario`). For a friend group picking a restaurant, that's often just wrong — they usually want "the thing nobody hates," exactly what IRV throws out first. Two layers:
    - **Layer 1 (explain the result) — SHIPPED.** See the "Outcome legibility" bullet below — the result now carries a one-line plain-language gloss, and the broadly-acceptable-eliminated-early case is detected via full Borda scores and called out prominently.
...
- **Outcome legibility — ranked-choice result gloss + empty-suggestion terminal state (shipped).** Implements the actionable parts of the outcomes TODO above.
  ...
  - **`lib/rankedChoiceGloss.ts: rankedChoiceResultGloss(results)`** is the pure, unit-tested gloss generator (returns `{tone:'warn'|'info', text} | null`). `warn` fires when an *eliminated* option has a STRICTLY higher Borda score than the winner (the strongest such is named) — "‹X› was ranked on more ballots than ‹winner› but had fewer first-choice picks…". Else, for a ≥2-round result, a neutral `info` gloss explains how IRV reached the winner. Single-round majorities and ties/no-winner return null. `CompactRankedChoiceResults` renders it at the top, **gated on `isQuestionClosed`** so we never claim "eliminated early" while preliminary results are still moving. Strictly-greater (not ≥) avoids flagging an equal-breadth option. Tests: `tests/__tests__/ranked-choice/result-gloss.test.ts` (FE) + `server/tests/test_ranked_choice.py::TestFullBordaScores` (algorithm).
