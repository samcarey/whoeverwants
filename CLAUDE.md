...
- **Compact row height.** The status-line slots (category icon left, collapse-chevron right) default to `w-8 h-8` — bumping them up to `w-11` to host a bigger chevron adds ~12px of whitespace above every collapsed card's title. Keep slot sizes small; scale the SVG inside (`w-7 h-7` fits in a `w-8 h-8` button).
