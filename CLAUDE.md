...
- **Explain decision OUTCOMES in plain language; fix the empty-brainstorm dead-end.** (Decision from the social-test review, May 2026.) Surface *meaning*, not just *mechanics*. **Actionable parts (a) + (b)-Layer-1 are SHIPPED** (see "Outcome legibility" bullet below); **(b)-Layer-2 (reconsider the headline aggregation) remains an open, owner-in-the-loop exploration.**
  ...
  - **(b) IRV result is opaque, and may be the wrong method for social groups.** Pure Instant-Runoff (Borda only as tiebreak, then alphabetical) eliminates the option with the FEWEST first-place votes each round — so a broadly-liked compromise (everyone's #2, few #1s) is eliminated EARLY and a polarizing plurality wins (documented in `social_tests/tests/test_ranked_preferences.py::test_condorcet_scenario`). For a friend group picking a restaurant, that's often just wrong — they usually want "the thing nobody hates," exactly what IRV throws out first. Two layers:
    - **Layer 1 (explain the result) — SHIPPED.** See the "Outcome legibility" bullet below — every closed poll's result carries a one-line plain-language explanation, surfaced **behind an info (ⓘ) icon** (owner: "only behind info icon", June 2026 — never inline). For ranked-choice the broadly-acceptable-eliminated-early case is detected via full Borda scores and the icon tints amber to flag it.
...
- **Outcome legibility — ranked-choice result gloss + empty-suggestion terminal state (shipped).** Implements the actionable parts of the outcomes TODO above.
  ...
  - **`lib/outcomeExplainer.ts: outcomeExplainer(results)`** is the single pure, unit-tested entry point for the plain-language outcome one-liner across ALL decision types (returns `{tone:'warn'|'info', text} | null`). It switches on `question_type`: **yes_no** → "Yes won, 4 to 2." / tie / all-abstain / null on no-votes; **time** → "‹slot› works best — N of M can make it, and …" / null when the event was cancelled (the "event's off" banner is self-explanatory) or there's no winner; **ranked_choice** → delegates to `rankedChoiceResultGloss` (below), with a single-round-majority fallback so a decided IRV result always has a sentence; **limited_supply / other** → null (first-come, no "why this won"). Gated on `isQuestionClosed` at every call site (describes the final outcome, never an in-progress tally).
  - **`components/OutcomeInfoButton.tsx`** is the shared ⓘ affordance that reveals the explanation in a tap-to-toggle popover (click-outside / Escape to close, `stopPropagation` so the tap doesn't bubble to the result card's round-swipe / tap-to-detail handlers). `tone:'warn'` tints the icon amber. **The explanation is ONLY ever behind this icon — never rendered inline** (owner constraint). Wired into the closed-result view of `CompactRankedChoiceResults` (top-right), `QuestionResults.tsx: YesNoResults` (non-compact `!hideLoser` branch — compact group-card pills are untouched), and `TimeResults` (winner branch). Tests: `tests/__tests__/outcome-explainer.test.ts`.
