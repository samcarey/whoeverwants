...
- **Inventory of instrumented sites (commit moments only — UI state changes below "form-field threshold" deliberately uninstrumented):**
  ...
  - Create Group FAB: `app/template.tsx: CreateGroupButton.onClick` (medium).
...
- Home "+ Group" FAB → empty group (`slideToNewGroup` from `CreateGroupButton` in `app/template.tsx`)
