...
- **Group list row layout** (`components/GroupListItem.tsx`): NO hairline dividers between rows; the visual structure is avatar-driven, not chrome-driven. Each row is a flex with `[selection checkbox?] [unread-counter column] [avatar] [text block]`. The unread counter sits in its own fixed-width column (`w-7`) left of the avatar — always-reserved when an avatar renders, so indentation stays consistent whether or not unread items exist. The badge is `self-center` vertically with the avatar. Gap tuning is done with negative right-margin on the counter column (`-mr-[10.5px]` → 1.5px effective counter↔avatar gap) since flex `gap-3` (12px) is otherwise enforced uniformly; the avatar↔text gap uses `-ml-[3px]` on the text block (→ 9px effective). The outer `mr-1.5` (right-only) and the inner row's `pl-px` keep the left edge tight against the viewport's safe-area padding while preserving asymmetric right-side breathing room. Both are gated on `!draftMode` so the draft poll card on group pages keeps the symmetric `mx-1.5` + `pl-2` chrome it relies on for its dashed border.
- **Text block structure** (same component): row 1 is `flex items-baseline` with the title (`truncate flex-1`) + optional draft pill + the relative-time stamp (`shrink-0`); the time stamp moved upper-right and the "X questions" prefix is gone. Below that, an empty-group status row renders only when `createdAt` is null. The latest-poll body is a `text-sm leading-tight` div with `maxHeight: 2.55em` + `overflow: hidden` to cap at 2 lines — `-webkit-line-clamp` is NOT used because it's incompatible with floated siblings (the `-webkit-box` display ignores or breaks `float` descendants). To pin the countdown timer to "line 3" of the row's overall content (= line 2 of the poll body) regardless of how long the preview text is, render an invisible zero-width `float-right` phantom span on line 1 (height: 1.25em, no width), then a real `float-right clear-right` countdown that uses the phantom to skip down. Line 1's available width stays full (phantom `w-0` removes nothing); line 2 wraps around the countdown. Don't try to drop the phantom — without it, `clear:right` has no preceding right-float to clear and the countdown floats to line 1 instead. The `pr-4` (16px) right-edge buffer on the text block reserves whitespace so neither the wrapping text nor the right-aligned chrome (timestamp, countdown) touches the row's right edge.
