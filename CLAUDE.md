...
- **"Yes/No" is a CATEGORY, not display text.** Per user spec: the string "Yes/No" should not appear anywhere on the poll page header or in section headers. The yes/no question type is conveyed by the icon (👍) and the buttons below; duplicating "Yes/No" as a label is redundant and reads as if the category were the question prompt. Three layers enforce this:
  ...
  > **TODO — stale `"Yes/No?"` server tests (pre-existing, harmless, not yet fixed).** Three server tests still assert the retired single-question default title `"Yes/No?"` and fail against the current `"Question?"` behavior: `server/tests/test_polls_api.py::TestCreatePoll::test_create_single_question`, `server/tests/test_poll_title.py::TestSingleQuestion::test_yes_no_no_context`, and `server/tests/test_poll_title.py::TestEdgeCases::test_yes_slash_no_alias`. They've been failing since the `_single_question_default_title` "Yes/No?" branch was retired (see bullet 3 above) — the *code* is correct, the *assertions* are stale. Fix = update the three expected values from `"Yes/No?"` to `"Question?"` (a single-question all-yes_no poll with no context now titles "Question?"). Out of scope for the gap-3 notification work that surfaced them; spun off as its own cleanup so a future session can pick it up cold without re-deriving that these are pre-existing.
