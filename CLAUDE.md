...
- **Compact pill rendering on the card.** Status label (countdown / "Closed X ago" / "Taking Suggestions" / "Collecting Availability") is `float-right` at the top of the card so the title wraps around it; the question-type-specific compact pill stays in the lower-right of the card via a `clear-both` footer row when present. Single-question polls show one pill; multi-question polls stack one pill per sub-question in a `flex flex-col items-stretch gap-1 w-full min-w-0` column. Pills no longer use a `CompactPreviewClip` (the inverse grid-rows clip that animated to 0 on expand) since there's nothing to clip into anymore — they just render at natural height. Pills have no tap handler; clicks bubble to the card click handler that navigates. **Card frame padding uses `pt-1.5` always but `pb-0.5` only when a pill row renders, else `pb-1.5`** — otherwise titles without a pill row look top-heavy in the card. **`line-clamp-2` is intentionally absent from the title:** `-webkit-line-clamp` requires `display: -webkit-box`, which breaks float-around behavior. Long titles wrap freely; this is accepted, since user-typed yes_no prompts are the only uncapped source (auto-titles hit the server's 40-char limit).
