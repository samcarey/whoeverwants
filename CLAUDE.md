...
### Submit Morph (Optimistic Placeholder + FLIP)

Submitting a draft used to navigate to `/p/<newPollShortId>`. The user described the route change as a "page reload" feeling. The flow now stays on the current page and morphs the draft card into a real live poll card inline.

- **`POLL_PENDING_EVENT` + `POLL_HYDRATED_EVENT`** in `lib/eventChannels.ts` are the new cross-component event channels. On Submit, `CreateQuestionContent` (a) synthesizes an optimistic placeholder `Poll` from the draft state via `synthesizePlaceholderPoll` (id prefixed `pending-`), (b) caches it, (c) captures the draft card's bbox via `document.querySelector('[data-draft-poll-card]').getBoundingClientRect()`, (d) dispatches `POLL_PENDING_EVENT` with `{ poll, fromBbox }`, (e) clears draft state, and (f) starts `apiCreatePoll` in parallel. On API success, `POLL_HYDRATED_EVENT` swaps the placeholder fields for the real Poll keyed by the placeholder id — same React key, no remount mid-FLIP.
- **ThreadContent's POLL_PENDING listener** rebuilds the thread state to include the placeholder, then runs a JS-driven FLIP on the new card's bordered frame. The placeholder card mounts with `pendingPollFirstQuestionId === question.id` so the status row / voter circles / countdown are suppressed until hydration — only the title shows, matching the user's "render the new poll inside the draft frame with only the title visible" intent.
- **FLIP animates `width`/`height`, not `transform: scale`.** `scale(sx, sy)` stretches the rendered content (e.g. the title gets vertically stretched 5x when the draft is 5x taller than the live card). Setting inline `width`/`height` to the captured draft bbox + transitioning to natural pixel values physically resizes the wrapper while content sits at its real font size at the top. CSS can't transition to `auto` so we transition to explicit pixel values then clear the inline overrides after the animation finishes.
- **Target the bordered card div, not the outer grid wrapper.** The thread page renders each poll inside a 2-col grid (`grid-cols-[1.75rem_minmax(0,1fr)]`) where col 1 holds the category icon and col 2 holds the bordered card. FLIP'ing the grid wrapper expanded the icon column into the morph bbox; FLIP'ing the bordered card (`cardFrameRefs`) keeps the icon column still and morphs only the visible card frame.
- **View Transitions API was tried first but didn't fit.** The browser's `::view-transition-old(name)` / `::view-transition-new(name)` are bitmap snapshots — even with `width: 100%; height: 100%; object-fit: fill` they stretch as the group resizes (so the title squashes/elongates with it). For "the card frame physically resizes while content sits at its real size", a JS-driven FLIP on the actual DOM node is the correct tool. Keep `::view-transition-*` rules in `app/globals.css` only for page-level slide transitions — they're great for cross-page navigation, wrong for in-place content morphs.
...
- **Always cancel rAF + setTimeout in event-listener cleanups.** The FLIP schedules a `requestAnimationFrame` and a `setTimeout(..., 1050)` to clear inline styles. If the component unmounts mid-animation, both must be cancelled and the cleanup callback must bail with `if (!card.isConnected) return` before touching the (now detached) node's style.
