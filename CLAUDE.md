...
- **A custom (non-built-in) category can carry a creator-chosen emoji, stored on `questions.category_icon` (migration 127, nullable TEXT).** Built-in categories (Restaurant/Movie/Time/Yes-No/…) keep their fixed `BUILT_IN_TYPES.icon`; custom categories otherwise fall back to the generic question-type glyph (🗳️). `category_icon` is the override.
...
- **`components/CategoryEmojiField.tsx`** is the picker, rendered as a row inside the create-poll Category card gated on `!getBuiltInType(category)`. Collapsed = label + value input only; focusing the input expands a multi-line preset grid (no persistent square backgrounds — bare glyphs, hover + single-selected highlight only). Free-text entry is validated by `isEmoji()` (controlled input reverts on invalid + shows an "Enter a single emoji" hint). The grid is **sorted, not filtered**, by `rankEmojiOptions(categoryWord)`.
...
- **`effectiveCategoryIcon(draft)` (`app/create-poll/createPollHelpers.ts`) is the single source of truth** for "what icon does this draft persist" (trimmed custom emoji when `!getBuiltInType(category)`, else null). Used by BOTH `draftToQuestionParams` and `synthesizePlaceholderPoll` so the API payload + optimistic placeholder can't diverge. The render gate (`!getBuiltInType(category)` in page.tsx) is a separate concern (visibility), kept inline.
