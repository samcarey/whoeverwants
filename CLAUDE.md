...
- **`overflow: hidden` does not fully prevent `scrollTop` from advancing on iOS PWA.** Even when an ancestor is `overflow-hidden`, iOS WebKit will happily accept programmatic and gesture-driven scroll events and leave a non-zero `scrollTop` on the element — which pushes the `.pwa-safe-top` safe-area padding (and everything anchored to it) up behind the notch. On the thread page (`app/thread/[threadId]/page.tsx`), an effect walks the ancestor chain from `scrollListRef` up and attaches a passive `scroll` listener that snaps `scrollTop` back to 0 on `.pwa-safe-top` and `.safari-scroll-container`. Pair this with `overscroll-contain` on the inner scroll container and `touch-none` on fixed header bars.

### Portal Targets and Mount-Timing Races

- **Don't use a single `setTimeout` retry to find a DOM target that's mounted by a sibling component.** `CommitInfo` (in `layout.tsx`) needs the `#commit-badge-portal` element rendered by `template.tsx` behind its own `isMounted` flag. A 100ms retry worked on the home page but raced unpredictably on `/thread/...` and other routes where the template's mount effect commits later — leaving the commit-age badge missing for the rest of the session. Use a `MutationObserver` on `document.body` (with `childList: true, subtree: true`) that sets the target and disconnects as soon as the element appears. Single-shot retries become session-long bugs as soon as the timing shifts.
