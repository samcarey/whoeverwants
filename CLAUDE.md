...
- **Poll-level Context vs Notes** are two separate fields on the bottom panel: Context is a short single-line input that drives the auto-title's "for X" suffix (maps to `polls.context`); Notes is a multi-line textarea with link support (maps to `polls.details` — column added in migration 068, wired through `CreatePollRequest` / `PollResponse` / `_insert_poll` / `_row_to_poll` in this branch).
