...
2. Computes bundle ID + display name from `CAP_ENV` (`com.whoeverwants.app` / `Decide` for prod, `com.whoeverwants.app.latest` / `Decide α` for latest). No per-actor suffix. **The Home Screen label is `CFBundleDisplayName`, NOT the brand name.** iOS renders icon labels as a single line truncated to ~11-12 chars (width-based; wide caps count for more) with no wrap option — embedding a newline in the display name doesn't produce a two-line label, it collapses to one line. The old `WhoeverWants` (12) clipped and `WhoeverWants Latest` always truncated, so the label was deliberately shortened to a plain generic verb (`Decide`) that reads as "what the app does" rather than as the app's name (which avoids confusing the Home Screen label for the actual brand). The label is set in TWO places that must stay in sync: this workflow step's per-tier `DISPLAY_NAME` (the `plutil -replace` that actually ships) AND the committed `ios/App/App/Info.plist` default (only used by local Xcode builds). Changing `CFBundleDisplayName` is isolated — it does NOT touch the bundle id, the App Store Connect listing name, the APNS topic, or universal-link entitlements. Renaming does NOT reach already-installed apps via the WebView reload path; it needs a fresh TestFlight build + reinstall/update. The canary suffix is `α` (alpha), not `β` — the tier is "latest", not a beta.
...
Real-device feedback after shipping surfaced four refinements. None are blockers; tackle in a future session.

1. **Phase-transition push copy is too generic — make it option-specific + show the poll title.** The transition notification's title is the hardcoded "Voting is open" (`_notification_base` / `_build_transition_notification` in `routers/polls.py`; body is already the poll title). Desired: for ranked_choice suggestion polls, title like **"New options available"** (or "New options to rank"); for time/availability, something like "Time to vote". Surface the poll title prominently alongside. On-device the user only registered "Voting is open" — verify the APNS `aps.alert.body` (the poll title, set in `_send_apns`) is actually rendering on iOS; if iOS is collapsing to title-only, fold the poll title into the title string itself. Consider per-question-type copy (the payload builder has the poll row, so it can branch on whether any question is ranked_choice-with-suggestions vs time).
