...
**Gap 1 + 4 — IMPLEMENTED (claude/todo-selection-handling-EfPMu). What shipped, resolving the open questions with the spec's proposed defaults:**
  ...
  - **Notification + badge suppression.** `compute_badge_count` excludes Old polls **account-aware** (`old_poll_ids_for_browsers(bids)`, both to-do + unread modes). `fan_out_poll_closed` + `fan_out_phase_transition` exclude `'old'` recipients via the `_NOT_IGNORED` SQL fragment (mirrors `_PREF_JOIN`/`_PREF_ON_TRUE`) — **per-browser** for now; the cross-device account-union refinement on the FAN-OUT side (✕ on device A also silencing device B's push) is a documented follow-up, since push delivery isn't demoable. New-poll fan-out is untouched (a brand-new poll can't be Old yet). **Gap 3's "compose with Old-set suppression" TODO is now satisfied for poll-closed** (the `_NOT_IGNORED` clause).
