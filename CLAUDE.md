...
- **Explain decision OUTCOMES in plain language; fix the empty-brainstorm dead-end.** (Decision from the social-test review, May 2026.) Surface *meaning*, not just *mechanics*. **Actionable parts (a) + (b)-Layer-1 are SHIPPED** (see "Outcome legibility" bullet below); **(b)-Layer-2 (reconsider the headline aggregation) remains an open, owner-in-the-loop exploration.**
  - **(a) Empty-brainstorm cold start — SHIPPED.** A suggestion poll with zero suggestions was a dead-end: `cutoff-suggestions` correctly 400s (can't rank nothing), and a prephase deadline reached with zero suggestions auto-finalizes to an empty ballot. Shipped: (1) a blue empty-state nudge ("No suggestions yet — add the first idea below, or share the link…") in `SuggestionVotingInterface` while the phase is open and `existingSuggestions.length === 0`; (2) an explicit "No suggestions were added, so there was nothing to decide." terminal outcome on the closed-poll results view instead of the misleading "No Voters". (Still TODO if desired: *encourage* creator pre-seeding via `initial_suggestions` with a create-form hint — not built; brainstorms have a notorious cold-start problem.)
