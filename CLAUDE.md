...
**⚠️ STATUS (owner review, June 2026): direction LOCKED, NOT yet implemented.** A prototype of the original gaps (segmented Open/Decided filter + 3 layout variants + decisions-ledger drawer + "the plan" summary + outcome-notification body + home rollup) was built on branch `claude/group-hub-ui-variants-FzkNe`, deployed to one dev server with an on-device variant switcher, and compared by the owner. The owner picked the **segmented-filter** layout and then **redefined the semantics** into the follow/ignore model below. **That prototype code was reverted** (its value was the comparison); the branch was merged carrying only this spec. Implement the spec below from scratch in a future session.
