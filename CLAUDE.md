...
- **The home `ThreadList` appends `?thread=1` to its links.** The query-param key lives as a `THREAD_QUERY_PARAM` constant exported from `lib/threadUtils.ts`. `PollPageInner` reads it via `useSearchParams` to compute `fromThreadList`: when it's present AND the linked poll is voted-on AND closed (i.e. nothing actionable about the poll), it passes `initialExpandedQuestionId={null}` to `ThreadContent` and the destination's initial scroll lands at the bottom of the document so the user sees the draft poll form instead of an irrelevant expanded card. Direct deep-links (no `?thread`) always expand the linked poll regardless of voted/closed state — that path treats the URL as a "share this specific poll" and respects the share. When you add another consumer that needs the same flag, import `THREAD_QUERY_PARAM` rather than re-typing `'thread'`.
