...
- **Shared pill + empty-note primitives** in `PollResults.tsx`: `PILL_CLASS`, `PILL_COLORS_OPEN` (blue), `PILL_COLORS_CLOSED` (green), and `<EmptyNote text="…" />` for "No voters" / "No suggestions yet"-style fallbacks. Reuse these rather than copying class stacks — the review agents flag divergence fast.
