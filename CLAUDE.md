...
- **Endpoints in `routers/groups.py`** (between the join-request and Phase-G-invite sections):
  ...
  - `POST /{route_id}/members` body `{user_ids: [...]}` → `{added: N}`. Each id must (a) be a valid uuid (`_is_uuid_like`, skipped silently otherwise — NOT 404'd, so one bad id doesn't fail the batch), (b) NOT be the caller, and (c) be in the caller's `user_contacts` (`is_contact`) — so a member can't add an arbitrary stranger by guessing user_ids. `add_member_for_user` writes the `group_members` row keyed on the account's earliest-linked browser_id (same pattern as `decide_request`), returning `True` only when a NEW membership was established (already-a-member via any browser → skipped, no dup, no notification). Newly-added accounts get a `fan_out_member_added` push.
- **`fan_out_member_added(group_id, added_user_id, payload)` in `services/push.py`** targets `user_browsers WHERE user_id = added_user_id` (the invitee, not the group at large), gated on the same default-ON `notify_new_poll` pref as `fan_out_join_request` (a fresh member has no pref row → ON). Byte-for-byte the join-request fan-out shape with `uid = added_user_id`. Payload: title `Added to <group_phrase>` (via `group_name_phrase`), body `<inviter> added you to the group` (falls back to "You were added to a group" when the inviter has no display_name), `url: /g/<route>` (path form, not `?p=`).
...
- **Two kinds of plus-one (hybrid model):** a **freeform/unnamed** entry is weighted on the submitter's row (`plus_one_names`, can't be edited later); a **looked-up account** (picked from the contact dropdown → carries `userId`) gets its OWN **seeded, editable vote** attributed to that account so they can change it later. `SubmitPollVotesRequest.plus_one_user_ids` carries the looked-up accounts; `_seed_plus_one_votes` (routers/polls.py) creates one vote per question for each, mirroring the submitter's choice, attributed via the account's earliest browser, voter_name = their `display_name`. **It does NOT auto-add them to the group** — instead it mints a single-use **invite** (Phase G `issue_invite`, `target_poll_id` = the poll) and returns a per-account notification spec; `submit_poll_votes` fans those out as pushes (reusing `fan_out_member_added`, which targets one user's browsers) after the response. The seeded vote counts in the tally immediately even though they're not a member yet; tapping the invite (`/invite/<token>`) lets them join + land on the poll, where the detail page's discovery pass adopts the seeded vote for editing. **Guards:** only the submitter's contacts (`is_contact`), skip self, skip accounts that **already responded** (`user_responded_to_poll` — never overwrite their own vote), each seeded once. Anonymous submitters (no account) can't seed (no contacts). Seeded votes weigh 1 (no plus-ones of their own).
...
- **TODO / follow-ups (deferred):**
  ...
  - **`fan_out_member_added` is reused** as the per-user push for the plus-one invite — the name is now a slight misnomer (it's "notify this one user, gated on the group pref"). If a third caller appears, rename to a generic `fan_out_to_user` / add `fan_out_plus_one_invite`.
...
- **Ballot self-submits on tap** (`components/QuestionBallot/LimitedSupplyBallot.tsx`): "Claim a spot" / "No thanks" post immediately via QuestionBallot's `submitVote` (speed matters), using the `pendingSupplySubmit` deferred-submit pattern (mirrors binary-RC tap). It is EXCLUDED from the wrapper-Submit machinery (`SELF_SUBMIT_TYPES` in `PollDetailPage`, alongside yes_no) and `prepareBatchVoteItem` returns `{skip}` for it so a multi-question wrapper Submit can't double-claim. The ballot shows live "N of M spots left" / "Full · K waitlisted", the viewer's own secured/waitlist status (matched by their own vote's `created_at` against the roster), and the signup roster.
...
- **OPEN QUESTIONS (owner to confirm):** (a) auto-close when full vs keep collecting a waitlist (currently keeps); (b) keep the explicit "No thanks" decline vs treat non-claim as implicit decline; (c) per-person multi-slot claims (currently one slot/person — see the TODO in `QuestionBallot.tsx`); (d) default for the name-visibility toggle (currently visible).
...
- **TODO (pre-existing test debt, not limited-supply):** `server/tests/test_poll_title.py::TestSingleQuestion::test_yes_no_no_context` and `::TestEdgeCases::test_yes_slash_no_alias` fail on `main` — they expect `generate_poll_title(["yes/no"], None) == "Yes/No?"`, but `_single_question_default_title` intentionally returns `"Question?"` for yes_no (the FE requires a user-typed title for yes_no, so the auto-title fallback is unreachable in practice). Either update the two tests to expect `"Question?"` or restore the "Yes/No?" branch — the code comment says the branch was deliberately retired, so the tests are stale. Surfaced during the limited-supply work; out of scope there.
