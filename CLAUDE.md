...
- **Sheet height is fixed (`height: calc(100dvh - 3rem - env(safe-area-inset-top, 0px))`), not content-driven (`maxHeight`).** Using `maxHeight` made the panel size to its content, so a short form (e.g. yes_no with no options yet) only slid up partway — the slide-up animation ended at a visually arbitrary height that depended on how many fields were rendered. Fixed height pins the top edge to a consistent location regardless of form state; the inner `overflow-y-auto` already handles tall content, and short content just gets empty space below. Don't switch back to `maxHeight` to "tighten" the visual — the inconsistent top edge is more disorienting than the empty space. **The `- env(safe-area-inset-top)` term is load-bearing on iOS**: the sheet is bottom-anchored (`flex items-end`), so its top edge sits at `100dvh - height` = `3rem + safe-area-inset-top` from the physical top. Without subtracting the inset, the top edge sat at a flat 48px (`3rem`) — right where the Dynamic Island / notch lives on iPhone X-class devices (`safe-area-inset-top` ≈ 47–59px), so the sheet's rounded top corners intersected the island. `100dvh` on iOS includes the area behind the notch (`viewport-fit=cover`), so the inset has to be subtracted explicitly. On non-iOS the inset resolves to 0 and the height is unchanged.
