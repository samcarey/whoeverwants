...
- **OPEN QUESTIONS (owner to confirm):** (a) auto-close when full vs keep collecting a waitlist (currently keeps); (b) keep the explicit "No thanks" decline vs treat non-claim as implicit decline; (d) default for the name-visibility toggle (currently visible). **(c) per-person multi-slot claims — RESOLVED:** done via plus-ones (see the "Multi-slot claims" + "Two submit modes" bullets above); not a quantity stepper.
