...
- **Thread-page scroll-helper buttons (up/down) live at `app/p/[shortId]/page.tsx`** as a single `<ScrollHelperButton>` component portaled into `#floating-fab-portal`. Visibility is driven by a `useEffect` that calls `getBoundingClientRect()` on every awaiting card anchor + `document.querySelector(SUBMIT_POLL_SELECTOR)` (the `[aria-label="Submit poll"]` of the draft poll's Submit button). The handler is wired to `scroll`/`resize` AND a `MutationObserver` on `document.body` subtree — the body observer is required because the draft Submit button is portaled in by `CreateQuestionContent`'s lazy mount, AND because vote-driven re-renders flip a card's awaiting border without firing scroll. **Always rAF-coalesce a body-subtree MutationObserver that calls `getBoundingClientRect()`.** Without coalescing each mutation burst (every vote, every expand/collapse, every countdown text update from non-React paths) triggers N+1 forced layouts (one rect-read per awaiting card + one per submit button). Pattern: a `let rafId: number | null = null;` + `const schedule = () => { if (rafId !== null) return; rafId = requestAnimationFrame(evaluate); };` + clear `rafId` at the top of `evaluate`. Cancel in cleanup.
- **Up-arrow visibility uses an "any-above" gate, not a strict "none-visible" gate.** "All awaiting cards scrolled off above the viewport" almost never triggers in practice — most threads aren't tall enough below the awaiting block (the awaiting cards sort to the bottom by `created_at` ascending, with only the draft form below them) to push every awaiting card above the viewport. Showing the arrow as soon as ANY awaiting card has been scrolled past surfaces the affordance when it's actually useful (rewinding to the oldest unresponded poll). The "oldest awaiting above viewport" is the FIRST awaiting we encounter in DOM order whose `bottom <= headerHeight` — DOM order matches the sort, so first match = oldest.
- **Down-arrow targets the draft Submit button's vertical center, up-arrow targets the awaiting card's top flush with header bottom.** `targetY = scrollY + r.top + r.height/2 - innerHeight/2` for "center the element"; `targetY = scrollY + r.top - headerHeight` for "flush below the fixed header". Both use `window.scrollTo({ top, behavior: 'smooth' })` — the rAF-driven animation pattern documented elsewhere is only needed when scrolling alongside an in-flight grid-rows expand (since `scrollHeight` clamps mid-animation).
...
- **Within-thread sort puts closed/expired polls first, non-expired second.** `lib/threadUtils.ts: collectDescendants` pre-computes `expiredById: Map<pollId, boolean>` once before the sort so the comparator is O(1) per compare instead of re-parsing `response_deadline` on every comparison. Result: the most recently submitted non-expired poll always lands at the very bottom of the polls list, just above the always-on draft card. `latestActivityMs` is computed as the true `max(created_at)` across all polls (separate from the post-sort tail) so the home page's cross-thread "by recency" sort still works regardless of in-thread order.
