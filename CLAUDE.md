...
**Gap 1 + 4 — SHIPPED + MERGED (#575 follow/ignore tabs + home rollup; #576 account-aware Old-set push suppression). What shipped, resolving the open questions with the spec's proposed defaults:**
  ...
  - **FE.** `lib/followState.ts`: pure `classifyPollTab(poll, voted, abstained, nowMs)` (Old → 'old'; followed + needs-input → 'todo'; else 'new') + `tallyFollowTabs`. NOTE: `pollNeedsInput` intentionally diverges from `isPollUnread`'s todoMode (it COUNTS active-prephase-contributable polls as To Do; the badge does not) — cross-referenced in both files so the difference reads as deliberate. `Poll.viewer_follow_state` on the type + `toPoll`; `apiSetPollFollowState(pollId, state)` patches the per-poll + accessible-polls caches; `isPollContentEqual` (groupRefresh) includes the field so a cross-device flip reflects on the 5s refresh. Group page (`GroupPage.tsx`): a sticky **To Do · New · Old** segmented control (default tab = To Do if any, else New; `selectedTab` null until tapped), filters `visibleGroupedQuestions` (New shows every followed poll — To Do ⊆ New), empty-state per tab. `GroupCardItem`: inline red ✕ (ignore) / green + (re-follow) button in the title row, `stopPropagation` so it never triggers tap→detail / long-press, optimistic `patchGroupPolls` + revert. Home rollup (Gap 4): `GroupList`/`GroupListItem` show "N to do · M new" per group row.
...
- **Turnout context on the group card ("M of V seen") — SHIPPED.** The viewer-vs-voter distinction is a social signal — a 1-vote poll that 6 people OPENED is "low turnout," not a decisive result. `PollResponse.viewed_total` (FE `Poll.viewed_total`) is a counts-only distinct-viewer total over `poll_views`, computed in `_compute_poll_voter_data` (returns a **`PollVoterData` dataclass** — `voter_names`, `anonymous_count`, `viewed_ignored_count`, `viewed_total`, `suggestion_count`; was a positional tuple, converted in the simplify pass so adding a field doesn't churn 8 callsites) as `COUNT(DISTINCT COALESCE(ub.user_id, pv.browser_id))` via `LEFT JOIN user_browsers` — so a signed-in viewer on two devices counts once (account-aware, mirroring `load_user_visibility`). NO 5-min / no-action filter (unlike `viewed_ignored_count`); it's the raw "how many saw it" denominator. **`_compute_poll_voter_data` runs exactly 2 queries** — one over the poll's votes (voter_names + anon + `suggestion_count` via `unnest(votes.suggestions)`), one over `poll_views` (`viewed_total` + `viewed_ignored_count` together via `COUNT(DISTINCT COALESCE(...))` + a `COUNT(*) FILTER(...)`); don't split these back out per-metric (it's called per-poll in the `polls_for_poll_ids` 5s-refresh loop). `_row_to_poll` takes the `PollVoterData` directly; threaded → `toPoll` → `isPollContentEqual` (so the card re-renders when it changes on the 5s refresh). The group card (`GroupCardItem`) shows these as text labels (NO emojis): `viewed_total` is `{N} View(s)` on the bottom-LEFT (after author·date); `voted` (= `voter_names.length + anonymous_count`) and `suggestion_count` are in the bottom-RIGHT corner cluster (see the Row-layout bullet's Row 3). `suggestion_count` counts distinct non-empty `votes.suggestions` across the poll's ranked_choice questions. **The respondent/viewed bubbles were removed from the group card** and now live only on the poll detail page. Counts only — viewer identities never cross the API (consistent with the ballot-privacy TODO). The poll-detail "Viewed (N)" roster was left unchanged (it conveys turnout via the bubbles + the "N viewed but haven't responded" ignored sub-line). Tests: `server/tests/test_notification_events.py::test_viewed_total_counts_distinct_viewers`, `::test_viewed_total_collapses_account_browsers`, `::test_suggestion_count_distinct`. **Demo caveat:** a CLOSED poll is hidden from a fresh viewer by the closed-before-join filter, so a live demo uses OPEN polls.
  ...
  - **TODO (not built) — per-user "remind me to vote" notification setting.** A RECIPIENT-side preference each user sets in THEIR OWN Settings page to control reminders **to themselves** (it is NOT a creator-side field in the create-poll form — the poll's creator does not control it). Behavior: at a chosen lead time before a poll auto-closes, IF the user has access to the poll but still hasn't voted AND hasn't ignored/moved the poll to their Old tab, they get a reminder push. The setting is a dropdown of a few fractional choices (of the poll's open duration) + a few absolute lead times + an **"Off"** option; **default `0.2x`** (fire when 20% of the poll's open window remains, i.e. 0.2 × duration before auto-close). Reuse the create-poll suggestion-cutoff dropdown widget/option style to the extent reasonable.
    - **Design notes for whoever builds this (read before starting):**
      - The setting is the RECIPIENT's, stored **per-user + account-synced** (mirror the badge-settings / `display_name` pattern: a `users` column on `UserSummary`/`SessionUser` + a `POST /api/auth/me/...` writer, with a localStorage mirror so anonymous/browser-only users get it too). NOT per-poll, NOT in the create form.
      - Fire time is **per-(recipient, poll)**: `poll.response_deadline − offset(poll.open_duration)` where the offset comes from the recipient's global setting and `open_duration = response_deadline − created_at`. So the cron tick (`POST /api/internal/tick`) would, for each OPEN poll that has a `response_deadline`, evaluate each not-yet-acted member's reminder setting to decide whose reminder is due now — or precompute per-(member, poll) reminder rows. Track sent-state per (recipient, poll) so each fires once.
      - Reuse the existing gates: Old-set suppression (moving the poll to Old silences it like every other push — reuse the `_NOT_IGNORED` SQL fragment in `services/push.py`), the per-group mute pref, a push subscription, and "hasn't voted/abstained" (a `_NOT_VOTED`-style account-aware fragment: `NOT EXISTS` a `votes` row on any of the poll's questions attributed to a browser in the member's account-union). Only applies to polls with a voting deadline.
    - **Caution:** an earlier session mis-built this as a CREATOR-side per-poll dropdown in the create-poll form (migration 136 — `polls.reminder_at`/`reminder_sent` + a tick pass + `fan_out_poll_reminder`; reverted before merge). Do NOT revive that shape — it's a recipient-side Settings preference, not a poll property. The reverted code is in branch history if the `_NOT_VOTED` fragment / tick-pass plumbing is a useful starting reference.
...
- **Duplicate flow hard-sets `collectAvailability = true`** — the duplicate snapshot doesn't carry the creator's windows, so a no-availability copy couldn't derive slots; re-open the two-phase flow instead. (Mirrors the `collectSuggestions` duplicate-flow caution.)
...
- **TODO / follow-ups (deferred):**
  - **Notification delivery is untestable in CI** (needs a real push subscription) — only recipient-selection + invite minting are covered. Verify on a real device/TestFlight.
  - **Cross-device editing requires the represented person be signed in** on the device where they open the invite (the seeded vote attributes to their account's browser; the detail-page discovery pass finds it via the account-scoped `/votes`). Anonymous-on-a-fresh-device can't reach it.
...
- **Name gate (SHIPPED):** a nameless user tapping Claim/Decline would otherwise hit the server's `400 "name is required"` (surfaced as a raw vote error). limited_supply self-submits on tap and bypasses the wrapper-Submit name gate, so `QuestionBallot` takes an optional `onRequireName?: (retry) => boolean` prop that `handleSupplyClaim`/`handleSupplyDecline` consult before firing the deferred submit; `PollDetailPage` passes its `gateOnName` (scoped to `question_type === 'limited_supply'`, since yes/no's self-submit is gated separately via `dispatchYesNoTap`). The gate stashes the tap as a retry and replays it once `AccountGateModal` saves a name — `submitVote`'s existing `getUserName()` fallback then picks up the freshly-saved name. The prop is the canonical hook for any future self-submitting ballot type that needs the name gate; don't re-inline a getUserName check in the handler.
