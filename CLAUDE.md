...
### Document Scroll Architecture + Pull-to-Refresh
...
- **Custom PTR runs on all mobile touch devices, not just iOS PWA.** Native browser PTR is unreliable in some configurations (stale state after navigation, inconsistent across PWA/browser), so `template.tsx` ships one implementation that covers both. It's gated on `('ontouchstart' in window || navigator.maxTouchPoints > 0) && matchMedia('(pointer: coarse)')` to exclude desktops with touchscreens. Listeners are attached to `window`, and the drag transform is applied to `document.body`.
- **PTR must not fire when a nested scroller is mid-scroll.** `nestedScrollerAboveTop()` in the PTR touchstart walks from `e.target` up to `body`; if any ancestor with `overflow-y: auto|scroll` has `scrollTop > 5`, PTR refuses to engage (the user is scrolling that element, not the page). The walker reads cheap props (`scrollTop`, `scrollHeight`) before falling back to `getComputedStyle` — reordering saves per-touch work on deep trees.
...
- **NEVER use `e.preventDefault()` in touchmove on a scrollable element.** On iOS, calling `preventDefault()` on even a 1px touchmove causes the browser to classify the entire gesture as non-scrollable, permanently blocking scroll for that touch sequence. Use `overscroll-behavior: none` on `html, body` (globals.css) instead to suppress bounce. All PTR touch listeners must be `{ passive: true }`.
...
- **Never leave `translateY(0px)` on `<body>` after a PTR gesture.** Clear the transform with `body.style.transform = ''`, not a zero-translateY — even though visually identical, `translateY(0px)` is a real CSS transform that creates a containing block on iOS and can break fixed children.
...
- **Body gets horizontal safe-area padding** (`padding-left/right: env(safe-area-inset-left/right)`); vertical safe-area insets are handled per-element by whatever sits at the top/bottom (fixed thread header, home/profile titles via `.page-title-safe-top`, bottom bar via `.pwa-bottom-bar`).
...
### Bottom Bar Scroll-Hide (Document Scroll)

- **Scroll-hide listens to `window` scroll + touchstart/touchend events.** Touch events are the primary mechanism (reliable on iOS), scroll events are a desktop mouse-wheel fallback. Read `window.scrollY` for position.
- **Reserve space for the bottom bar by its measured height**, not a hardcoded Tailwind class. The old `mb-14` (56px) didn't match the bar's actual rendered height in the browser (~50px — `py-1.5` content + 1px border, no safe-area inset on web). Use a `ResizeObserver` on the bar, store the height, and animate `padding-bottom` on a wrapper with `transition: padding-bottom 200ms ease-out` matching the bar's slide transform so the bar's top edge stays flush with the content as it hides.
- **Thread pages must force the bottom bar visible on pathname change**, not just suppress hides during scroll. If the bar was hidden on a previous page and the user navigates to a thread without scrolling, it stays hidden. The pathname-tracking effect should call `setShowBottomBar(true)` whenever the new path starts with `/thread/`.
