...
- **KNOWN PRE-EXISTING (not introduced by the scroll-restore work): the scroll-helper arrows briefly double during the overlay→real-route handoff.** During a back-slide, BOTH the slide overlay's `GroupContent` and the real route's `GroupContent` render their own arrow set (and their own `BubbleBarPanel`), so for ~the slide duration there are two overlapping arrow sets (`2→4→2` on a tall group; `1→2→1` on a short one — verified identical on `latest`/main). They sit at the same coordinates so visually it usually reads as one, but a real-device render-lag at the moment the overlay unmounts can flash it. This is the slide-overlay dual-instance pattern, not the scroll pin. TODO if it proves user-visible: have the overlay's `GroupContent` suppress its scroll-helper arrows (e.g. an `inOverlay` prop gating the portal) so only the real route renders them during the handoff.
