...
- **Expanded card uses `pb-0.5`, collapsed uses `pb-1`.** The thread card wrapper picks bottom padding off `isExpanded`. Collapsed cards use `pb-1` (4px) so the status/pill footer row sits snug against the card edge; expanded cards trail with compact links (e.g., the gold "Your Ballot" edit link) where extra `pb` left visible dead space. When adding new trailing content to an expanded card, this padding is already tight — don't re-pad inside `PollPageClient` to try to fix trailing whitespace.
