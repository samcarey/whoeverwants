...
- **Every poll type has a compact "top result" strip in the lower-right of the collapsed thread card.** Yes/no uses `<YesNoResults hideLoser>` rendered outside the expand clip (DOM-stable across toggles). Ranked choice / suggestion / time use smaller type-specific components (`CompactRankedChoicePreview`, `CompactSuggestionPreview`, `CompactTimePreview`) in `components/PollResults.tsx`, rendered by the thread page only when `!isExpanded`. To add a preview for a new poll type: (a) add the component, (b) add a `!isExpanded && poll.poll_type === '...'` block in `app/thread/[threadId]/page.tsx` right after the yes/no external block, (c) extend the `wantsResults` allowlist in `maybeFetch` so results get fetched when the card enters the viewport.
