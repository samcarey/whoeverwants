...
async function applyFunctionFix() {
  try {
    ...
    const sql = fs.readFileSync('scripts/fix-ranked-choice-function.sql', 'utf8');
    
    console.log('🔄 Applying function fix to test database...');
    ...
    if (response.ok) {
      ...
      console.error('❌ Failed to apply function fix');
