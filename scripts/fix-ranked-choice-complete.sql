...
BEGIN
    ...
    LOOP
        ...
        -- Count ACTIVE ballots (those with at least one non-eliminated candidate, excluding abstain votes)
        ...
        vote_counts AS (
            SELECT 
                ...
                COUNT(*) as vote_count
            FROM (
                -- For each ACTIVE ballot, find the highest-ranked non-eliminated option (excluding abstain votes)
        ...
        ELSE
            ...
            -- First, determine which candidate to eliminate based on lowest Borda score
            ...
            -- Store Borda scores for ALL tied candidates
            ...
            WHERE ranked_choice_rounds.poll_id = target_poll_id 
              ...
              AND ranked_choice_rounds.option_name = atb.tied_candidate;
        END IF;
    ...
    -- Return the winner
    RETURN QUERY SELECT winning_option, current_round - 1;
END;
$$ LANGUAGE plpgsql;