...
from middleware import BrowserIdMiddleware, IdentityMiddleware, RateLimitMiddleware
...
# Phase A: resolve `Authorization: Bearer <session-token>` into
# `request.state.user_id`. Anonymous requests (no header) skip the DB
# entirely. Independent of BrowserIdMiddleware — they don't read each
# other's state, so ordering is for readability only.
...
# `Authorization` is explicitly listed in `allow_headers` even though `*`
# is set — some browser CORS implementations (older Safari + iOS WebView
# under specific configurations) honor the wildcard for simple headers
# only and reject Authorization unless named. Cheap belt-and-braces.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=False,
    allow_methods=["*"],
    allow_headers=["*", "Authorization", "X-Browser-Id"],
