...
async function updateSqlReverseAlpha() {
  try {
    console.log('Updating SQL function to eliminate alphabetically LAST (reverse order)...')
...
BEGIN
    ...
    LOOP
        ...
        -- Check if we have a winner (majority of votes) - use consistent ordering
        ...
        ELSE
            ...
            -- First, calculate and store Borda scores
            ...
            -- Update the rounds with Borda scores
            ...
            -- NEW: Eliminate alphabetically LAST when Borda tied (DESC order)
            WITH borda_scores_2 AS (
                SELECT 
                    tied_candidate,
                    SUM(borda_points) as total_borda_score
                FROM (
            ...
            all_tied_with_borda_2 AS (
                SELECT 
                    tc.tied_candidate,
                    COALESCE(bs.total_borda_score, 0) as total_borda_score
                FROM unnest(tied_candidates) AS tc(tied_candidate)
                LEFT JOIN borda_scores_2 bs ON tc.tied_candidate = bs.tied_candidate
            )
            SELECT ARRAY[tied_candidate] INTO options_to_eliminate
            FROM all_tied_with_borda_2
            ORDER BY total_borda_score ASC, tied_candidate DESC  -- CHANGED: Eliminate Z before A when tied
            LIMIT 1;
        END IF;
...
$$ LANGUAGE plpgsql;
    ...
    const response = await fetch(`https://api.supabase.com/v1/projects/kfngceqepnzlljkwedtd/database/query`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      console.log('SQL function updated to eliminate alphabetically LAST!')
    } else {
